...
Codex and Claude token history uses the vendored, commit-locked `cc-statistics` 1.1.0 Python core as its only parser and normalization source. Its Farming adapter persists per-file byte checkpoints and normalized events in `history/cc-statistics-usage-v1.sqlite3`; unchanged files never reread JSONL content, appends start at the committed offset, and truncation or prefix replacement rebuilds only that file. The adapter retains 52 weeks by default, keeps exact recent events, aggregates older output by session and hour, and prunes expired event rows while preserving continuation state. Do not restore whole-history `rg`, JavaScript token parsing, or whole-file reads on the refresh path. The pure-Python core is OS-independent, requires Python 3.10 or newer, and has no third-party Python dependencies; a missing runtime must make token usage visibly unavailable instead of selecting another parser.
