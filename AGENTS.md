...
- Add or update both `release-notes/vX.Y.Z.md` and `release-notes/vX.Y.Z.zh_cn.md`. The package version, Git tag, note filenames, H1, bilingual links, uppercase milestone codename, and upgrade target must match exactly. Run `node scripts/verify-release-notes.mjs X.Y.Z`; the Release workflow must derive the title `Farming X.Y.Z · CODENAME` from that validated metadata and publish the English note as its body.
