...
Codex and Claude token history uses the vendored, commit-locked `cc-statistics` 1.1.0 Python core as its only parser and normalization source. Its Farming adapter persists per-file byte checkpoints in `history/cc-statistics-usage-v1.sqlite3`; unchanged files never reread JSONL content, appends start at the committed offset, and truncation or prefix replacement rebuilds only that file. Long-term Codex usage is written directly as source/session/local-hour buckets, while the recent window keeps exact events and Claude keeps only the event rows required for cross-file message de-duplication. The adapter retains 52 weeks by default and prunes expired rows while preserving continuation state. A cold rebuild scans newest files first, returns a bounded partial snapshot, and continues through one background scanner. Incompatible derived-cache schemas must delete and recreate the database plus its WAL/SHM sidecars instead of migrating large event tables in place. Do not restore whole-history `rg`, JavaScript token parsing, raw long-term Codex event rows, or whole-file reads on the refresh path. The pure-Python core is OS-independent, requires Python 3.10 or newer, and has no third-party Python dependencies; a missing runtime must make token usage visibly unavailable instead of selecting another parser.
