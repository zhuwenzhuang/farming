...
Release packages carry the exact reviewed Codex and Claude ACP JavaScript runtimes, but must not carry the transitive platform Agent CLIs from `@openai/codex-*` or `@anthropic-ai/claude-agent-sdk-*`. Codex CLI, Claude Code CLI, and `agent-browser` are startup dependencies rather than package dependencies. One checked-in manifest pins each supported platform artifact, source URL, integrity, executable entry, and expected version. A fresh launcher may reuse an exact-version system Codex or Claude executable; `agent-browser` is always downloaded into and launched from Farming's verified immutable cache. Otherwise the launcher downloads, verifies, stages, and atomically activates each cache entry before the Server opens its port, then supplies `CODEX_PATH`, `CLAUDE_CODE_EXECUTABLE`, and `FARMING_AGENT_BROWSER_BIN` to runtime consumers. Concurrent starts share one preparation lock, corrupt cache entries are replaced without becoming active, and a failed preparation must leave the Server stopped with an actionable error. npm and bundle updates run the new release's dependency preparation while the old Server remains live and may become `ready-to-restart` only after it succeeds; failure leaves the old package and Server untouched. Only after the new Server is listening may it prune dependency cache entries not selected by its active manifest; cleanup failure is visible but does not turn a healthy Server startup into failure. Package smoke must prove that both vendored adapters complete ACP `initialize` with platform packages absent from the release.
