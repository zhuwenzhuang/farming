...
Codex and Claude token history uses the vendored, commit-locked `cc-statistics` 1.1.0 Python core as its only parser and normalization source. Its Farming adapter persists per-file byte checkpoints in `history/cc-statistics-usage-v1.sqlite3`; unchanged files never reread JSONL content, appends start at the committed offset, and a destructive Codex rewrite rebuilds the provider ledger. The first Codex `SessionMeta` is the rollout owner; later metadata and token events may be copied fork history. Farming de-duplicates those copied events through compact lineage-bound usage fingerprints, assigns the canonical event to its earliest source record, and keeps long-term totals in session/local-hour buckets while the recent window keeps exact events. Claude keeps only the event rows required for cross-file message de-duplication. The adapter retains 52 weeks by default and prunes expired rows while preserving continuation state. A cold rebuild scans newest files first, returns a bounded partial snapshot, and continues through one background scanner. Incompatible derived-cache schemas must delete and recreate the database plus its WAL/SHM sidecars instead of migrating large event tables in place. Do not restore whole-history `rg`, JavaScript token parsing, duplicate Codex fork events, raw long-term Codex event rows, or whole-file reads on the refresh path. The pure-Python core is OS-independent, requires Python 3.10 or newer, and has no third-party Python dependencies; a missing runtime must make token usage visibly unavailable instead of selecting another parser.

Interactive runtime sessions default to the native pty host. The host uses a Farming-specific local socket derived from `configDir`, keeps PTY processes outside the Farming server process, and exposes recovery metadata to the server after restarts. By default, same-revision server shutdown preserves the host for restart recovery; after the last live session and client disappear, the host shuts itself down after a short idle grace period. The server and host exchange a runtime code fingerprint when connecting. An application upgrade or fingerprint mismatch performs a transactional controlled rotation: block new mutations, drain and freeze the exact reducer cut, serialize every still-live Terminal, require a matching preparation token to stop the old host, and revive the serialized screen in a new PTY epoch. Serialization failure must resume the old host and abort rotation. A Codex Terminal that has accepted user input but still lacks an exact provider session id is likewise not restartable: abort the rotation and resume the old host instead of launching a fresh Codex process under the same `fsess_*` record. An unexpected host crash is process loss and must never be presented as successful revival. Set `FARMING_NATIVE_PTY_HOST_PERSIST=0` only when the host should die with every server shutdown. Avoid adding alternate terminal-runtime paths when improving product behavior.
