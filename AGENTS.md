...
The system-browser-backed Browser Resource module lives under `extensions/browser` and is enabled by default; Agent tool and MCP attachment remain on demand. When no compatible system browser is installed, show a Settings explanation that names Chrome, Brave, Edge, and Chromium but no toggle, and show no Project Browser section. When one is installed, label the setting `System browser`; do not call the browser itself built in or shared. Disabling must first stop every owned runtime; an unproved cleanup keeps the Extension enabled and fails visibly. Each Project may own multiple Browser identities; each identity has one isolated profile and one serialized lifecycle owner. The single supported MVP implementation launches a compatible system Chrome, Brave, Edge, or Chromium executable in headless mode and uses raw CDP for both Agent operations and the authenticated `Page.startScreencast` Viewer. Do not add Playwright, Puppeteer, a bundled Chromium, a second browser implementation, or silent attach/recovery fallback. A stale Viewer generation is rejected, and restart recovery marks previously live Browser rows failed rather than claiming an unproven process. Codex, Claude Code, OpenCode, and Qoder receive the Farming bootstrap from `backend/farming-agent-bootstrap.zh_cn.md` at startup and discover live capability state through `farming capabilities`; do not auto-mount Browser MCP. `farming browser` is the on-demand Agent bridge to the same Browser identity, `farming browser mcp` is explicit opt-in, and `farming-browser` is only the npm bin alias.
...
- `browserExtensionEnabled` (enabled by default; controls the system-browser integration and does not imply automatic Agent MCP attachment)
