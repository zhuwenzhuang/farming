...
The Browser Resource module lives under `extensions/browser` and is disabled by default; Agent tool and MCP attachment remain on demand. It has one raw-CDP operation and Viewer implementation with two explicit connection sources: launch a compatible Chromium-based system browser, or connect to a user- or Agent-managed external browser configured through `FARMING_BROWSER_CDP_URL`. The external endpoint must be loopback-only; use a user-managed tunnel for another host. Farming must not access a Docker socket, select or pull images, manage containers, expose the endpoint to clients, or close an externally owned browser process. Each external Resource creates and closes only its own page targets. The external owner controls browser, container, profile, and endpoint lifecycle; restart recovery marks interrupted rows failed without trying to kill that browser. System-browser Resources retain isolated profiles and exact process-group cleanup. Settings labels the available source `System browser` or `External browser`; when neither source is available it explains the requirement and shows no toggle or Project Browser section. Do not add Playwright, Puppeteer, a bundled Chromium, WebDriver, a second operation implementation, or silent connection fallback. A stale Viewer generation is rejected. Codex, Claude Code, OpenCode, and Qoder receive the Farming bootstrap from `backend/farming-agent-bootstrap.zh_cn.md` at startup and discover live capability state through `farming capabilities`; do not auto-mount Browser MCP. `farming browser` is the on-demand Agent bridge to the same Browser identity, `farming browser mcp` is explicit opt-in, and `farming-browser` is only the npm bin alias.
