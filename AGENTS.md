...
The Browser Resource module lives under `extensions/browser` and is disabled by default; Agent tool and MCP attachment remain on demand. Its one supported operations and Viewer runtime is the exact `agent-browser` version declared by Farming's startup-dependency manifest. Farming must not reimplement browser automation with raw CDP, add Playwright or Puppeteer to its production package, bundle Chromium, add WebDriver, or keep a silent second implementation. Before a fresh Server opens its port, the Farming launcher resolves the exact system `agent-browser` or prepares its verified immutable cache entry. For a local Resource, Farming passes an installed compatible Chromium executable and an isolated profile to `agent-browser`; Farming does not keep a separate Chromium launcher or process-gate implementation. For an external Resource, `agent-browser` connects to the user- or Agent-managed endpoint in `FARMING_BROWSER_CDP_URL`. The endpoint must be loopback-only; use a user-managed tunnel for another host. Farming must not access a Docker socket, select or pull images, manage containers, expose the endpoint to clients, or close an externally owned browser process. Each external Resource owns only its labeled tab and agent-browser Session. The external owner controls browser, container, profile, and endpoint lifecycle. A stale Viewer generation is rejected; the authenticated Viewer proxies the runtime's JPEG WebSocket stream and maps pointer, keyboard, wheel, and viewport input back to that same Session. Codex, Claude Code, OpenCode, and Qoder receive the Farming bootstrap from `backend/farming-agent-bootstrap.zh_cn.md` at startup and discover live capability state through `farming capabilities`; do not auto-mount Browser MCP. `farming browser` is the on-demand Agent bridge to the same Browser identity, `farming browser mcp` is explicit opt-in, and `farming-browser` is only the npm bin alias.
...
Release packages carry the exact reviewed Codex and Claude ACP JavaScript runtimes, but must not carry the transitive platform Agent CLIs from `@openai/codex-*` or `@anthropic-ai/claude-agent-sdk-*`. Codex CLI, Claude Code CLI, and `agent-browser` are startup dependencies rather than package dependencies. One checked-in manifest pins each supported platform artifact, source URL, integrity, executable entry, and expected version. A fresh launcher reuses a system executable only when it reports the exact required version; otherwise it downloads, verifies, stages, and atomically activates the immutable cache entry before the Server opens its port, then supplies `CODEX_PATH`, `CLAUDE_CODE_EXECUTABLE`, and `FARMING_AGENT_BROWSER_BIN` to runtime consumers. Concurrent starts share one preparation lock, corrupt cache entries are replaced without becoming active, and a failed preparation must leave the Server stopped with an actionable error. npm and bundle updates run the new release's dependency preparation while the old Server remains live and may become `ready-to-restart` only after it succeeds; failure leaves the old package and Server untouched. Package smoke must prove that both vendored adapters complete ACP `initialize` with platform packages absent from the release.
