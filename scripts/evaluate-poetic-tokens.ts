...
} = require('../backend/haiku-token.cjs') as {
  generateChineseHaikuToken: () => string;
  generateIndianHaikuToken: () => string;
  generateJapaneseHaikuToken: () => string;
  getPoeticTokenEntropyBits: (options: { locale: string }) => number;
};
...
interface SourceReport {
  ...
  uniqueRatio: number;
  invalidCount: number;
  badLineLengthCount: number;
  repeatedStrongImageTokenRatio: number;
  repeatedStrongImageAverage: number;
  markerCoverage: number;
  markerContaminationCount: number;
  imageContaminationCount: number;
  samples: string[];
}
...
function parseArgs(argv: string[]): EvaluateOptions {
  const options: EvaluateOptions = { count: 1000, samples: 10, check: false };
...
function lineLengths(token: string): number[] {
  return token.split('-').map(part => Array.from(part).length);
}

function countRepeatedStrongImageChars(token: string): number {
...
function markerMatched(sourceKey: string, token: string): boolean {
...
function markerContaminated(sourceKey: string, token: string): boolean {
  if (sourceKey === 'india') return false;
  const firstLine = token.split('-')[0] || '';
  return NON_NATIVE_MARKER_RE.test(firstLine);
}

function imageContaminated(sourceKey: string, token: string): boolean {
  if (sourceKey === 'india') return false;
  return NON_NATIVE_IMAGE_RE.test(token);
}

function scoreSource(source: TokenSource, options: EvaluateOptions): SourceReport {
  const tokens: string[] = [];
  const invalidTokens: string[] = [];
  const badLineLengths: string[] = [];
...
function assertReport(report: SourceReport): string[] {
  const errors: string[] = [];
