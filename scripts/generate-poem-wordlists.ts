...
function hasAnyChar(word: string, chars: Set<string>): boolean {
  return Array.from(word).some(char => chars.has(char));
}

function hasAllCharClasses(word: string, leftClass: Set<string>, rightClass: Set<string>): boolean {
  const chars = Array.from(word);
  return leftClass.has(chars[0]) && rightClass.has(chars[1]);
}

function qualityScore(word: string, count: number): number {
  let score = 0;
...
function extractJsonTextValues(node: unknown): string[] {
  const values: string[] = [];
  if (typeof node === 'string') {
    ...
    ['paragraphs', 'content'].forEach((key) => {
      ...
      if (obj[key]) values.push(...extractJsonTextValues(obj[key]));
    });
  }
  return values;
}
...
function countText(text: string, counts: Map<string, number>): number {
...
function countJsonSource(files: string[]): { counts: Map<string, number>; textPieceCount: number } {
...
function countTextSource(file: string): { counts: Map<string, number>; textPieceCount: number } {
  const counts = new Map<string, number>();
...
function sourceCommit(dir: string): string {
  if (!fs.existsSync(path.join(dir, '.git'))) return '';
  return childProcess.execFileSync('git', ['-C', dir, 'rev-parse', 'HEAD'], { encoding: 'utf8' }).trim();
}

function tierForIndex(index: number, total: number): string {
...
function writePayload(payload: Record<string, unknown>): void {
  ...
  const wordCount = (payload.words as Array<unknown>).length;
  console.error(`Wrote ${wordCount} ${payload.key} words to ${outputFile}`);
}
