...
function usage(): void {
  console.log([
    'Usage: tsx scripts/repro-mobile-ime-layout.ts [farming-url]',
    '',
...
interface ParseTarget {
  url: URL;
  basePath: string;
  api(pathnameSuffix: string): string;
}

function parseTarget(rawUrl: string): ParseTarget {
...
async function installMobileViewportMock(page: Page, profile: MobileProfile): Promise<void> {
  await page.addInitScript(({ width, height }) => {
    ...
    window.__farmingImeReproSetVisualViewport = (next: Partial<typeof state>, eventNames = ['resize', 'scroll']) => {
...
async function createControlAgent(page: Page, target: ParseTarget, workspace: string): Promise<string> {
...
async function deleteControlAgent(page: Page, target: ParseTarget, agentId: string): Promise<void> {
  if (!agentId) return;
  await page.request.delete(target.api(`/api/control/agents/${encodeURIComponent(agentId)}`)).catch(() => null);
}

async function revealAgent(page: Page, agentId: string): Promise<void> {
...
async function writeTerminalFixture(page: Page, agentId: string): Promise<void> {
  const lines = [
...
async function sampleLayout(page: Page, agentId: string, label: string) {
...
interface ProfileRun {
  ...
  workspace: string;
  agentId: string;
  samples: Array<Awaited<ReturnType<typeof sampleLayout>>>;
  screenshots: string[];
  reproduced: boolean;
}

async function runProfile(browser: Awaited<ReturnType<typeof chromium.launch>>, target: ParseTarget, outputDir: string, profile: typeof PROFILES[number]): Promise<ProfileRun> {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    (report as Record<string, unknown>).screenshotFiles = runs.flatMap(run => run.screenshots);
