...
const packagedCodexEntry = path.join(projectRoot, 'dist', 'acp', 'codex-acp-1.12.0.mjs');
const packagedClaudeEntry = path.join(projectRoot, 'dist', 'acp', 'claude-agent-acp-0.79.0.mjs');
...
const packagedAcpPlugin: esbuild.Plugin = {
  ...
  setup(build) {
    ...
    build.onLoad({ filter: /claude-agent-acp-0\.79\.0\.mjs$/ }, async (args) => {
