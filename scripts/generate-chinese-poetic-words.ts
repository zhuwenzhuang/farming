...
function extractTextValues(node: unknown): string[] {
  ...
  if (typeof node === 'string') {
    ...
    ['paragraphs', 'content'].forEach((key) => {
      ...
      if (obj[key]) values.push(...extractTextValues(obj[key]));
    });
  }
  return values;
}
...
function countJsonWords(files: string[]): { counts: Map<string, number>; textPieceCount: number } {
...
function countPlainTextWords(files: string[], counts: Map<string, number>): number {
  let textPieceCount = 0;
  files.forEach((file) => {
    try {
