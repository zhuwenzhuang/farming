...
function requireCrtAgentStateBridge() {
    const bridge = typeof window === 'undefined' ? null : window.FarmingAgentState;
    if (!bridge)
        throw new Error('Farming Agent state bridge is unavailable');
    return bridge;
}
function isCrtAgentStateMessageCandidate(value) {
    return value !== null
        && typeof value === 'object'
        && ('type' in value)
...
const TERMINAL_ATTACHMENT_COORDINATOR = (typeof window !== 'undefined' ? window.FarmingTerminalAttachmentCoordinator : null);
...
function findDirectionalNavigationIndex(rects, currentIndex, key, wrap = false) {
    ...
    if (bestIndex >= 0 || !wrap || rects.length < 2)
        return bestIndex;
...
function calculateCrtAgentPageLayout(availableWidth, availableHeight, itemCount) {
    ...
    const maxColumns = Math.max(1, Math.floor((innerWidth + CRT_AGENT_GRID_GAP) / (CRT_AGENT_CARD_MIN_WIDTH + CRT_AGENT_GRID_GAP)));
    const maxRows = Math.max(1, Math.floor((innerHeight + CRT_AGENT_GRID_GAP) / (CRT_AGENT_CARD_MIN_HEIGHT + CRT_AGENT_GRID_GAP)));
...
function getCrtAgentVerticalPageTarget({ itemIndex, totalItems, pageSize, columns, key }) {
    ...
    if (index < 0 || index >= total || (key !== 'ArrowUp' && key !== 'ArrowDown'))
        return -1;
    ...
    if (key === 'ArrowDown') {
        ...
        if (row !== lastRow || nextStart >= total)
            return -1;
...
function findDefaultNewAgentIndex(agentOptions, preferredAgentName) {
    if (!Array.isArray(agentOptions) || agentOptions.length === 0)
        return -1;
...
function buildCrtHistoryItems({ taskHistory = [], agents: agentRecords = [], sessions = [], mainPageSessionKeys = [] } = {}) {
    ...
    const candidates = historyCandidates.map((item) => ({
        ...item,
    ...
    candidates.forEach((item) => {
        ...
        if (!current || shouldReplaceCrtHistoryItem(current, item))
            resumable.set(resumeKey, item);
    });
...
function buildCrtSearchResults({ query = '', agents: agentRecords = [], sessions = [], mainAgentId = '', projectNames = {} } = {}) {
    ...
    if (!normalizedQuery)
        return [];
    const liveAgents = agentRecords.filter((agent) => (agent
...
function getCrtNavigationScope() {
    ...
    if (billingArea && !billingArea.classList.contains('hidden'))
        return billingArea;
...
function setCrtNavigationSelection(element) {
    ...
    const scrollTarget = element.closest('#settings-modal .settings-panel') || element;
...
function moveCrtNavigationSelection(key) {
    ...
    if (!items.length)
        return false;
    const currentIndex = items.findIndex((element) => (element.classList.contains('crt-nav-selected') || element.dataset.crtNavKey === crtNavigationKey));
    if (currentIndex < 0) {
    ...
    const wrap = Boolean(scope && (scope.id === 'input-dialog'
...
function isCurrentCrtSession(agentId, sessionToken = null) {
    if (!agentId || focusedAgentId !== agentId)
        return false;
...
function showCrtWebglFailure(error) {
    showCrtTerminalFailure('CRT WEBGL ERROR', error, 'The Agent is still running. Close and reopen this terminal after WebGL is available.');
}
function showCrtProtocolMismatchNotice(titleText, messageText) {
...
function crtStructuredPreviewStatus(agent, cached) {
    if (cached && cached.error && !cached.preview)
        return 'PREVIEW OFFLINE';
    if ((!cached || !cached.preview) && (!cached || cached.loading))
        return 'SYNCING';
...
function renderCrtDashboardIfNeeded(force = false) {
    ...
    if (!force && signature === lastCrtDashboardSignature)
        return false;
...
function updateCrtAgentPreviewCard(agent) {
    if (typeof document === 'undefined' || !isCrtLiveAgent(agent))
        return false;
    ...
    if (isMain) {
        ...
        if (selected)
            block.classList.add('crt-nav-selected');
...
function getCrtProjectName(agent, projectNames = globalSettings && globalSettings.projectNames) {
    ...
    if (projectNames && typeof projectNames === 'object') {
        const configuredEntry = Object.entries(projectNames).find(([candidate]) => (normalizeWorkspaceValue(candidate).replace(/[/\\]+$/, '') || candidate) === workspaceKey);
...
function isCrtAgentWorking(agent) {
    ...
    if (activity === 'busy')
        return true;
    if (activity === 'idle' || activity === 'exited')
        return false;
    if (terminalStatus && terminalStatus.busy === true)
        return true;
...
function getCrtBrandPaneKey(agentId, currentState) {
    if (!agentId || !currentState || !Array.isArray(currentState.agents))
        return null;
    const agent = currentState.agents.find((candidate) => candidate.id === agentId);
    if (!agent)
        return null;
    if (agent.id === currentState.mainAgentId || agent.isMain)
        return 'main';
    const workers = currentState.agents.filter((candidate) => candidate.id !== currentState.mainAgentId && !candidate.isMain);
...
function updateCrtBrandState(currentState) {
    ...
    if (!brand || !currentState || !Array.isArray(currentState.agents))
        return;
    const paneLive = {
        main: false,
        'worker-a': false,
    ...
    currentState.agents.forEach((agent) => {
        ...
        if (paneKey && isCrtBrandAgentLive(agent))
            paneLive[paneKey] = true;
    });
    Object.entries(paneLive).forEach(([paneKey, live]) => {
        ...
        if (pane)
            pane.classList.toggle('is-live', live);
    });
}
...
function crtLogicalTerminalLineAtRow(currentTerminal, row) {
    ...
    if (!buffer || typeof buffer.getLine !== 'function' || !Number.isFinite(row) || row < 0)
        return null;
    let startRow = row;
    while (startRow > 0 && buffer.getLine(startRow)?.isWrapped)
        startRow -= 1;
    let endRow = row;
    while (buffer.getLine(endRow + 1)?.isWrapped)
        endRow += 1;
    const lines = [];
...
function deriveSessionTextPatch(fullText, previousLength = 0, forceReplace = false) {
...
function settleInitialCrtTerminalFocus(replication, terminalInstance = terminal) {
    if (!replication || replication.disposed || !replication.initialFocusPending)
        return false;
    replication.initialFocusPending = false;
    const modal = document.getElementById('session-modal');
    if (!modal || !modal.classList.contains('active') || !terminalInstance)
        return false;
    const activeElement = document.activeElement;
    if (activeElement?.matches?.('#terminal-output .xterm-helper-textarea'))
        return true;
    if (activeElement
...
function installCrtTerminalTestApi() {
    ...
    window.__farmingCrtTerminalTest = {
        ...
        replaceStream(stream) {
            const replication = crtTerminalReplication;
            if (!replication || replication.disposed || !stream)
                return false;
            handleCrtTerminalStream({
                ...stream,
                agentId: replication.agentId,
...
function disposeCrtTerminalReplication() {
    ...
    if (replication.fitResizeTimer)
        clearTimeout(replication.fitResizeTimer);
...
function finishCrtTerminalReplay(replication = crtTerminalReplication) {
    ...
    if (replication.attachment.queuedTransitionCount > 0 && !replication.attachment.recovering) {
        ...
        if (replication.attachment.queuedTransitionCount > 0 ||
            replication.writeInProgress ||
            replication.checkpointInFlight ||
...
function stopCrtTerminalReplay(replication, error) {
    ...
    showCrtTerminalFailure('CRT TERMINAL SYNC ERROR', error, 'The Agent is still running. Close and reopen this terminal to retry recovery.');
}
function applyCrtTerminalTransition(event) {
    const replication = crtTerminalReplication;
    if (!replication || !terminal || replication.disposed)
        return;
    ...
    terminalInstance.write(transitionData, () => {
        ...
        refreshSessionTerminalUi({ preserveSearchIndex: true });
        if (drainCrtTerminalCheckpointInstall(replication))
            return;
        flushCrtTerminalTransitions();
...
function flushCrtTerminalTransitions() {
    ...
    if (!replication ||
        ...
        replication.writeInProgress)
        return;
    while (!replication.attachment.recovering &&
        !replication.checkpointInFlight &&
        !replication.installInProgress &&
        !replication.writeInProgress) {
...
function performCrtTerminalCheckpointInstall(replication, pending) {
...
function installCrtTerminalCheckpoint(sessionView, operation = crtTerminalReplication?.attachment.beginCheckpointOperation() || null) {
    const replication = crtTerminalReplication;
    if (!replication || !terminal || replication.disposed || !operation)
        return false;
    const checkpoint = {
        runtimeEpoch: sessionView.runtimeEpoch,
        outputSeq: sessionView.outputSeq,
    ...
    if (!replication.attachment.isCurrentOperation(operation))
        return false;
    const decision = replication.attachment.evaluateCheckpoint(checkpoint);
    if (decision.action === 'reject') {
        retryCrtTerminalReplayAfterFailure(replication, replication.attachment.recordInvariantFailure(decision.signature || 'invalid-checkpoint', decision.message || 'Terminal replay returned an invalid screen state'));
        return false;
    }
    const validatedCheckpoint = checkpoint;
...
function handleCrtTerminalStream(stream) {
    const replication = crtTerminalReplication;
    if (!replication || !stream || stream.agentId !== replication.agentId)
        return;
...
function deriveSessionStreamPatch(stream, currentFocusedAgentId, currentSessionSource) {
    if (!stream)
        return null;
    if (stream.agentId !== currentFocusedAgentId)
        return null;
    if (!currentSessionSource)
        return null;
    if (typeof stream.data !== 'string' || stream.data.length === 0)
        return null;
    return {
        text: stream.data,
    ...
    const save = globalSettingsSaveTail.catch(() => { }).then(async () => {
        try {
            ...
            if (data.success) {
                ...
                keys.forEach((key) => {
                    if (globalSettingsSaveRevisions.get(key) !== revisions.get(key))
                        return;
...
function selectDefaultNewAgentNavigation() {
    ...
    if (defaultAgentIndex < 0)
        return false;
    const defaultItem = getCrtNavigationItems().find((item) => (item.dataset.crtNavKey === `new-agent:provider:${defaultAgentIndex}`));
...
function crtHistoryItemTitle(item) {
    ...
    if (item.kind === 'agent')
        return getCrtAgentTitle(item.agent);
...
function setCrtMainView(view) {
    ...
    if (previousView === 'billing' && crtMainView !== 'billing')
        CrtBilling.leave();
    if (mapArea)
        mapArea.classList.toggle('hidden', crtMainView !== 'agents');
    if (historyArea)
        historyArea.classList.toggle('hidden', crtMainView !== 'history');
    if (searchArea)
        searchArea.classList.toggle('hidden', crtMainView !== 'search');
    if (billingArea)
        billingArea.classList.toggle('hidden', crtMainView !== 'billing');
    if (historySidebarItem)
        historySidebarItem.classList.toggle('active', crtMainView === 'history');
    if (searchSidebarItem)
        searchSidebarItem.classList.toggle('active', crtMainView === 'search');
    if (billingSidebarItem)
        billingSidebarItem.classList.toggle('active', crtMainView === 'billing');
    if (previousView !== 'agents' && crtMainView === 'agents')
...
function crtSearchResultTitle(result) {
    if (result.kind === 'agent')
        return getCrtAgentTitle(result.agent);
...
async function loadCrtSearchAgentSessions(query, requestSequence) {
    ...
    try {
        ...
        if (!response.ok)
            throw new Error(data && data.error ? data.error : `Search request failed (${response.status})`);
        ...
        if (controller.signal.aborted || requestSequence !== searchRequestSequence)
            return;
        searchAgentSessions = [];
...
function activateCrtSearchResult(result = getCrtSearchResults()[searchSelectionIndex]) {
    if (!result || searchActionPendingKey)
        return false;
    if (result.kind === 'agent') {
...
function showCrtSearch() {
    ...
    window.requestAnimationFrame(() => {
        const input = document.getElementById('crt-search-input');
        if (input)
            setCrtNavigationSelection(input);
    });
}
...
function calculateCrtHistoryPageSize(availableHeight, rowHeight = 68) {
    ...
    if (!Number.isFinite(height) || !Number.isFinite(itemHeight) || height <= 0 || itemHeight <= 0)
        return 1;
...
function updateCrtHistoryPagination(pageState) {
    ...
    if (status)
        status.textContent = `${pageState.page + 1}/${pageState.totalPages} · ${pageState.totalItems}`;
    if (previous)
        previous.disabled = pageState.page <= 0;
    if (next)
        next.disabled = pageState.page >= pageState.totalPages - 1;
}
function renderCrtHistory() {
    ...
    const selectedIndex = historyItems.findIndex((item) => (crtNavigationKey === `history:${item.historyKey}`
        || crtNavigationKey === `history:${item.historyKey}:restore`));
    if (selectedIndex >= 0)
        historyPage = Math.floor(selectedIndex / historyPageSize);
    ...
    if (historyError)
        list.appendChild(createCrtHistoryMessage('history-error', historyError));
    ...
    pageState.items.forEach((item, index) => {
        ...
        if (index === 0)
            row.dataset.crtNavDefault = 'true';
        row.tabIndex = -1;
...
async function loadCrtHistoryAgentSessions() {
    ...
    try {
        ...
        const data = await response.json().catch(() => null);
        if (!response.ok)
            throw new Error(data && data.error ? data.error : `History request failed (${response.status})`);
...
async function resumeCrtHistorySession(resumed, customTitle, historyKey) {
    if (!resumed || historyActionPendingKey)
        return;
...
async function resumeCrtSearchSession(session, searchKey) {
    ...
    if (!resumed || searchActionPendingKey)
        return;
...
async function restoreCrtArchivedAgent(agentId, openAfterRestore, historyKey) {
    if (!agentId || historyActionPendingKey)
        return;
    historyActionPendingKey = historyKey;
    historyError = '';
    renderCrtHistory();
    try {
        ...
        const data = await response.json().catch(() => null);
        if (!response.ok)
            throw new Error(data && data.error ? data.error : `Failed to restore Agent (${response.status})`);
        crtNavigationKey = `agent:${agentId}`;
        if (openAfterRestore)
            pendingProviderSessionOpenAgentId = String(agentId);
        historyActionPendingKey = '';
        hideHistory();
        openPendingProviderSessionAgentIfReady();
    }
...
function activateCrtHistoryItem(item) {
    if (!item || historyActionPendingKey)
        return;
    if (item.kind === 'run') {
...
function openPendingProviderSessionAgentIfReady() {
    if (!pendingProviderSessionOpenAgentId || !state)
        return false;
...
async function confirmStartAgent() {
    if (waitingForAgent || workspaceDirectoryPrompt || selectedAgentIndex === null || selectedAgentIndex < 0 || selectedAgentIndex >= agents.length)
        return;
...
function showWorkspaceDirectoryPrompt(kind, { workspace, code = '' }) {
    ...
    if (!prompt || !input || !createButton || !cancelButton || !errorBackButton)
        return;
...
async function createCrtWorkspaceAndStart() {
    if (!workspaceDirectoryPrompt || workspaceDirectoryPrompt.kind !== 'confirm')
        return;
...
function openCrtAgentDeeplinkIfReady() {
    if (didApplyAgentDeeplink || !state)
        return false;
...
function applyCrtAgentStateDelta(data) {
    ...
    if (data.upserts.length === 0 && data.removedAgentIds.length === 0 && !data.state)
        return state;
    const nextAgents = requireCrtAgentStateBridge().applyAgentStateDelta(state.agents, data.upserts, data.removedAgentIds);
    return {
        ...state,
        ...(data.state || {}),
        agents: nextAgents,
...
function applyCrtAgentStateSnapshotPage(data) {
    ...
    const snapshotTransition = requireCrtAgentStateBridge().advanceAgentStateSnapshot(agentStateSnapshotCursor, data.generation, data.sequence, page, data.state.agents.length);
...
function connect() {
    ...
    socket.onmessage = (event) => {
        ...
        if (getSessionClient()?.handleServerMessage(data))
            return;
        if (data.type === 'protocol-hello') {
            if (!agentStateBridge.protocolCompatible(data.protocolVersion)) {
                if (data.protocolVersion > CRT_PROTOCOL_VERSION) {
                    ...
                    showCrtProtocolMismatchNotice('FARMING BACKEND OUT OF DATE', 'Update and restart the Farming backend, then reload this page.');
                }
        ...
        if (data.type === 'state') {
            ...
            const disposition = agentStateBridge.agentStateDeltaDisposition(agentStateGeneration
                ? { generation: agentStateGeneration, sequence: agentStateSequence }
                : null, data.generation, data.sequence);
            ...
            if (preview && preview.agentId) {
                ...
                if (agent) {
                    ...
                    if (preview.terminalStatus)
                        agent.terminalStatus = preview.terminalStatus;
                    if (preview.runtimeObservation)
                        agent.runtimeObservation = preview.runtimeObservation;
                    if (isCrtSessionOpen())
                        dashboardRenderDeferred = true;
                    else
                        scheduleCrtPreviewCardRender(agent, previewChanged);
                }
            }
        }
            ...
            if (agent) {
                ...
                if (isCrtSessionOpen())
                    ...
                    scheduleCrtPreviewCardRender(agent);
            }
        }
...
function getCrtMainAgentDialogAction({ currentState, mainView = 'agents', dialogActive = false, pendingMainAgent = false, } = {}) {
...
function updateCrtAgentInventoryStatus(currentState) {
...
function updateCrtAgentPageStatus(pageState) {
    ...
    if (item)
        item.hidden = pageState.totalPages <= 1;
    if (status)
        status.textContent = `${pageState.page + 1}/${pageState.totalPages}`;
}
...
function moveCrtAgentPageSelection(key) {
    if (crtMainView !== 'agents' || (key !== 'ArrowUp' && key !== 'ArrowDown'))
        return false;
...
function renderState() {
    ...
    const pageLayout = calculateCrtAgentPageLayout(mapArea.clientWidth, mapArea.clientHeight, regularAgents.length);
    crtAgentPageSize = pageLayout.pageSize;
    crtAgentPageColumns = pageLayout.columns;
    const selectedRegularAgentIndex = regularAgents.findIndex((agent) => (crtNavigationKey === `agent:${agent.id}`));
    ...
    agentPage.items.forEach((agent, pageItemIndex) => {
        ...
        if (keyIndex === 1)
            block.dataset.crtNavDefault = 'true';
        block.tabIndex = -1;
...
function hasCrtStructuredLocalEscapeAction(context = {}) {
    return Boolean(context.structuredTranscriptFocused
...
function resolveCrtSessionKeyboardCommand(event, context = {}) {
    ...
    if (context.structuredSessionActive === true
        && key === 'escape'
        && !event.altKey
        && !event.shiftKey
...
function updateCrtRuntimeSwitchControl(agent) {
    ...
    if (!control || !chatButton || !terminalButton)
        return;
...
async function switchCrtSessionRuntimeMode(mode) {
    if (runtimeSwitchPending || !focusedAgentId || !state)
        return;
    ...
    if (crtRuntimeView(agent) === (targetMode === 'terminal' ? 'terminal' : 'chat'))
        return;
    ...
    try {
        ...
        const data = await response.json().catch(() => null);
        if (requestSequence !== runtimeSwitchRequestSequence)
            return;
        if (!response.ok || !data) {
...
function toggleCrtSessionRuntimeMode() {
    if (!focusedAgentId || !state)
        return;
    const agent = state.agents.find((candidate) => candidate.id === focusedAgentId);
    if (!canSwitchCrtAgentRuntime(agent)) {
        const reason = crtRuntimeSwitchUnavailableReason(agent);
        if (reason)
            setCrtRuntimeSwitchStatus(reason, true);
        return;
    }
...
function structuredComposerAction(agent, draft = '') {
    if (!isCrtAgentInteractive(agent) || structuredRuntimeError(agent))
        return 'disabled';
    if (structuredComposerAttachments.some(item => item.status === 'uploading'))
        return 'disabled';
    const status = String(structuredRuntimeStatus(agent) || 'idle');
    const working = ['working', 'waiting-for-permission'].includes(status);
    if (working) {
        if (agent.runtimeBinding?.kind === 'acp' && String(draft || '').trim())
            return 'send';
        return 'interrupt';
    }
    if (['starting', 'interrupting'].includes(status))
        return 'disabled';
...
function flushStructuredComposerFollowUp(agent) {
    if (!agent || structuredRuntimeStatus(agent) !== 'idle')
        return;
    ...
    if (!next)
        return;
    if (queue.length === 0)
        structuredComposerPendingFollowUps.delete(agent.id);
...
function structuredSelectOptions(option) {
    if (!option || !Array.isArray(option.options))
        return [];
    return option.options.flatMap((candidate) => (candidate && Array.isArray(candidate.options) ? candidate.options : [candidate])).filter(Boolean);
}
function structuredVisibleConfigOptions(session) {
    ...
    return options.filter((option) => (String(option && option.id || '').toLowerCase() !== 'mode'
...
function setStructuredComposerMenu(menu, { focusFirst = false, opener = null } = {}) {
    if (opener && opener.id)
        structuredComposerMenuOpenerId = opener.id;
    structuredComposerMenu = structuredComposerMenu === menu ? '' : menu;
    if (structuredComposerMenu !== 'config')
        structuredComposerConfigId = '';
...
function structuredComposerToolbarButtons() {
    ...
    ].filter((button) => Boolean(button && !button.hidden && !button.disabled));
}
...
function restoreStructuredComposerMenuFocus(snapshot) {
    if (!snapshot || snapshot.path !== structuredComposerMenuPath())
        return false;
    const buttons = structuredComposerMenuButtons();
    if (!buttons.length)
        return false;
...
async function patchStructuredAcpSession(agentId, generation, patch) {
    ...
    if (!isCurrentStructuredSession(agentId, generation))
        return;
    if (!response.ok)
        throw new Error(body && body.error ? body.error : `Failed to update ACP session (${response.status})`);
    structuredComposerMenu = '';
    structuredComposerConfigId = '';
...
function renderStructuredComposerMenu() {
    ...
    if (structuredComposerMenu === 'commands') {
        ...
        structuredVisibleConfigOptions(session).forEach((option) => {
            items.appendChild(structuredMenuButton(`${option.name}: ${structuredConfigValueLabel(option)}`, option.description || '', false, () => {
...
function renderStructuredSessionControls() {
    ...
    const configButton = document.getElementById('crt-structured-config');
    const usage = document.getElementById('crt-structured-composer-usage');
    if (!commandButton || !modeButton || !configButton || !usage)
        return;
...
async function refreshStructuredSessionControls(agentId = focusedAgentId, force = false, generation = structuredSessionGeneration) {
    if (!agentId)
        return;
    if (!isCurrentStructuredSession(agentId, generation) || structuredSessionControlsLoading)
        return;
    ...
    if (!force && ((revision && revision === structuredSessionControlsRevision)
...
function updateStructuredTranscriptScrollState() {
    const container = document.getElementById('terminal-output');
    const status = document.getElementById('crt-structured-composer-status');
    if (!container || !status || !container.classList.contains('crt-structured-session'))
        return false;
...
function focusStructuredTranscript() {
    const container = document.getElementById('terminal-output');
    if (!container || !updateStructuredTranscriptScrollState())
        return false;
...
function updateStructuredComposerState(agent, { refreshControls = true } = {}) {
    ...
    const statusNode = document.getElementById('crt-structured-composer-status');
    if (!input || !sendButton || !statusNode)
        return;
...
async function prepareStructuredAttachment(file) {
    ...
    try {
        if (String(file.type || '').startsWith('image/') || String(file.type || '').startsWith('audio/')) {
            ...
            const body = await response.json().catch(() => null);
            if (!response.ok || !body || !body.path)
                throw new Error(`${image ? 'Image' : 'Audio'} upload failed (${response.status})`);
...
function structuredComposerPromptAttachments() {
    return structuredComposerAttachments.flatMap((attachment) => (attachment.status === 'ready' && attachment.path
        ? [{
...
function navigateStructuredComposerHistory(input, direction) {
    ...
    if (direction < 0) {
        ...
        structuredComposerHistoryIndex += 1;
        if (structuredComposerHistoryIndex >= history.length)
            structuredComposerHistoryIndex = -1;
    }
    else {
        return false;
...
function renderStructuredPermissions(agent) {
    ...
    requests.forEach((request) => {
        ...
        (request.options || []).forEach((option) => {
            ...
            button.onclick = () => void respondToStructuredPermission(agentId, generation, request.requestId, option.optionId, false);
            actions.appendChild(button);
        });
        const decline = document.createElement('button');
        decline.type = 'button';
        decline.textContent = '[DECLINE]';
        decline.onclick = () => void respondToStructuredPermission(agentId, generation, request.requestId, undefined, true);
...
function structuredTranscriptAttachmentLabel(content) {
    ...
    const labels = blocks.flatMap((block) => {
        ...
        if (type.includes('file') || type.includes('resource'))
...
function crtStructuredPreviewMessageLines(transcript, limit = 8) {
    ...
    if (turnLines.length > 0)
        return turnLines.slice(-maxLines);
...
function buildCrtStructuredPreview(transcript, agent = null) {
    ...
    if (entries.length > 0) {
        ...
        const latestAssistant = [...turnEntries].reverse().find((entry) => (entry.type === 'message' && entry.role === 'assistant'));
        if (latestAssistant) {
        ...
        activityText = normalizeCrtStructuredPreviewText(processItem && (processItem.detail || processItem.title), 180);
    }
    return {
        messageLines,
        userText,
...
function crtStructuredPreviewRevision(agent) {
    if (!agent || !isStructuredRuntimeAgent(agent))
        return '';
    const runtime = agent.runtimeBinding;
    return JSON.stringify([
...
function pruneCrtStructuredPreviews(currentState = state) {
    ...
    crtStructuredPreviewCache.forEach((_value, agentId) => {
        if (!visibleIds.has(agentId))
            crtStructuredPreviewCache.delete(agentId);
    });
...
function renderStructuredTranscript(transcript, force = false, agentId = focusedAgentId) {
    const container = document.getElementById('terminal-output');
    if (!container || agentId !== focusedAgentId)
        return;
    const updatedAt = String(transcript && transcript.updatedAt || '');
    if (!force && updatedAt && updatedAt === structuredSessionRenderedAt)
        return;
    const nearBottom = container.scrollHeight - container.scrollTop - container.clientHeight < 80;
    if (!nearBottom)
        saveCrtStructuredReadingAnchor(agentId);
    const turns = structuredTranscriptTurns(transcript);
    ...
    try {
        ...
        if (typeof markdownRenderer.unmount === 'function')
            markdownRenderer.unmount(container);
        const errorNode = document.createElement('div');
        errorNode.className = 'crt-structured-error';
...
async function refreshStructuredSession(agentId = focusedAgentId, force = false, generation = structuredSessionGeneration) {
    if (!isCurrentStructuredSession(agentId, generation) || structuredSessionLoading)
        return;
    const agent = state && state.agents.find((candidate) => candidate.id === agentId);
    if (!agent || !isStructuredRuntimeAgent(agent))
        return;
...
function setupStructuredSessionComposer() {
    ...
    const configButton = document.getElementById('crt-structured-config');
    const menu = document.getElementById('crt-structured-composer-menu');
    if (!composer || !input || composer.dataset.bound === 'true')
        return;
    ...
    if (commandButton)
        commandButton.addEventListener('click', (event) => setStructuredComposerMenu('commands', { opener: event.currentTarget }));
    if (modeButton)
        modeButton.addEventListener('click', (event) => setStructuredComposerMenu('mode', { opener: event.currentTarget }));
    if (configButton)
        configButton.addEventListener('click', (event) => setStructuredComposerMenu('config', { opener: event.currentTarget }));
    composer.addEventListener('keydown', (event) => {
        const eventTarget = event.target instanceof Element ? event.target : null;
        const tool = eventTarget
            ? eventTarget.closest('.crt-structured-tool')
            : null;
        if (!tool)
            return;
        ...
        if (event.key === 'ArrowUp' || event.key === 'Escape') {
            event.preventDefault();
            if (structuredComposerMenu)
                closeStructuredComposerMenu({ restoreFocus: false });
            input.focus();
            return;
        }
    ...
    if (menu)
        menu.addEventListener('keydown', (event) => {
            ...
            const item = eventTarget
                ? eventTarget.closest('.crt-structured-menu-item')
                : null;
            if (!item)
                return;
            if (event.key === 'ArrowDown' || event.key === 'ArrowUp') {
    ...
    if (transcript) {
        ...
        transcript.addEventListener('keydown', (event) => {
            if (!transcript.classList.contains('crt-structured-session'))
                return;
    ...
    composer.addEventListener('submit', (event) => {
        ...
        if (!message && promptAttachments.length === 0)
            return;
        const statusNode = document.getElementById('crt-structured-composer-status');
        ...
        const sent = getSessionClient()?.sendComposerMessage(agentId, message, promptAttachments, {
            ...
            onResult(result) {
                ...
                if (focusedAgentId !== agentId || structuredSessionGeneration !== generation)
                    return;
                if (statusNode) {
...
async function openStructuredSession(agent, sessionToken) {
    ...
    if (!isCurrentStructuredSession(agent.id, generation))
        return;
    await refreshStructuredSession(agent.id, true, generation);
    if (!isCurrentStructuredSession(agent.id, generation))
        return;
    if (runtime && !runtime.isCurrentSession(agent.id, sessionToken))
        return;
...
async function openSession(agentId) {
    ...
    if (terminal && typeof terminal.onTitleChange === 'function') {
        terminal.onTitleChange((title) => {
            if (runtime && !runtime.isCurrentSession(agentId, sessionToken))
                return;
            if (!runtime && focusedAgentId !== agentId)
                return;
    ...
    if (!mountedTerminal) {
        ...
        terminal.onResize(({ cols, rows }) => {
            ...
            if (runtime && !runtime.isCurrentSession(agentId, sessionToken))
                return;
            if (crtTerminalReplication?.applyingLocalResize)
                return;
...
function closeSession() {
    if (focusedAgentId) {
        if (terminal)
            saveCrtTerminalReadingAnchor(focusedAgentId, terminal);
        else
            saveCrtStructuredReadingAnchor(focusedAgentId);
    }
...
function commitCrtTerminalResize(agentId, normalizedDimensions, { allowDuringRecovery = false } = {}) {
    const replication = crtTerminalReplication;
    if (!agentId ||
        !terminal ||
        !replication ||
    ...
    const delivered = getSessionClient()?.resizeAgent(agentId, normalizedDimensions.cols, normalizedDimensions.rows);
    if (delivered) {
...
function sendSessionResize(agentId = focusedAgentId, requestedDimensions = null, { forceDuringRecovery = false } = {}) {
    const replication = crtTerminalReplication;
    if (!agentId ||
        !terminal ||
        !fitAddon ||
        ...
        return;
    const dimensions = requestedDimensions || (typeof fitAddon.proposeDimensions === 'function'
        ? fitAddon.proposeDimensions()
        : null);
    ...
    if (!requestedDimensions) {
        if (normalizedDimensions.cols === replication.lastResizeCols &&
            normalizedDimensions.rows === replication.lastResizeRows) {
            ...
            if (terminal.cols !== normalizedDimensions.cols ||
...
async function refreshSessionView(_forceReplace = false, expectedAgentId = focusedAgentId, expectedSessionToken = getCurrentSessionToken()) {
    ...
    const timeout = setTimeout(() => controller.abort(), CRT_TERMINAL_CHECKPOINT_REQUEST_TIMEOUT_MS);
    try {
        const sessionClient = getSessionClient();
        if (!sessionClient)
        ...
        if (crtTerminalReplication === replication &&
            replication.attachment.isCurrentOperation(operation)) {
            retryCrtTerminalReplayAfterFailure(replication, replication.attachment.recordTransportFailure(), error);
        }
    }
    finally {
        clearTimeout(timeout);
...
if (typeof document !== 'undefined') {
    window.addEventListener('resize', () => {
        ...
        if (crtMainView === 'agents' && state) {
            if (crtAgentPageResizeFrame !== null)
                window.cancelAnimationFrame(crtAgentPageResizeFrame);
    ...
    document.addEventListener('keydown', (event) => {
        ...
        if (!sessionActive || !isCrtRuntimeSwitchShortcut(event))
            return;
        const agent = state && state.agents.find((candidate) => candidate.id === focusedAgentId);
        if (!canSwitchCrtAgentRuntime(agent))
            return;
    ...
    document.addEventListener('keydown', (e) => {
        ...
        if (terminalFontSizeInputFocused && (e.key === 'ArrowLeft' || e.key === 'ArrowRight'))
            return;
        if (searchInputFocused)
            return;
        if (billingHourCellFocused && (e.key === 'ArrowLeft' || e.key === 'ArrowRight'))
            return;
        if (crtMainView === 'billing'
        ...
        if (sessionActive) {
            ...
            const structuredInterruptFocused = Boolean(structuredInputFocused
...
if (typeof module !== 'undefined' && module.exports) {
    ...
    document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'hidden')
            suspendCrtPageConnection();
        else
            resumeCrtPageConnection();
    });
    ...
    CrtBilling.init({
        ...
        setMainView: (view) => setCrtMainView(view),
        renderShellState: () => renderState(),
        setNavigationSelection: (el) => setCrtNavigationSelection(el),
        clearNavigationSelection: () => clearCrtNavigationSelection(),
        apiPath: (path) => farmingApiPath(path),
        formatTokenRate: (value) => formatCrtTokenRate(value),
    });
