...
async function loadCrtBillingDayDetail(dateValue, { force = false, live = false, retryCount = 0 } = {}) {
    ...
    try {
        ...
        if (controller.signal.aborted || requestSequence !== billingDayDetailRequestSequence)
            return;
...
function selectCrtBillingDayByArrow(key) {
    if (crtMainView !== 'billing' || billingMode !== 'days')
        return false;
    const points = billingSummary && billingSummary.daily && Array.isArray(billingSummary.daily.points)
        ? billingSummary.daily.points
        : [];
...
function setCrtBillingMode(mode) {
    ...
    if (billingMode === 'live')
        window.requestAnimationFrame(() => drawCrtBillingScope());
}
function selectCrtBillingMode(mode) {
    ...
    if (selectedTab)
        setCrtNavigationSelection(selectedTab);
    if (billingMode === 'live' && !billingLoading)
        void loadCrtBilling();
}
...
async function loadCrtBilling({ fresh = false } = {}) {
    ...
    try {
        ...
        if (controller.signal.aborted || requestSequence !== billingRequestSequence)
            return;
...
function startCrtBillingRefresh() {
    ...
    billingRefreshTimer = setInterval(() => {
        if (crtMainView === 'billing' && document.visibilityState !== 'hidden')
            void loadCrtBilling();
...
function refreshCrtBilling() {
    if (crtMainView !== 'billing' || billingLoading)
        return;
...
function showCrtBilling() {
    ...
    window.requestAnimationFrame(() => {
        ...
        const daysTab = document.getElementById('billing-days-tab');
        if (daysTab || refresh)
            setCrtNavigationSelection(daysTab || refresh);
    });
}
