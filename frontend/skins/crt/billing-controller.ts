...
type BillingDomElement = HTMLElement & HTMLInputElement & HTMLButtonElement & HTMLCanvasElement;
interface Document {
  getElementById(elementId: string): BillingDomElement;
  querySelector<E extends Element = BillingDomElement>(selectors: string): E | null;
  querySelectorAll<E extends Element = BillingDomElement>(selectors: string): NodeListOf<E>;
}
interface Element {
  querySelector<E extends Element = BillingDomElement>(selectors: string): E | null;
  querySelectorAll<E extends Element = BillingDomElement>(selectors: string): NodeListOf<E>;
...
// CrtBillingController — vertical owner of all CRT billing state, fetching,
// scheduling, animation, and rendering. Loaded as a classic script before
// app.js. Exposes the global `CrtBilling` lifecycle object; app.ts wires
// narrow ports at bootstrap and delegates lifecycle/navigation events.
...
type BillingDirectionKey = 'ArrowLeft' | 'ArrowRight' | 'ArrowUp' | 'ArrowDown';
...
let billingPorts: BillingPorts | null = null;
...
function crtBillingDayArrowTargetIndex(points: BillingDayPoint[], index: number, key: BillingDirectionKey) {
...
function isBillingTransientIncompleteHourlyDetail(error: unknown): boolean {
  return error instanceof BillingTransientIncompleteHourlyDetailError;
}

function isBillingRetryableFailure(error: unknown, status?: number): boolean {
  if (error instanceof Error && error.name === 'AbortError') return false;
  if (isBillingTransientIncompleteHourlyDetail(error)) return true;
...
// --- Generation-fenced deadline helper with per-operation ownership ---

type BillingDeadlineSlot = { timer: number | null; handle: number };
let billingDeadlineHandleSequence = 0;

// Each fetch+JSON operation owns an exact token. The deadline invalidates the
// token BEFORE abort/releaseOwner, so any late fetch settle or late
// response.json() completion is a no-op even though releasing the owner does
// not bump the owning request sequence. Each operation still applies its own
// identity check (request sequence or controller presence) on top of
// token.valid.
type BillingOperationToken = {
  valid: boolean;
  generation: number;
};

function armBillingDeadline(
  slot: BillingDeadlineSlot,
  controller: AbortController,
  ms: number,
  operation: BillingOperationToken,
  releaseOwner: () => void,
): number {
  ...
  slot.timer = setTimeout(() => {
    slot.timer = null;
    if (slot.handle !== handle || operation.generation !== billingGeneration) return;
...
function clearBillingDeadline(slot: BillingDeadlineSlot, handle: number) {
...
const billingTrackedFrames = new Set<number>();

function trackBillingFrame(callback: (now: number) => void): number {
...
function drawCrtBillingScope(summary = billingSummary) {
  ...
  if (!canvas || !billingPorts || !billingPorts.isActiveView()) return;
...
function pruneBillingDayDetailCache() {
  ...
  const authoritativeDates = new Set(
    Array.isArray(daily.points) ? daily.points.map(point => point.date) : [],
  );
  for (const cachedDate of billingDayDetailCache.keys()) {
    if (!authoritativeDates.has(cachedDate)) billingDayDetailCache.delete(cachedDate);
  }
}
...
function selectCrtBillingDayByArrow(key: BillingDirectionKey) {
  if (!billingPorts || !billingPorts.isActiveView() || billingMode !== 'days') return false;
  const points = billingSummary && billingSummary.daily && Array.isArray(billingSummary.daily.points)
    ? billingSummary.daily.points
    : [];
...
const billingSummaryDeadlineSlot: BillingDeadlineSlot = { timer: null, handle: 0 };
const billingDayDeadlineSlot: BillingDeadlineSlot = { timer: null, handle: 0 };
const billingTopbarDeadlineSlot: BillingDeadlineSlot = { timer: null, handle: 0 };
...
async function loadCrtBilling({ fresh = false } = {}) {
  ...
  billingError = '';
  renderCrtBilling();
  const token: BillingOperationToken = { valid: true, generation };
  const deadlineHandle = armBillingDeadline(
    billingSummaryDeadlineSlot, controller, CRT_BILLING_SUMMARY_DEADLINE_MS, token,
    () => { billingLoading = false; billingAbortController = null; renderCrtBilling(); },
  );
  try {
    ...
    if (!token.valid || generation !== billingGeneration || requestSequence !== billingRequestSequence) return;
    billingSummary = data.usage;
    pruneBillingDayDetailCache();
    renderCrtTopBarTokenRate();
  } catch (error) {
    if (!token.valid || controller.signal.aborted || generation !== billingGeneration || requestSequence !== billingRequestSequence) return;
...
async function loadCrtBillingDayDetail(dateValue: string, { force = false, live = false, retryCount = 0 } = {}) {
  ...
  const token: BillingOperationToken = { valid: true, generation };
  const deadlineHandle = armBillingDeadline(
    billingDayDeadlineSlot, controller, CRT_BILLING_DAY_DEADLINE_MS, token,
    () => { billingDayDetailLoading = false; billingDayDetailAbortController = null; },
  );
  ...
  try {
    ...
    if (!token.valid || generation !== billingGeneration || requestSequence !== billingDayDetailRequestSequence) return;
    ...
    if (!token.valid || controller.signal.aborted || generation !== billingGeneration || requestSequence !== billingDayDetailRequestSequence) return;
...
async function loadCrtTopBarTokenRate() {
  ...
  const token: BillingOperationToken = { valid: true, generation };
  const deadlineHandle = armBillingDeadline(
    billingTopbarDeadlineSlot, controller, CRT_BILLING_TOPBAR_DEADLINE_MS, token,
    () => { crtTokenRateAbortController = null; },
  );
  try {
    ...
    const data = await response.json().catch(() => null);
    if (!token.valid || controller.signal.aborted || generation !== billingGeneration) return;
    ...
    if (token.valid && !controller.signal.aborted && generation === billingGeneration) renderCrtTopBarTokenRate(null);
  } finally {
...
function startCrtTopBarTokenRateRefresh({ immediate = false } = {}) {
  ...
  if (immediate) {
    ...
    }, CRT_TOKEN_RATE_FIRST_LOAD_MS) as unknown as number;
  }
  crtTokenRateRefreshTimer = setInterval(() => {
    if (document.visibilityState !== 'hidden') void loadCrtTopBarTokenRate();
  }, CRT_TOKEN_RATE_REFRESH_MS) as unknown as number;
}
...
function startCrtBillingRefresh() {
  ...
  billingRefreshTimer = setInterval(() => {
    if (generation !== billingGeneration) return;
    if (billingPorts && billingPorts.isActiveView() && document.visibilityState !== 'hidden') void loadCrtBilling();
  }, CRT_BILLING_REFRESH_MS) as unknown as number;
  billingLiveDayRefreshTimer = setInterval(() => {
    if (generation !== billingGeneration) return;
    if (
      !billingPorts || !billingPorts.isActiveView()
      || billingMode !== 'days'
  ...
  }, CRT_BILLING_LIVE_DAY_REFRESH_MS) as unknown as number;
}
...
function selectCrtBillingMode(mode: string) {
  ...
  if (billingPorts) {
    ...
    if (selectedTab) billingPorts.setNavigationSelection(selectedTab);
  }
...
function refreshCrtBilling() {
  if (!billingPorts || !billingPorts.isActiveView() || billingLoading) return;
  void loadCrtBilling({ fresh: true });
}

function showCrtBilling() {
  if (!billingPorts) return;
  ...
  trackBillingFrame(() => {
    const refresh = document.getElementById('billing-refresh');
    const daysTab = document.getElementById('billing-days-tab');
    if (billingPorts && (daysTab || refresh)) billingPorts.setNavigationSelection(daysTab || refresh);
  });
}
...
function resumeBilling(isBillingView: boolean) {
...
const CrtBilling = {
  init(ports: BillingPorts) { billingPorts = ports; },
  show: showCrtBilling,
  hide: hideCrtBilling,
  leave: leaveBilling,
