...
// payloads are migrated incrementally without changing CRT runtime behavior.

type CrtDomElement = HTMLElement
  & HTMLInputElement
  & HTMLTextAreaElement
  & HTMLButtonElement
  & HTMLCanvasElement
...
interface Document {
  getElementById(elementId: string): CrtDomElement;
  querySelector<E extends Element = CrtDomElement>(selectors: string): E | null;
  querySelectorAll<E extends Element = CrtDomElement>(selectors: string): NodeListOf<E>;
}
...
interface Element {
  ...
  closest<E extends Element = CrtDomElement>(selectors: string): E | null;
}

declare const module: { exports?: Record<string, unknown> } | undefined;
declare function require(id: string): unknown;

type CrtTerminalSnapshotCell = {
  char?: string;
  width: number;
  attributes?: number;
  fg?: number;
...
type CrtTerminalSnapshot = {
  cells?: CrtTerminalSnapshotCell[][];
  cursorVisible?: boolean;
  cursorY?: number;
};
type CrtStructuredPreviewLine = { role?: string; label?: string; text?: string };
type CrtStructuredPreviewData = {
  messageLines?: CrtStructuredPreviewLine[];
  userText?: string;
  assistantText?: string;
  activityText?: string;
};
type CrtStructuredPreviewCacheEntry = {
  preview?: CrtStructuredPreviewData | null;
  loading?: boolean;
  ready?: boolean;
  error?: string;
  revision?: string;
...
type CrtNormalizedSessionView = Omit<CrtSessionPayload, 'agentId'> & {
  agentId: string | null;
  command: string;
  cwd: string;
  status: string;
...
type CrtTerminalLineProjection = { text: string; offsetToCell: number[] };
type CrtWrappedLinkSegment = CrtTerminalLineProjection & { row: number };
type CrtWrappedLinkContext = {
  mergedText: string;
  segments: CrtWrappedLinkSegment[];
  segmentOffsets: number[];
};
type CrtMainView = 'agents' | 'history' | 'search' | 'billing';
...
type BillingAnimatedMetric = {
  date: string;
  value: number;
  target: number;
  frame: number | null;
  write(value: number | null, target: number | null): void;
};
type BillingUsage = {
  totalTokens?: number;
  inputTokens?: number;
...
type BillingHour = BillingUsage & {
  hour?: number;
  cacheReadTokens?: number;
  cacheWriteTokens?: number;
};
type BillingDayPoint = BillingUsage & {
  date: string;
  cacheReadTokens?: number;
  cacheWriteTokens?: number;
  providers?: Record<string, BillingUsage>;
  unattributedTokens?: number;
};
type BillingDayDetail = BillingDayPoint & {
  hours: BillingHour[];
  total?: BillingUsage;
};
type BillingQuotaLimit = {
  usedPercent?: number;
...
type BillingProvider = {
  ...
  auth?: { available?: boolean; status?: string };
  quota?: { available?: boolean; primary?: BillingQuotaLimit; secondary?: BillingQuotaLimit };
};
...
type BillingDaily = {
  points: BillingDayPoint[];
  startDate?: string;
  endDate?: string;
  timeZone?: string;
  partial?: boolean;
  coverage?: Array<{ available?: boolean }>;
  summary?: {
    todayTokens?: number;
...
type BillingSummary = {
  ...
  liveTimeline?: BillingTimeline;
  sampledAt?: number;
};
type StructuredComposerFollowUp = {
  message: string;
  attachments: CrtStructuredPromptAttachment[];
};
type CrtGlobalSettings = {
  workspace: string;
...
type CrtSessionKeyboardContext = {
  structuredSessionActive?: boolean;
  composing?: boolean;
  structuredTranscriptFocused?: boolean;
  structuredToolFocused?: boolean;
  structuredMenuItemFocused?: boolean;
  structuredInterruptFocused?: boolean;
  structuredInputFocused?: boolean;
  structuredComposerMenuOpen?: boolean;
};

let ws: WebSocket | null = null;
let wsReconnectTimer: number | null = null;
let state: CrtWorkspaceState | null = null;
...
let agents: CrtAgent[] = [];
let waitingForAgent: boolean = false;
let workspaceDirectoryPrompt: { kind: string; workspace: string; code: string } | null = null;
let selectedAgentIndex: number | null = null;
let terminal: CrtTerminalInstance | null = null;
let fitAddon: CrtTerminalFitAddon | null = null;
let availableThemes: CrtTheme[] = [];
let currentTheme = 'terminal';
let themeSettings = {};
let currentSessionSkin: FarmingSessionSkin | null = null;
let currentSessionTitle = 'Agent Session';
let sessionClient: FarmingSessionClient | null = null;
let globalSettings: CrtGlobalSettings = {
  workspace: '',
  workspaceHistory: [],
  defaultLaunchAgent: '',
...
let crtContentFontSizeSaveTimer: number | null = null;
let workspaceHistorySelection = -1;
let workspaceHistoryExpanded = false;
let pendingMainAgentLaunch = false;
let pendingAgentLaunchPrefill: CrtAgentLaunchPrefill | null = null;
...
let searchQuery = '';
let searchAgentSessions: CrtProviderSession[] = [];
...
let billingRefreshTimer: number | null = null;
let billingLiveDayRefreshTimer: number | null = null;
let crtTokenRateRefreshTimer: number | null = null;
let crtTokenRateFirstLoadTimer: number | null = null;
let crtTokenRateAbortController: AbortController | null = null;
let billingCanvasFrame: number | null = null;
let billingMode = 'days';
let billingSelectedDate = '';
let billingSelectedHour: number | null = null;
let billingDailyRenderSignature = '';
let billingDayDetail: BillingDayDetail | null = null;
let billingDayDetailLoading = false;
let billingDayDetailError = '';
let billingDayDetailRequestSequence = 0;
let billingDayDetailAbortController: AbortController | null = null;
let billingDayDetailRetryTimer: number | null = null;
let billingDisplayedTotalDate = '';
let billingDisplayedTotalValue: number | null = null;
let billingTotalAnimationFrame: number | null = null;
let billingTotalAnimationTarget: number|null = null;
const billingAnimatedMetrics = new Map<string, BillingAnimatedMetric>();
const billingDayDetailCache = new Map<string, { detail: BillingDayDetail; fetchedAt: number }>();
let crtAgentPage = 0;
let crtAgentPageSize = 1;
let crtAgentPageColumns = 1;
let crtAgentPageResizeFrame: number | null = null;
let dashboardRenderDeferred = false;
let lastCrtDashboardSignature = '';
let crtPreviewRenderTimer: number | null = null;
let crtPreviewRenderFrame: number | null = null;
...
let structuredSessionSnapshot: CrtStructuredSessionSnapshot | null = null;
...
let structuredComposerAttachments: CrtHistoryComposerAttachment[] = [];
const structuredComposerHistory = new Map<string, string[]>();
const structuredComposerPendingFollowUps = new Map<string, StructuredComposerFollowUp[]>();
const structuredComposerRequestIds = new Map<string, string>();
...
let globalSettingsSaveTail: Promise<unknown> = Promise.resolve();
...
function farmingApiPath(path: string): string {
  return requiredRuntimePaths().apiPath(path);
}

function farmingWebSocketUrl(): string {
  return requiredRuntimePaths().webSocketUrl();
}

function findDirectionalNavigationIndex(
  rects: RectLike[],
  currentIndex: number,
  key: DirectionKey,
  wrap = false,
...
function calculateCrtAgentPageLayout(
  availableWidth: number,
  availableHeight: number,
  itemCount: number,
): { columns: number; rows: number; pageSize: number } {
...
function getCrtAgentPage<T>(items: T[], page: number, pageSize: number): CrtPage<T> {
...
function findDefaultNewAgentIndex(agentOptions: Array<{ name?: string }>, preferredAgentName: string) {
...
function crtHistoryItemUpdatedAt(item: CrtHistoryCandidate) {
  if (item.kind === 'session') {
...
function crtHistoryItemPriority(item: Pick<CrtHistoryItem, 'kind'>) {
  if (item.kind === 'session') return 30;
  if (item.kind === 'agent') return 20;
  return 10;
}

function shouldReplaceCrtHistoryItem(current: CrtHistoryItem, candidate: CrtHistoryItem) {
  if (candidate.updatedAt !== current.updatedAt) return candidate.updatedAt > current.updatedAt;
  return crtHistoryItemPriority(candidate) > crtHistoryItemPriority(current);
}

function crtHistorySessionDisplayKey(item: CrtHistoryItem) {
...
function buildCrtHistoryItems({ taskHistory = [], agents: agentRecords = [], sessions = [], mainPageSessionKeys = [] }: {
  ...
  const historyCandidates: CrtHistoryCandidate[] = [
    ...taskHistory.map((entry) => ({ kind: 'run' as const, historyKey: `run:${entry.id}`, entry })),
    ...agentRecords
      .filter((agent) => agent.isMain !== true && agent.archived === true)
      .map((agent) => ({ kind: 'agent' as const, historyKey: `agent:${agent.id}`, agent })),
    ...historySessions.map((session) => ({ kind: 'session' as const, historyKey: crtAgentSessionKey(session), session }))
  ];
  const candidates: CrtHistoryItem[] = historyCandidates.map((item) => ({
    ...item,
    updatedAt: crtHistoryItemUpdatedAt(item),
  }));

  const retained: CrtHistoryItem[] = [];
  const resumable = new Map<string, CrtHistoryItem>();
  ...
  const visualSessions = new Map<string, CrtHistoryItem>();
...
function getCrtNavigationItems(): CrtDomElement[] {
  const scope = getCrtNavigationScope();
  if (!scope) return [];
  return Array.from(scope.querySelectorAll<CrtDomElement>('[data-crt-nav-key]')).filter((element) => {
...
function setCrtNavigationSelection(element: CrtDomElement | null) {
  ...
  const scrollTarget = element.closest<CrtDomElement>('#settings-modal .settings-panel') || element;
...
function moveCrtNavigationSelection(key: DirectionKey) {
  const scope = getCrtNavigationScope();
...
let sessionSearchMatches: Array<{ line: number; startColumn: number; length: number; preview: string }> = [];
...
const SESSION_MODAL_BRIDGE: FarmingSessionModalBridge | null = (() => {
  ...
  if (typeof require === 'function') {
    try {
      return require('./session-modal-bridge.js') as FarmingSessionModalBridge;
    } catch {
      return null;
    }
  }
...
function isBrowserShortcut(event: KeyboardEvent) {
...
function normalizeTerminalSelectionText(terminalInstance: CrtTerminalInstance) {
  ...
  return selection.split('\n').reduce((text: string, part: string, index: number) => {
    if (index === 0) {
      return part;
    }
...
function getOrderedSelectionPosition(position: CrtTerminalSelectionPosition) {
...
function readTerminalLineSelectionText(line: CrtTerminalBufferLine | undefined, startCol: number, endCol: number) {
...
function rebuildTerminalSelectionFromBuffer(position: CrtTerminalSelectionPosition, buffer: CrtTerminalBuffer) {
  const ordered = getOrderedSelectionPosition(position);
  const rows = [];
  ...
  for (let row = ordered.start.y; row <= ordered.end.y; row += 1) {
    ...
    const fallbackEndCol = line ? line.length - 1 : ordered.end.x;
...
function isCurrentCrtSession(agentId: string | null, sessionToken: number | null = null) {
...
function isCrtNativeTerminalPasteTarget(
  target: EventTarget | { closest(selectors: string): Element | object | null } | null,
) {
  return Boolean(
    target
    && 'closest' in target
...
function normalizeCrtTerminalFontSize(value: unknown) {
...
function normalizeCrtContentFontSize(value: unknown) {
  const fontSize = Number(value);
  if (!Number.isFinite(fontSize)) return DEFAULT_CONTENT_FONT_SIZE;
...
async function createTerminalInstance(options: { disableStdin?: boolean } = {}) {
...
function showCrtTerminalFailure(titleText: string | null, error: unknown, detailText: string | null) {
...
function showCrtWebglFailure(error: unknown) {
...
function getAgentDisplayText(agent: Pick<CrtAgent, 'previewText' | 'output'> | null | undefined) {
  if (!agent) return '';
  ...
  return text
    ...
    .filter((line: string) => line && !decorativeLine.test(line) && !terminalChrome.test(line))
    .join('\n');
}

function crtPreviewColor(index: number | undefined) {
  const ansi16 = [
...
function getCrtPreviewCellStyle(cell: CrtTerminalSnapshotCell) {
...
function getCrtTerminalSnapshotRows(snapshot: CrtTerminalSnapshot | null | undefined) {
  if (!snapshot || !Array.isArray(snapshot.cells)) return [];
  let lastMeaningfulRow = -1;
  snapshot.cells.forEach((cells, rowIndex: number) => {
    if (!Array.isArray(cells)) return;
    const meaningful = cells.some((cell) => {
      if (!cell || cell.width === 0) return false;
      const attributes = cell.attributes || 0;
      const hasBackground = typeof cell.bg === 'number' && Number.isFinite(cell.bg) && cell.bg >= 0;
      return String(cell.char || '').trim() !== '' || hasBackground || Boolean(attributes & 0x10);
    });
    if (meaningful) lastMeaningfulRow = rowIndex;
  });
  const cursorRow = snapshot.cursorVisible === true && typeof snapshot.cursorY === 'number' && Number.isInteger(snapshot.cursorY)
...
function renderCrtTerminalSnapshot(container: HTMLDivElement, snapshot: CrtTerminalSnapshot | null | undefined) {
...
function crtStructuredPreviewStatus(agent: CrtAgent, cached: CrtStructuredPreviewCacheEntry | null) {
...
function appendCrtStructuredPreviewLine(
...
function renderCrtStructuredPreview(output: Element, agent: CrtAgent) {
...
function renderCrtAgentOutput(output: Element, agent: CrtAgent, { main = false } = {}) {
...
function crtDashboardStateSignature(value: CrtWorkspaceState|null) {
...
function scheduleCrtRenderedStructuredPreviews() {
  ...
  document.querySelectorAll('#map-area .agent-block[data-agent-id], #main-agent-block[data-agent-id]')
    .forEach((block) => {
      const agent = currentState.agents.find((candidate) => candidate.id === block.dataset.agentId);
      if (agent) scheduleCrtStructuredPreviewRefresh(agent);
    });
}

function updateCrtAgentPreviewCard(agent: CrtAgent|null|undefined) {
...
function flushCrtPreviewCardRenders() {
  ...
  pending.forEach(({ agent, previewChanged }) => {
    const currentState = state;
    const currentAgent = currentState && currentState.agents.find((candidate) => candidate.id === agent.id);
    ...
    if (!updateCrtAgentPreviewCard(currentAgent)) {
      if (currentAgent.id !== currentState?.mainAgentId && !isCrtAgentOnCurrentPage(currentAgent.id)) {
...
function scheduleCrtPreviewCardRender(agent: CrtAgent, previewChanged = false) {
...
function crtCommandProgram(command: unknown) {
...
function crtAgentDisplayName(command: unknown) {
  const program = crtCommandProgram(command);
  return CRT_AGENT_DISPLAY_NAMES[program] || program || 'Agent';
}

function crtWorkspaceBasenames(agent: Pick<CrtAgent, 'cwd' | 'projectWorkspace'>) {
...
function getCrtProjectName(
  agent: Pick<CrtAgent, 'projectWorkspace' | 'cwd'>,
  projectNames = globalSettings && globalSettings.projectNames,
) {
...
function crtTitleComparisonKey(title: string) {
  return String(title || '')
    .trim()
    .replace(/^[\s*＊✳✱✲✶·•:.◇✋✦⏲\u2800-\u28FF]+/u, '')
    .replace(/\s+/g, ' ')
...
function isCrtAgentWorking(agent: CrtAgent) {
  if (!agent) return false;
  if (agent.status === 'pending') return true;
  if (agent.status !== 'running') return false;

  const terminalStatus = typeof agent.terminalStatus === 'object' && agent.terminalStatus !== null
    ? agent.terminalStatus
    : null;
  const activity = terminalStatus && terminalStatus.activity;
  if (activity === 'busy') return true;
  if (activity === 'idle' || activity === 'exited') return false;
  if (terminalStatus && terminalStatus.busy === true) return true;
...
function getCrtAgentReadPatch(agent: CrtAgent) {
...
async function markCrtAgentReadIfNeeded(agent: CrtAgent) {
  ...
  try {
    ...
    console.warn('Failed to mark CRT agent as read:', error instanceof Error ? error.message : error);
  }
}

function getCrtBrandPaneKey(agentId: unknown, currentState: CrtWorkspaceState|null) {
...
function isCrtBrandAgentLive(agent: CrtAgent) {
  return Boolean(agent && (agent.status === 'running' || agent.status === 'pending'));
}

function updateCrtBrandState(currentState: CrtWorkspaceState) {
...
function pulseCrtBrandForAgent(agentId: unknown) {
...
function normalizeSessionLink(rawUrl: string) {
...
function extractSessionLinks(text: unknown, limit = SESSION_LINK_LIMIT) {
...
function formatSelectionStatus(
  selectionPosition: CrtTerminalSelectionPosition | undefined,
  selectionText = '',
) {
...
function deriveSessionSearchMatchesFromLines(lines: unknown[], query: string) {
  const normalizedQuery = typeof query === 'string' ? query.trim().toLowerCase() : '';
  if (!normalizedQuery) {
    return [];
  }

  const matches: { line: number; startColumn: number; length: number; preview: string; }[] = [];
  lines.forEach((lineText: unknown, lineIndex: number) => {
...
function buildTerminalLineProjection(line: CrtTerminalBufferLine | undefined): CrtTerminalLineProjection {
...
function crtLogicalTerminalLineAtRow(currentTerminal: CrtTerminalInstance | null, row: number) {
...
function saveCrtTerminalReadingAnchor(agentId: string, currentTerminal = terminal) {
...
function restoreCrtTerminalReadingAnchor(agentId: string, currentTerminal = terminal) {
  const api = crtReadingAnchorApi();
...
function saveCrtStructuredReadingAnchor(agentId: string|null) {
  const api = crtReadingAnchorApi();
...
function restoreCrtStructuredReadingAnchor(agentId: string|null) {
  const api = crtReadingAnchorApi();
  const container = document.getElementById('terminal-output');
...
function collectWrappedLinkContext(buffer: CrtTerminalBuffer, row: number): CrtWrappedLinkContext | null {
...
function mapMergedOffsetToTerminalPosition(context: CrtWrappedLinkContext, offset: number) {
...
function collectTerminalHyperlinkRange(
  buffer: CrtTerminalBuffer,
  row: number,
  column: number,
  hyperlinkId: number,
) {
...
function createTerminalOsc8LinkProvider(terminalInstance: CrtTerminalInstance): CrtTerminalLinkProvider {
  return {
...
function createTerminalUrlLinkProvider(terminalInstance: CrtTerminalInstance): CrtTerminalLinkProvider {
  return {
    provideLinks(row, callback) {
      const buffer = terminalInstance.buffer.active;
...
function registerTerminalLinks(terminalInstance: CrtTerminalInstance | null) {
...
function getTerminalBufferLines(terminalInstance: CrtTerminalInstance | null) {
  const buffer = terminalInstance && terminalInstance.buffer ? terminalInstance.buffer.active : null;
...
function navigateSessionSearchMatch(direction: number) {
  if (!sessionSearchMatches.length) {
...
function deriveSessionTextPatch(fullText: string | unknown[], previousLength = 0, forceReplace = false) {
...
function replaceTerminalOutput(terminalInstance: CrtTerminalInstance | null, text: string) {
...
function normalizeSessionViewPayload(
  payload: Record<string, unknown> | null | undefined,
  fallbackAgent: CrtAgent | null = null,
): CrtNormalizedSessionView {
  const session = (
    payload && typeof payload.session === 'object' && payload.session !== null
      ? payload.session
      : {}
  ) as CrtSessionPayload;
...
function settleInitialCrtTerminalFocus(replication: CrtTerminalReplication, terminalInstance = terminal) {
...
function installCrtTerminalTestApi() {
  ...
  window.__farmingCrtTerminalTest = {
    ...
    notifyResizeForTest(cols: number, rows: number) {
      sendSessionResize(focusedAgentId, { cols, rows });
    },
    streamSequenced(data: unknown, outputSeq: number, runtimeEpoch: string, stateRevision: number) {
      const replication = crtTerminalReplication;
    ...
    replaceStream(stream: FarmingTerminalTransition) {
      const replication = crtTerminalReplication;
...
function scheduleCrtTerminalCheckpointRetry(replication: CrtTerminalReplication, delay: number) {
  if (
    !replication ||
    replication.disposed ||
    replication.replayState.halted ||
...
function retryCrtTerminalReplayAfterFailure(
  replication: CrtTerminalReplication,
  failure: FarmingTerminalReplayFailure,
  error: unknown = null,
) {
  if (!replication || replication.disposed) return;
...
function stopCrtTerminalReplay(replication: CrtTerminalReplication, error: unknown) {
  if (!replication || replication.disposed) return;
  replication.checkpointInFlight = false;
...
function applyCrtTerminalTransition(event: FarmingTerminalTransition) {
  ...
  if (event.kind === 'resize') {
    ...
    try {
      terminal.resize(Math.floor(Number(event.cols)), Math.floor(Number(event.rows)));
    } finally {
...
function installCrtTerminalCheckpoint(sessionView: CrtTerminalSessionView | CrtNormalizedSessionView) {
  ...
  const validatedCheckpoint = checkpoint as FarmingTerminalCheckpoint;
  const validatedSessionView: CrtTerminalSessionView = {
...
function handleCrtTerminalStream(stream: CrtTerminalStream) {
  const replication = crtTerminalReplication;
...
function deriveSessionStreamPatch(
...
function createSessionModalState(
  agent: CrtAgent | null,
  themeId: string,
  currentThemeSettings: Record<string, unknown>,
...
function setSessionOutputLength(length: number) {
  const runtime = getSessionRuntime();
  if (runtime) {
...
async function fetchCrtSettingsRequest(url: RequestInfo | URL, init: RequestInit = {}) {
...
async function loadGlobalSettings() {
  try {
    ...
    applyCrtContentFontSize(
      data.settings?.crtContentFontSize === undefined
...
async function saveGlobalSettings(settingsPatch: Partial<CrtGlobalSettings>) {
  const patch = settingsPatch && typeof settingsPatch === 'object'
    ? { ...settingsPatch }
    : {};
  const keys = Object.keys(patch) as Array<keyof CrtGlobalSettings>;
    ...
    try {
      ...
      if (data.success) {
        ...
        keys.forEach((key) => {
          ...
          const value = Object.prototype.hasOwnProperty.call(savedSettings, key)
            ? savedSettings[key]
            : patch[key];
...
function activateUiTheme(themeId: string) {
...
function applyCRTEffects(enabled: boolean) {
...
function applyCrtContentFontSize(value: unknown) {
  ...
  if (input) input.value = String(contentFontSize);
  if (output) output.textContent = `${contentFontSize} px`;
  if (terminal && terminal.options) terminal.options.fontSize = terminalFontSize;
...
function scheduleCrtContentFontSizeSave() {
  if (crtContentFontSizeSaveTimer) clearTimeout(crtContentFontSizeSaveTimer);
...
function formatCrtHistoryAge(timestamp: unknown, now = Date.now()) {
...
function crtHistoryItemTitle(item: CrtHistoryItem) {
  if (item.kind === 'run') {
...
function crtHistoryAgentName(command: unknown) {
...
function crtHistoryItemMeta(item: CrtHistoryItem) {
  if (item.kind === 'run') {
    return [
...
function crtHistoryPrimaryAction(item: Pick<CrtHistoryItem, 'kind'>) {
  if (item.kind === 'run') return 'Continue';
  if (item.kind === 'agent') return 'Open';
  return 'Resume';
}

function setCrtMainView(view: string) {
  const previousView = crtMainView;
  crtMainView = view === 'history' || view === 'search' || view === 'billing' ? view : 'agents';
...
function getCrtSearchResults() {
  return buildCrtSearchResults({
    ...
    mainAgentId: state && typeof state.mainAgentId === 'string' ? state.mainAgentId : '',
    projectNames: globalSettings.projectNames,
  });
}

function crtSearchResultTitle(result: CrtSearchResult) {
...
function crtSearchResultMeta(result: CrtSearchResult) {
...
async function loadCrtSearchAgentSessions(query: string, requestSequence: number) {
  const controller = new window.AbortController();
...
function scheduleCrtSearch(query: unknown) {
...
function moveCrtSearchSelection(direction: number) {
  const results = getCrtSearchResults();
...
function formatCrtUsageValue(value: string|number|null|undefined) {
...
function formatCrtExactUsageValue(value: string|number|null|undefined) {
  if (value === null || value === undefined || value === '') return '--';
  const numberValue = Number(value);
  if (!Number.isFinite(numberValue) || numberValue < 0) return '--';
...
function formatCrtCompactTotalValue(value: string|number|null|undefined) {
  if (value === null || value === undefined || value === '') return '--';
  const numberValue = Number(value);
  if (!Number.isFinite(numberValue) || numberValue < 0) return '--';
...
function parseCrtBillingDate(dateValue: unknown) {
  const parts = String(dateValue || '').split('-').map(Number);
  if (parts.length !== 3 || parts.some(part => !Number.isFinite(part))) return null;
  return new Date(parts[0], parts[1] - 1, parts[2], 12, 0, 0, 0);
}

function crtBillingDayLabel(dateValue: unknown) {
...
function updateCrtBillingAnimatedMetric(key: string, value: unknown, {
  date = '',
  live = false,
  write,
}: {
  date?: string;
  live?: boolean;
  write?: BillingAnimatedMetric['write'];
} = {}) {
  ...
  if (shouldSnap) {
    const metric: BillingAnimatedMetric = { date, value: roundedTarget, target: roundedTarget, frame: null, write };
    billingAnimatedMetrics.set(key, metric);
    write(roundedTarget, roundedTarget);
    return;
  }

  const metric: BillingAnimatedMetric = {
    date,
...
function updateCrtBillingExactMetric(id: string, value: unknown, { date = '', live = false }: {
  date?: string;
  live?: boolean;
} = {}) {
...
function writeCrtBillingTotalDisplay(value: number|null, target: number|null, { live = false }: { live?: boolean } = {}) {
...
function updateCrtBillingTotalDisplay(value: unknown, { date = '', live = false }: {
  date?: string;
  live?: boolean;
} = {}) {
  const target = Number(value);
...
function crtBillingOverrangeTier(value: number) {
...
function crtBillingOverrangeLabel(tier: number) {
  return tier > 0 ? `${2 ** (tier - 1)}B+ OVERRANGE` : '';
}

function crtBillingHeatThresholds(values: unknown) {
...
function crtBillingHeatLevel(value: number, thresholds: number[]) {
  const total = Math.max(0, Number(value) || 0);
  if (total <= 0) return 0;
...
function crtBillingDayDetailHasHourlyActivity(detail: BillingDayDetail | null) {
  return Boolean(detail && Array.isArray(detail.hours) && detail.hours.some((hour) => (
    Math.max(0, Number(hour && hour.totalTokens) || 0) > 0
  )));
}

function crtBillingHourlyPath(hours: BillingHour[], valueForHour: (hour: BillingHour) => number, maximum: number) {
  const width = 600;
  const height = 120;
...
function renderCrtBillingDayInsight() {
  ...
  if (totalPath) totalPath.setAttribute('d', crtBillingHourlyPath(hours, (hour) => Number(hour.totalTokens) || 0, maximum));
...
(    hour) => (Number(hour.cacheReadTokens) || 0) + (Number(hour.cacheWriteTokens) || 0),
  ...
  updateCrtBillingAnimatedMetric('billing-day-curve-scale', maximum > 0 ? maximum : null, {
    ...
    live: isToday,
    write: (displayed: string|number|null|undefined) => {
  ...
  if (hourStrip) {
    ...
    if (hours.length > 0) {
      ...
      if (billingSelectedHour === null || !Number.isInteger(billingSelectedHour) || billingSelectedHour < 0 || billingSelectedHour >= hours.length) {
        billingSelectedHour = hours.reduce((peakIndex: number, hour: BillingHour, index: number) => (
          Number(hour && hour.totalTokens) > Number(hours[peakIndex] && hours[peakIndex].totalTokens) ? index : peakIndex
        ), 0);
      }
      const selectHour = (index: number, { focus = false } = {}) => {
        ...
        if (focus) (hourStrip.children[index] as HTMLElement | undefined)?.focus();
      };
      hours.forEach((hour: BillingHour, index: number) => {
...
function selectCrtBillingDay(dateValue: string | undefined, { focus = false } = {}) {
...
async function loadCrtBillingDayDetail(dateValue: string, { force = false, live = false, retryCount = 0 } = {}) {
...
function crtBillingDayArrowTargetIndex(points: BillingDayPoint[], index: number, key: DirectionKey) {
...
function selectCrtBillingDayByArrow(key: DirectionKey) {
...
function renderCrtBillingDaily(summary = billingSummary) {
  ...
  const setValue = (id: string, value: string | number | null | undefined) => {
    const element = document.getElementById(id);
  ...
  if (signature !== billingDailyRenderSignature) {
    ...
    chartPoints.forEach((point, index: number) => {
...
function setCrtBillingMode(mode: string) {
...
function selectCrtBillingMode(mode: string) {
...
function formatCrtBillingWindow(windowMinutes: number) {
...
function crtBillingTimelineLabels(timeline: BillingTimeline | null) {
...
function formatCrtBillingReset(resetsAt: unknown, now = Date.now()) {
...
function appendCrtBillingMessage(container: HTMLElement, text: string|null, isError = false) {
...
function drawCrtBillingScope(summary = billingSummary) {
  ...
  const xAt = (index: number) => paddingX + (points.length <= 1 ? graphWidth : index / (points.length - 1) * graphWidth);
  const yAt = (value: number) => paddingY + graphHeight - Math.max(0, Math.min(1, value / peak)) * graphHeight;

  const strokeSeries = (series: number[], color: string, lineWidth: number, dash: number[] = []) => {
    if (!series.length) return;
    context.save();
    context.beginPath();
    series.forEach((value, index: number) => {
      const x = xAt(index);
      const y = yAt(value);
...
function createCrtHistoryMessage(className: string, text: string|null) {
  const message = document.createElement('div');
...
function calculateCrtHistoryPageSize(availableHeight: number, rowHeight = 68) {
...
function getCrtHistoryPage<T>(items: T[], page: number, pageSize: number): CrtPage<T> {
  const source = Array.isArray(items) ? items : [];
  const size = Math.max(1, Math.floor(Number(pageSize) || 1));
  const totalPages = Math.max(1, Math.ceil(source.length / size));
  const currentPage = Math.max(0, Math.min(totalPages - 1, Math.floor(Number(page) || 0)));
...
function updateCrtHistoryPagination(pageState: CrtPage<CrtHistoryItem>) {
...
function changeCrtHistoryPage(direction: number) {
  const historyItems = getCrtHistoryItems();
...
async function requestCrtSessionResume(resumed: CrtResumedProviderSession, customTitle = '') {
...
async function resumeCrtHistorySession(
  resumed: CrtResumedProviderSession | null,
...
async function resumeCrtSearchSession(session: CrtProviderSession, searchKey: string) {
  const resumed = crtHistoryItemResumeSession({ kind: 'session', historyKey: searchKey, session });
...
async function restoreCrtArchivedAgent(agentId: string|number|boolean, openAfterRestore: boolean, historyKey: string) {
  ...
  try {
    ...
    if (openAfterRestore) pendingProviderSessionOpenAgentId = String(agentId);
    historyActionPendingKey = '';
    hideHistory();
    openPendingProviderSessionAgentIfReady();
  } catch (error) {
...
function activateCrtHistoryItem(item: CrtHistoryItem) {
...
function getDefaultWorkspaceForDialog(asMainAgent: boolean) {
  return asMainAgent ? getRememberedWorkspace() : '';
}

function resolveWorkspaceToStart(workspaceInput: string, asMainAgent: boolean) {
...
function normalizeWorkspaceValue(workspace: string) {
  return typeof workspace === 'string' ? workspace.trim() : '';
}

function shouldRememberWorkspace(workspace: string) {
...
function buildWorkspaceHistory(workspace: string, history: string[] = []) {
...
function rememberWorkspace(workspace: unknown) {
  if (typeof workspace !== 'string' || !shouldRememberWorkspace(workspace)) {
    return;
  }
...
function formatWorkspaceForDisplay(workspace: string) {
  const value = normalizeWorkspaceValue(workspace);
  if (!value) {
    return '~/.farming';
  }
...
function selectWorkspaceHistory(index: number, { focusInput = true } = {}) {
...
function moveWorkspaceHistorySelection(direction: number) {
  const history = getWorkspaceHistory();
  if (!history.length) {
    return false;
  }
...
async function prepareCrtWorkspaceDirectory(workspace: string, create: boolean) {
...
function showWorkspaceDirectoryPrompt(kind: string, { workspace, code = '' }: {
  workspace: string;
  code?: string;
}) {
...
async function startCrtAgent(agent: CrtAgent, workspaceToUse: string|null, asMainAgent: boolean) {
...
function selectAgent(index: number|null) {
...
function connect(): void {
  ...
  socket.onmessage = (event: MessageEvent<string>) => {
    ...
    if (data.type === 'state') {
      ...
      const shouldApply = runtime && runtimeResult
...
function getCrtMainAgentDialogAction({
  ...
  currentState?: CrtWorkspaceState | null;
  mainView?: string;
  dialogActive?: boolean;
  pendingMainAgent?: boolean;
} = {}) {
...
function formatSystemClock(timestamp: unknown, timeZone: unknown) {
  if (typeof timestamp !== 'number' || !Number.isFinite(timestamp)) return '--';
  try {
...
function formatCrtTokenRate(value: string|number|null|undefined) {
  if (value === null || value === undefined || value === '') return '--';
...
function updateSystemStats(stats: CrtProtocolSystemStats, uptime: number | undefined) {
  if (stats.cpu !== undefined) {
    document.getElementById('cpu-usage').textContent = String(stats.cpu);
  }

  if (stats.memory) {
    document.getElementById('mem-percentage').textContent = String(stats.memory.percentage ?? '');
  }
...
function isCrtLiveAgent(agent: CrtAgent | null | undefined): agent is CrtAgent {
  return Boolean(
    agent
    && agent.archived !== true
    && agent.status !== 'dead'
...
function updateCrtAgentPageStatus(pageState: CrtPage<CrtAgent>) {
...
function isCrtAgentOnCurrentPage(agentId: string) {
...
function moveCrtAgentPageSelection(key: string) {
...
function renderState() {
  ...
  if (state.mainAgentId) {
    const currentState = state;
    const mainAgent = currentState.agents.find(a => a.id === currentState.mainAgentId);
...
function showInputDialog(prefill: CrtAgentLaunchPrefill | null = null) {
  hideWorkspaceDirectoryPrompt({ focusInput: false });
  clearCrtNavigationSelection();
  void loadAgents();
...
function hideInputDialog(options: { force?: boolean } = {}) {
  const needMainAgent = needsMainAgent();
...
function selectCrtStartedAgent(agentId: unknown) {
...
function crtRuntimeView(agent: CrtAgent|undefined) {
...
function crtRuntimeSwitchUnavailableReason(agent: CrtAgent|undefined) {
...
function isCrtRuntimeSwitchShortcut(event: KeyboardEvent) {
  return Boolean(
    event
    && event.altKey
    && !event.ctrlKey
...
function hasCrtStructuredLocalEscapeAction(context: CrtSessionKeyboardContext = {}) {
  return Boolean(
...
function resolveCrtSessionKeyboardCommand(event: KeyboardEvent, context: CrtSessionKeyboardContext = {}) {
  if (!event) return '';
...
function updateCrtRuntimeSwitchControl(agent: CrtAgent|undefined) {
...
function updateCrtSessionCloseControl(agent: CrtAgent) {
...
async function switchCrtSessionRuntimeMode(mode: string) {
...
function structuredRuntimeStatus(agent: CrtAgent | null | undefined) {
  if (!agent) return '';
  return String(agent.runtimeBinding?.state || 'idle');
}

function structuredRuntimeError(agent: CrtAgent | null | undefined) {
  if (!agent) return '';
  return String(agent.runtimeBinding?.error || '');
}

function structuredComposerAction(agent: CrtAgent|null|undefined, draft = '') {
...
function queueStructuredComposerFollowUp(
  agentId: string,
  message: string,
  attachments: CrtStructuredPromptAttachment[] = [],
) {
...
function flushStructuredComposerFollowUp(agent: CrtAgent | null | undefined) {
...
function structuredContextUsage(session: CrtStructuredSessionSnapshot | null | undefined) {
...
function formatStructuredUsage(session: CrtStructuredSessionSnapshot | null | undefined) {
...
function updateStructuredUsageDisplay(
  element: HTMLElement | null,
  session: CrtStructuredSessionSnapshot | null | undefined,
) {
  if (!element) return;
  const contextUsage = structuredContextUsage(session);
...
function structuredSelectOptions(option: CrtStructuredConfig) {
...
function structuredVisibleConfigOptions(session: CrtStructuredSessionSnapshot | null | undefined) {
...
function currentStructuredConfigLabel(session: CrtStructuredSessionSnapshot | null | undefined) {
  const options = structuredVisibleConfigOptions(session);
  const model = options.find((option: { type: string; id: unknown; name: unknown; }) => option.type === 'select' && /(^|[\s_-])model([\s_-]|$)/i.test(`${option.id} ${option.name}`));
  if (!model) return 'CONFIG';
  const selected = structuredSelectOptions(model).find((option: { value: unknown; }) => option.value === model.currentValue);
  return selected && selected.name ? selected.name : model.currentValue || 'CONFIG';
}

function structuredConfigValueLabel(option: CrtStructuredConfig) {
  if (!option) return '';
  if (option.type === 'boolean') return option.currentValue ? 'ON' : 'OFF';
  const selected = structuredSelectOptions(option)
    .find((candidate: { value: unknown; }) => candidate.value === option.currentValue);
  return selected && selected.name ? selected.name : String(option.currentValue || '');
}

function isCurrentStructuredSession(agentId: string|null, generation: number) {
  return Boolean(
    agentId
    && agentId === focusedAgentId
...
function setStructuredComposerMenu(menu: string, { focusFirst = false, opener = null }: {
  focusFirst?: boolean;
...
function structuredComposerToolbarButtons() {
  ...
  ].filter((button): button is CrtDomElement => Boolean(button && !button.hidden && !button.disabled));
}

function focusStructuredComposerToolbarButton(current: HTMLElement | null, offset = 0) {
  const buttons = structuredComposerToolbarButtons();
  if (!buttons.length) return false;
  const currentIndex = current ? buttons.indexOf(current as CrtDomElement) : -1;
...
function structuredComposerMenuButtons() {
  ...
  return Array.from(menu.querySelectorAll<CrtDomElement>('.crt-structured-menu-item'))
    .filter((button) => !button.disabled);
}

function focusStructuredComposerMenuButton(current: HTMLElement | null, offset = 0) {
...
function captureStructuredComposerMenuFocus(menu: HTMLElement) {
...
function restoreStructuredComposerMenuFocus(snapshot: { path: string; key: unknown; index: number; }|null) {
...
async function patchStructuredAcpSession(
  agentId: string,
  generation: number,
  patch: { modeId?: string; configId?: string; value?: unknown },
) {
...
function structuredMenuButton(
  label: unknown,
  description: unknown,
  active: boolean,
  onClick: ((this: GlobalEventHandlers, ev: PointerEvent) => unknown) | null,
  menuKey = '',
) {
...
function renderStructuredComposerMenu() {
  ...
  const agentId = focusedAgentId;
  const generation = structuredSessionGeneration;
  if (!structuredComposerMenu || !session || !agentId) {
    menu.hidden = true;
    return;
  }

  const selectedConfig = structuredComposerMenu === 'config'
    ? structuredVisibleConfigOptions(session).find((option: { id: string; }) => option.id === structuredComposerConfigId)
    : null;
  const title = document.createElement('div');
  ...
  if (structuredComposerMenu === 'commands') {
    ...
    (session.availableCommands || [])
      .filter((command: { name: unknown; }) => !query || String(command.name || '').toLowerCase().includes(query))
      .slice(0, 12)
      .forEach((command) => {
...
function renderStructuredSessionControls() {
  ...
  const currentMode = modes.find((mode: { id: unknown; }) => mode.id === (session && (session.currentModeId || (session.modes && session.modes.currentModeId))));
...
async function refreshStructuredSessionControls(
  ...
  try {
    ...
    structuredSessionSnapshot = body.session as CrtStructuredSessionSnapshot;
...
function showStructuredComposerError(error: unknown) {
  ...
  status.textContent = error instanceof Error && error.message
    ? error.message
    : String(error || 'Structured session error');
  status.classList.add('error');
}

function resizeStructuredComposerInput(input: HTMLTextAreaElement) {
  if (!input) return;
...
function setStructuredComposerActive(active: boolean|undefined) {
...
function updateStructuredComposerState(agent: CrtAgent|null|undefined, { refreshControls = true } = {}) {
  const input = document.getElementById('crt-structured-input');
...
function structuredAttachmentId(file: Pick<File, 'name'>) {
  return `crt-${Date.now()}-${String(file && file.name || 'attachment')}-${Math.random().toString(36).slice(2, 8)}`;
}

function structuredAttachmentBlock(current: string, block: unknown) {
...
function readStructuredTextFile(file: Blob): Promise<string> {
...
async function prepareStructuredAttachment(file: File) {
  const attachment: CrtHistoryComposerAttachment = {
  ...
  try {
    ...
    attachment.error = error instanceof Error && error.message ? error.message : 'Attachment failed';
    showStructuredComposerError(error);
  }
  renderStructuredComposerAttachments();
  updateStructuredComposerState(state && state.agents.find((agent) => agent.id === focusedAgentId));
...
function addStructuredAttachmentFiles(files: FileList | File[]) {
  Array.from(files || []).forEach((file) => void prepareStructuredAttachment(file));
}

function structuredComposerMessage(draft: unknown) {
...
function structuredComposerPromptAttachments(): CrtStructuredPromptAttachment[] {
...
function structuredComposerRequestKey(
  agentId: string,
  message: string,
  attachments: CrtStructuredPromptAttachment[] = [],
) {
...
function structuredComposerRequestId(requestKey: string) {
...
function structuredComposerHistoryFor(agentId: string|null) {
  if (!agentId) return [];
  return structuredComposerHistory.get(agentId) || [];
}

function addStructuredComposerHistory(agentId: string, value: unknown) {
...
function navigateStructuredComposerHistory(input: HTMLTextAreaElement, direction: number) {
...
function renderStructuredPermissions(agent: CrtAgent | null | undefined) {
  ...
  const requests = (Array.isArray(runtime.pendingPermissions) && runtime.pendingPermissions.length
    ...
    : (runtime.pendingPermission ? [runtime.pendingPermission] : [])) as Array<
      CrtPermissionRequest | CrtProtocolPendingPermission
    >;
...
async function respondToStructuredPermission(
  agentId: string,
  generation: number,
  requestId: string,
...
function structuredTranscriptContentText(content: unknown) {
...
function structuredTranscriptTurns(transcript: CrtStructuredTranscript | null | undefined): CrtStructuredTurn[] {
  ...
  const turns: CrtStructuredTurn[] = [];
  let current: CrtStructuredTurn | null = null;
...
function normalizeCrtStructuredPreviewText(value: unknown, limit = 240) {
  const text = String(value || '').replace(/\s+/g, ' ').trim();
  if (text.length <= limit) return text;
  return `${text.slice(0, Math.max(1, limit - 1)).trimEnd()}…`;
}

function structuredTranscriptAttachmentLabel(content: unknown) {
  const blocks = Array.isArray(content) ? content as CrtStructuredContentBlock[] : [];
...
function crtStructuredPreviewMessageLines(transcript: CrtStructuredTranscript, limit = 8) {
  ...
  const entries = transcript && Array.isArray(transcript.entries)
    ? transcript.entries
      ...
      .filter((line: { text: unknown; }) => line.text)
    : [];
  return entries.slice(-maxLines);
}

function structuredEntryActivityText(entry: CrtStructuredEntry) {
...
function buildCrtStructuredPreview(transcript: CrtStructuredTranscript, agent: CrtAgent | null = null) {
  const entries = transcript && Array.isArray(transcript.entries)
...
function crtStructuredPreviewRevision(agent: CrtAgent|null|undefined) {
...
async function refreshCrtStructuredPreview(agent: CrtAgent|null|undefined) {
  if (!isCrtLiveAgent(agent) || !isStructuredRuntimeAgent(agent)) return;
  const revision = crtStructuredPreviewRevision(agent);
  const cached = crtStructuredPreviewCache.get(agent.id);
  if (cached && cached.revision === revision) {
...
function renderStructuredTranscript(
  transcript: CrtStructuredTranscript,
  force = false,
  agentId = focusedAgentId,
) {
  const container = document.getElementById('terminal-output');
  ...
  try {
    ...
    errorNode.textContent = error instanceof Error && error.message
      ? error.message
      : 'Failed to render Markdown transcript';
    container.replaceChildren(errorNode);
    structuredSessionRenderedAt = updatedAt;
    return;
  }
...
async function refreshStructuredSession(
  ...
  try {
    ...
    if (container && !container.querySelector('.crt-structured-transcript')) {
      ...
      message.textContent = error instanceof Error && error.message ? error.message : 'Failed to read conversation';
...
function startStructuredSessionPolling(agentId: string | null, generation: number) {
...
function setupStructuredSessionComposer() {
  ...
  input.addEventListener('keydown', (event) => {
    ...
    if (event.key === 'Escape') {
      ...
      if (structuredComposerAction(agent, input.value) === 'interrupt') {
        ...
        const interrupted = focusedAgentId ? getSessionClient()?.interruptAgent(focusedAgentId) : false;
        if (!interrupted) {
          structuredComposerRestoreFocusAfterInterrupt = false;
  ...
  if (fileInput) fileInput.addEventListener('change', () => {
    if (fileInput.files) addStructuredAttachmentFiles(fileInput.files);
    fileInput.value = '';
  });
  if (commandButton) commandButton.addEventListener('click', (event) => setStructuredComposerMenu('commands', { opener: event.currentTarget as HTMLElement }));
  if (modeButton) modeButton.addEventListener('click', (event) => setStructuredComposerMenu('mode', { opener: event.currentTarget as HTMLElement }));
  if (configButton) configButton.addEventListener('click', (event) => setStructuredComposerMenu('config', { opener: event.currentTarget as HTMLElement }));
  composer.addEventListener('keydown', (event) => {
    const eventTarget = event.target instanceof Element ? event.target : null;
    const tool = eventTarget
      ? eventTarget.closest<HTMLElement>('.crt-structured-tool')
      : null;
    if (!tool) return;
  ...
  if (menu) menu.addEventListener('keydown', (event) => {
    ...
    const item = eventTarget
      ? eventTarget.closest<HTMLElement>('.crt-structured-menu-item')
      : null;
    if (!item) return;
    if (event.key === 'ArrowDown' || event.key === 'ArrowUp') {
      event.preventDefault();
...
async function openStructuredSession(agent: CrtAgent, sessionToken: number) {
  const runtime = getSessionRuntime();
  const container = document.getElementById('terminal-output');
  if (!container) return;
  const generation = ++structuredSessionGeneration;
...
async function openSession(agentId: string|null|undefined) {
  if (!state) return;
  ...
  const sessionToken = openResult ? openResult.sessionToken : 0;
  if (runtime) {
    syncSessionRuntimeState();
  }
  ...
  if (focusRegion && focusRegionName) {
    ...
    focusRegionName.textContent = String(agent.command || '').split(' ')[0];
  }
  ...
  const domState = runtime && openResult
    ? openResult.domState
...
function clearPendingCrtTerminalFitResize(replication: CrtTerminalReplication) {
  if (!replication) return;
  if (replication.fitResizeTimer) clearTimeout(replication.fitResizeTimer);
  replication.fitResizeTimer = null;
  replication.pendingFitResize = null;
...
function commitCrtTerminalResize(
  agentId: string,
  normalizedDimensions: CrtTerminalDimensions,
...
function scheduleCrtTerminalFitResize(
  replication: CrtTerminalReplication,
  agentId: string,
  normalizedDimensions: CrtTerminalDimensions,
) {
...
function sendSessionResize(
  agentId: string | null = focusedAgentId,
  requestedDimensions: CrtTerminalDimensions | null = null,
...
if (typeof document !== 'undefined') {
  ...
  document.addEventListener('copy', (e) => {
    ...
    if (e.target instanceof Element && e.target.closest('#crt-structured-composer')) {
      return;
    }
