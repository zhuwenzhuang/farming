...
declare const CrtBilling: {
  init(ports: {
    isActiveView(): boolean;
    setMainView(view: string): void;
    renderShellState(): void;
    setNavigationSelection(element: HTMLElement | null): void;
    clearNavigationSelection(): void;
    apiPath(path: string): string;
    formatTokenRate(value: string | number | null | undefined): string;
  }): void;
  show(): void;
  hide(): void;
  leave(): void;
  suspend(): void;
  resume(isBillingView: boolean): void;
  resize(): void;
  dispose(): void;
  selectMode(mode: string): void;
  refresh(): void;
  selectDay(dateValue: string, options?: { focus?: boolean }): boolean;
  selectDayByArrow(key: DirectionKey): boolean;
  isHourCellFocused(): boolean;
  startTopBarRefresh(options?: { immediate?: boolean }): void;
...
const CRT_TERMINAL_CHECKPOINT_SCROLLBACK_STEPS = [200, 500, 1_000, 2_000, 5_000] as const;
...
function getCrtNavigationScope() {
  ...
  if (aboutModal && aboutModal.classList.contains('active')) return aboutModal;
...
function requestCrtTerminalReplay(requestedScrollback = 0) {
  const replication = crtTerminalReplication;
  const installedScrollback = Math.max(
    replication?.checkpointScrollbackInstalled || 0,
    Number(terminal?.buffer?.active?.baseY) || 0,
  );
  if (replication) {
    replication.checkpointScrollbackDesired = Math.min(
      TERMINAL_SCROLLBACK,
      Math.max(
        CRT_TERMINAL_CHECKPOINT_SCROLLBACK_STEPS[0],
        replication.checkpointScrollbackDesired,
        requestedScrollback,
        installedScrollback,
      ),
    );
...
function expandCrtTerminalCheckpointHistory() {
  ...
  const installed = Math.max(
    ...
    Number(buffer.baseY) || 0,
  );
  if (
    installed >= TERMINAL_SCROLLBACK ||
  ...
  replication.checkpointScrollbackDesired = Math.min(
    nextStep,
    replication.checkpointScrollbackAvailable,
  );
...
function finishCrtTerminalReplay(replication = crtTerminalReplication) {
  ...
  if (
    replication.checkpointScrollbackDesired > replication.checkpointScrollbackInstalled &&
...
function performCrtTerminalCheckpointInstall(
  ...
  const finishInstall = () => {
    ...
    refreshSessionTerminalUi({ preserveSearchIndex: true });
    replication.checkpointScrollbackInstalled = Math.max(
      Number(terminal?.buffer?.active?.baseY) || 0,
      sessionView.renderedScrollback,
    );
    replication.checkpointScrollbackAvailable = Math.max(
      replication.checkpointScrollbackInstalled,
      sessionView.scrollbackAvailable,
    );
...
function installCrtTerminalCheckpoint(
  ...
  if (
    ...
    replication.checkpointScrollbackInstalled = Math.max(
      replication.checkpointScrollbackInstalled,
      Number(terminal?.buffer?.active?.baseY) || 0,
      validatedSessionView.renderedScrollback,
    );
    replication.checkpointScrollbackAvailable = Math.max(
      replication.checkpointScrollbackInstalled,
      validatedSessionView.scrollbackAvailable,
    );
...
let crtAboutReturnFocus: HTMLElement | null = null;
...
function setCrtMainView(view: string) {
  ...
  if (previousView === 'billing' && crtMainView !== 'billing') CrtBilling.leave();
...
function resumeCrtPageConnection() {
  ...
  connect();
  CrtBilling.resume(crtMainView === 'billing');
}
...
if (typeof document !== 'undefined') {
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (
      ...
      CrtBilling.selectDayByArrow(e.key);
      e.preventDefault();
      return;
    }
...
if (typeof module !== 'undefined' && module.exports) {
  ...
  CrtBilling.init({
    isActiveView: () => crtMainView === 'billing',
    setMainView: (view: string) => setCrtMainView(view),
    renderShellState: () => renderState(),
    setNavigationSelection: (el: HTMLElement | null) => setCrtNavigationSelection(el as CrtDomElement | null),
    clearNavigationSelection: () => clearCrtNavigationSelection(),
    apiPath: (path: string) => farmingApiPath(path),
    formatTokenRate: (value: string | number | null | undefined) => formatCrtTokenRate(value),
  });
  CrtBilling.startTopBarRefresh();
  connect();
}
