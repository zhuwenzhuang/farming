...
(function attachTerminalAttachmentCoordinator(global) {
    ...
    class TerminalAttachmentCoordinator {
        #replay;
        #replayState;
        #generation = 0;
        #revision = 0;
        ...
        admitCheckpointInstall(operation, checkpoint) {
            if (!this.isCurrentOperation(operation))
                return false;
            return this.#replay.evaluateCheckpoint(this.#replayState, checkpoint).action !== 'reject';
        }
        commitCheckpoint(operation, checkpoint) {
            if (!this.admitCheckpointInstall(operation, checkpoint))
                return false;
        ...
        recordInvariantFailure(signature, message) {
        ...
        queuedOutputBatch() {
            ...
            for (const event of this.#replayState.queuedTransitions) {
                if (event.kind === 'resize')
                    break;
                if (this.#replay.classifyTransition(shadow, event).action !== 'apply')
                    return null;
