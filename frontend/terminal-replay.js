...
(function attachTerminalReplay(global) {
    ...
    function noteReplayTarget(state, event) {
        if (!event || !event.runtimeEpoch || !isFiniteNumber(event.stateRevision))
            return;
    ...
    function isReplayTargetPending(state) {
        if (!state.replayTargetEpoch || !isFiniteNumber(state.replayTargetRevision))
            return false;
        if (!state.runtimeEpoch || !isFiniteNumber(state.stateRevision))
            return true;
    ...
    function classifyTransition(state, event) {
        ...
        if (state.runtimeEpoch && event.runtimeEpoch !== state.runtimeEpoch) {
            ...
            if (relation === -1)
                return { action: 'drop', reason: 'older-epoch' };
            beginRecovery(state, event);
    ...
    function queueTransition(state, event) {
        ...
        if (state.queuedTransitions.length >= state.maxQueuedTransitions
    ...
    function takeQueuedTransition(state) {
        ...
        if (event)
            state.queuedBytes = Math.max(0, state.queuedBytes - byteLength(event.data));
        return event;
    }
    ...
    function queuedTransitionsCoverTarget(state, checkpoint) {
        ...
        for (const event of state.queuedTransitions) {
            if (!isTransitionValid(event) || event.runtimeEpoch !== checkpoint.runtimeEpoch)
                return false;
            if (event.stateRevision <= stateRevision)
                continue;
            const outputAdvance = event.kind === 'output' ? 1 : 0;
            if (event.stateRevision !== stateRevision + 1
                || event.outputSeq !== outputSeq + outputAdvance)
                return false;
            outputSeq = event.outputSeq;
            stateRevision = event.stateRevision;
            if (stateRevision >= state.replayTargetRevision)
                return true;
        }
        return false;
    }
    function evaluateCheckpoint(state, checkpoint) {
        ...
        if (state.runtimeEpoch && checkpoint.runtimeEpoch !== state.runtimeEpoch) {
            ...
            if (relation === -1 || state.retiredRuntimeEpochs.has(checkpoint.runtimeEpoch)) {
                return checkpointInvariant(`older-epoch:${checkpoint.runtimeEpoch}:${state.runtimeEpoch}`, 'Terminal replay returned an older runtime epoch');
            }
        }
        else if (checkpoint.runtimeEpoch === state.runtimeEpoch
            && isFiniteNumber(state.stateRevision)
            && checkpoint.stateRevision < state.stateRevision) {
            return checkpointInvariant(`older-revision:${checkpoint.runtimeEpoch}:${checkpoint.stateRevision}:${state.stateRevision}`, 'Terminal replay returned an older screen state');
        }
        if (state.replayTargetEpoch && isFiniteNumber(state.replayTargetRevision)) {
            if (checkpoint.runtimeEpoch === state.replayTargetEpoch) {
                if (checkpoint.stateRevision < state.replayTargetRevision
                    && !queuedTransitionsCoverTarget(state, checkpoint)) {
                    return checkpointInvariant(`behind-target:${checkpoint.runtimeEpoch}:${checkpoint.stateRevision}:${state.replayTargetRevision}`, 'Terminal replay did not reach the latest observed screen state');
                }
            }
            else if (compareRuntimeEpochs(checkpoint.runtimeEpoch, state.replayTargetEpoch) !== 1) {
                return checkpointInvariant(`wrong-target-epoch:${checkpoint.runtimeEpoch}:${state.replayTargetEpoch}`, 'Terminal replay returned a different runtime epoch');
            }
        }
    ...
    function removeCheckpointCoveredTransitions(state, checkpoint) {
        ...
        state.queuedBytes = state.queuedTransitions.reduce((total, event) => total + byteLength(event.data), 0);
    }
    ...
    function commitCheckpoint(state, checkpoint) {
        if (state.runtimeEpoch && state.runtimeEpoch !== checkpoint.runtimeEpoch) {
            ...
            while (state.retiredRuntimeEpochs.size > 4) {
                ...
                if (oldestRuntimeEpoch !== undefined)
                    state.retiredRuntimeEpochs.delete(oldestRuntimeEpoch);
            }
        }
