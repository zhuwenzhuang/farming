 ...
 * on one side and the Farming Chrome extension's chrome.debugger transport on
...
import { resolveCreateTargetParams } from "./create-target-params.cjs";
import {
...
const BROWSER_TARGET_ID = "farming-extension-relay";
/** Playwright requires every attached page target to identify its browser context. */
const BROWSER_CONTEXT_ID = "farming-extension-context";
...
export class ExtensionRelayBridge {
  ...
  private sendToExtension(msg: RelayToExtensionMessage): void {
    if (!this.extension) {
      throw new Error("Farming Chrome extension is not connected to the relay");
    }
  ...
  private async ensureTabAttached(tabId: number): Promise<{ targetId: string; sessionId: string }> {
    ...
    if (!tab) {
      throw new Error(`tab ${tabId} is not available to Farming`);
    }
    if (tab.attached) {
      return tab.attached;
    }
    ...
    const attaching = (async () => {
      ...
      const sessionId = `farming-tab-${tabId}-${this.nextSessionOrdinal++}`;
   ...
   * "Farming is debugging this browser" infobar only spans active automation.
   */
    ...
    switch (request.method) {
      case "Browser.getVersion": {
        ...
        this.respond(client, request, {
          ...
          revision: "farming-extension-relay",
      ...
      case "Target.attachToBrowserTarget": {
        const sessionId = `farming-browser-${this.nextSessionOrdinal++}`;
      ...
      case "Target.attachToTarget": {
        ...
        if (request.sessionId && this.browserSessions.get(request.sessionId) === client) {
          ...
          const sessionId = `farming-tab-${found.tabId}-${this.nextSessionOrdinal++}`;
      ...
      case "Target.createBrowserContext": {
        this.respondError(
          client,
          request,
          "The Farming extension relay drives the user's real browser profile; isolated browser contexts are not supported.",
        );
        return;
      }
      default: {
