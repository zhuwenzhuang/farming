...
test.describe('display-backed agent flows', () => {
  ...
  test('shows a compact product version without raw git suffixes', async ({ page }) => {
    ...
    const documentationLink = brandDialog.getByRole('link', { name: 'Documentation' })
    await expect(documentationLink).toHaveAttribute('href', 'https://zhuwenzhuang.github.io/farming/en/')
    await expect(documentationLink).toHaveCSS('color', 'rgb(63, 76, 104)')
    ...
    await expect(documentationLink).toHaveCSS('color', 'rgb(140, 188, 255)')
    ...
    await settingsPanel.getByRole('group', { name: 'Language' }).getByRole('button', { name: '中文', exact: true }).click()
    await settingsPanel.getByRole('button', { name: '关闭' }).click()
    await productMark.click()
    const chineseBrandDialog = page.getByTestId('code-brand-dialog')
    await expect(chineseBrandDialog.getByRole('link', { name: '文档' })).toHaveAttribute('href', 'https://zhuwenzhuang.github.io/farming/cn/')
    await chineseBrandDialog.getByRole('button', { name: '关闭' }).click()
    await page.getByTestId('code-sidebar-options').click()
    const chineseSettingsPanel = page.getByTestId('code-settings-panel')
    await chineseSettingsPanel.getByRole('group', { name: '语言' }).getByRole('button', { name: 'English', exact: true }).click()
    await chineseSettingsPanel.getByRole('button', { name: 'Close' }).click()
  ...
  test('renders empty workspace guidance and routes its actions', async ({ page, workspaceRoot }) => {
    ...
    await page.getByTestId('code-plugins-panel').getByRole('button', { name: /Back|返回/ }).click()
    await expect(emptyWorkspace.getByTestId('code-empty-home-plugins')).toBeVisible()
  ...
  test('opens project changes into the editor diff surface', async ({ page, workspaceRoot }) => {
    ...
    await expect.poll(() => new URL(trackedReview.url(), page.url()).searchParams.get('scope')).toBe('tracked')
    await expect(changesTitle).toHaveAttribute('aria-expanded', 'false')
    await trackedReview.close()
    await untrackedGroup.locator('.code-file-change-group-toggle').click()
    const untrackedReviewPromise = page.waitForEvent('popup')
    await untrackedGroup.getByRole('button', { name: 'Review untracked', exact: true }).click()
    const untrackedReview = await untrackedReviewPromise
    await expect.poll(() => new URL(untrackedReview.url(), page.url()).searchParams.get('scope')).toBe('untracked')
    ...
    const fileSearchBox = filesSection.locator('.code-file-search-box')
  ...
  test('persists every Project until Remove and mounts sibling worktrees from the Git list', async ({ page, workspaceRoot }) => {
    ...
    const projectTitleActions = project.locator('.code-project-title-actions')
    await expect(projectTitleActions).toHaveCSS('opacity', '1')
    await expect(projectTitle).toHaveCSS('border-top-left-radius', '8px')
    await expect(projectTitle).toHaveCSS('border-bottom-left-radius', '8px')
    await expect(projectTitleActions).toHaveCSS('border-top-right-radius', '8px')
    await expect(projectTitleActions).toHaveCSS('border-bottom-right-radius', '8px')
    await projectWorktree.hover()
    expect(await projectTitleActions.evaluate(element => getComputedStyle(element).opacity)).toBe('0')
    await expect.poll(() => projectTitle.evaluate(element => getComputedStyle(element).backgroundColor))
      .toBe(idleProjectBackground)
    await expect(projectTitleActions).toHaveCSS('opacity', '0')
    await projectTitle.hover()
  ...
  test('keeps project files as a collapsible project-level section', async ({ page }) => {
    ...
    await expect(page.getByTestId('code-file-monaco')).toBeVisible()
    await expect(page.getByTestId('code-file-large-text-alert')).toHaveText('This large file is shown completely in read-only mode.')
  ...
  test('renders launch, Project, Files, and active Session controls', async ({ page, workspaceRoot }) => {
    ...
    await expect(activeCodexSessionRow.getByTestId('code-agent-row-pin')).toBeVisible()
    await expect(activeCodexSessionRow.getByTestId('code-agent-row-archive')).toBeVisible()
  ...
  test('keeps desktop Agent menus and terminal viewport state coherent', async ({ page, workspaceRoot }) => {
    ...
    await expect(page.getByRole('menuitem', { name: 'Continue in new Agent' })).toBeVisible()
    await expect(page.getByRole('menuitem', { name: 'Continue in new worktree' })).toBeVisible()
