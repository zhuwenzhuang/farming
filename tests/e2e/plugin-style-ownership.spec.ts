...
test('Plugin style owners preserve the light, dark, and narrow runtime cascade', async ({ page }) => {
  ...
  await expect(tabs).toHaveCSS('background-color', 'rgb(240, 242, 239)')
  await expect(selectedTab).toHaveCSS('background-color', 'rgba(9, 105, 218, 0.1)')
  await expect(firstCard).toHaveCSS('background-color', 'rgb(240, 242, 239)')

  await page.locator('body').evaluate(body => { body.dataset.appearance = 'dark' })
  await expect(tabs).toHaveCSS('background-color', 'rgb(34, 34, 34)')
  await expect(selectedTab).toHaveCSS('background-color', 'rgba(51, 156, 255, 0.18)')
  await expect(firstCard).toHaveCSS('background-color', 'rgb(34, 34, 34)')

  await page.locator('body').evaluate(body => { body.dataset.appearance = 'paper' })
  await panel.getByTestId('code-plugin-tab-farming').click()
  await expect(selectedTab).toHaveCSS('background-color', 'rgba(40, 41, 34, 0.09)')
  await expect(selectedTab).toHaveCSS('color', 'rgb(35, 37, 31)')
  const connectorButton = panel.getByRole('button', { name: 'Prepare Chrome extension folder' })
  await expect(connectorButton).toBeVisible()
  await expect(connectorButton).toHaveCSS('background-color', 'rgb(249, 248, 244)')
  await expect(connectorButton).toHaveCSS('border-width', '0px')
  await expect(connectorButton).toHaveCSS('color', 'rgb(40, 41, 34)')
