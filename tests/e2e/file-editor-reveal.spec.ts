...
test('editor reveal expands and relocates the current file without rereading or replacing its draft', async ({ page, workspaceRoot, isMobile }, testInfo) => {
  ...
  const visibleRevealControl = isMobile ? more : reveal
  const revealCurrentFile = async () => {
    ...
    await more.click()
    const menuReveal = page.getByTestId('code-file-editor-more-reveal')
    await expect(menuReveal).toHaveAccessibleName(`Reveal ${filePath} in Explorer`)
    await menuReveal.click()
  }
  if (isMobile) {
  ...
  for (const appearance of ['light', 'dark', 'paper']) {
    ...
    await closeSidebar(page)
    await expect(visibleRevealControl).toBeVisible()
    expect((await visibleRevealControl.boundingBox())!.width).toBe(isMobile ? 44 : 22)
    expect((await visibleRevealControl.locator('svg').boundingBox())!.width).toBe(16)
...
test('exact external files do not offer a Project-tree reveal action', async ({ page, workspaceRoot, isMobile }) => {
  ...
  if (isMobile) {
    await expect(editor.getByTestId('code-file-editor-share')).toBeHidden()
    await editor.getByTestId('code-file-editor-more').click()
    await expect(page.getByTestId('code-file-editor-more-share')).toBeVisible()
    await expect(page.getByTestId('code-file-editor-more-reveal')).toHaveCount(0)
  } else {
