...
test.describe('ACP human-like browser matrix', () => {
  ...
  test('keeps 53 structured chat interactions coherent across live, history, security, and runtime switching', async ({ page, workspaceRoot }) => {
    ...
    await test.step('33 keep an intermediate failed tool out of the collapsed summary', async () => {
      ...
      await expect(failedTurn.getByTestId('code-codex-transcript-process-compact-list')).toHaveCount(0)
      ...
      const failedGroup = failedTurn.getByTestId('code-codex-transcript-process-group')
      await expect(failedGroup.getByTestId('code-codex-transcript-process-group-toggle')).toContainText('Action failed')
      await failedGroup.getByTestId('code-codex-transcript-process-group-toggle').click()
      ...
      await expect(failedItem.locator('.code-codex-transcript-process-status')).toHaveText('failed')
    })
    ...
    await test.step('43 classify a runtime failure without hiding the transcript', async () => {
      ...
      await expect(errorTurn.getByTestId('code-codex-transcript-process-item')).toHaveCount(0)
  ...
  test('keeps a manually closed terminal detail closed when the running command fails', async ({ page, workspaceRoot }) => {
    ...
    const processSummary = turn.getByTestId('code-codex-transcript-process-summary')
    await expect(processSummary).toHaveAttribute('aria-expanded', 'false')
    await expect(turn.getByTestId('code-codex-transcript-process-item')).toHaveCount(0)
    await processSummary.click()
    const failedGroup = turn.getByTestId('code-codex-transcript-process-group')
    await failedGroup.getByTestId('code-codex-transcript-process-group-toggle').click()
  ...
  test('keeps a dense multi-step turn compact until the user opens its evidence', async ({ page, workspaceRoot }) => {
    ...
    await expect(turn.getByTestId('code-codex-transcript-process-earlier')).toHaveText('16 earlier actions')
