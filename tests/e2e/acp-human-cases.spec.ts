...
test('side chat keeps independent drafts and reopens the same child', async ({ page, workspaceRoot }, testInfo) => {
  await page.setViewportSize({ width: 1600, height: 1000 })
  const workspace = path.join(workspaceRoot, 'side-chat')
  fs.mkdirSync(workspace, { recursive: true })
  const parentId = await createCodexAcpAgent(page, workspace)
  await openFarming(page)
  await agentRow(page, parentId).click()
  await sendAcpMessage(page, 'image attachment side chat parent context')
  await expect(page.getByTestId('code-acp-composer-send')).toBeDisabled()
  const parentInput = page.getByTestId('code-acp-composer-input')
  await parentInput.fill('retained parent draft')
  const opened = page.waitForResponse(response => response.url().endsWith(`/agents/${parentId}/side-chat`))
  ...
  const pane = page.getByTestId('code-side-chat-panel')
  ...
  const reopened = page.waitForResponse(response => response.url().endsWith(`/agents/${parentId}/side-chat`))
  await page.getByRole('button', { name: 'Side chat', exact: true }).click()
  expect((await (await reopened).json()).agentId).toBe(child.agentId)
  await expect(childInput).toHaveValue('retained child draft')
  await childInput.fill('image attachment side chat independent reply')
  await pane.getByTestId('code-acp-composer-send').click()
  await expect(childInput).toHaveValue('')
  await expect(pane).toContainText('image attachment side chat independent reply')
  await expect(page.locator('.code-composer-shell').getByTestId('code-acp-composer-input')).toHaveValue('retained parent draft')
  await pane.getByRole('button', { name: 'Collapse related session' }).click()
  ...
  await page.getByRole('navigation', { name: 'Related sessions' }).getByRole('button', { name: 'Side chat', exact: true }).click()
  await expect(childInput).toBeVisible()
  await page.setViewportSize({ width: 1600, height: 1000 })
  ...
  for (const appearance of ['light', 'dark', 'paper']) {
    await page.locator('body').evaluate((body, value) => { body.dataset.appearance = value }, appearance)
    await page.screenshot({ path: testInfo.outputPath(`side-chat-${appearance}.png`), animations: 'disabled' })
  }
  ...
  const orphanSend = await page.request.post(`/farming/api/control/agents/${child.agentId}/messages`, {
    data: { message: 'Must not run after parent removal', requestId: 'side-parent-removed-test' },
  })
