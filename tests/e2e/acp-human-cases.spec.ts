...
test.describe('ACP human-like browser matrix', () => {
  ...
  test('shows readable Codex collaboration summaries and keeps raw Process evidence secondary', async ({ page, workspaceRoot }) => {
    ...
    await expect(followUpProcessSummary).toHaveAttribute('aria-expanded', 'true')
  ...
  test('keeps 53 structured chat interactions coherent across live, history, security, and runtime switching', async ({ page, workspaceRoot }) => {
    ...
    await test.step('31 keep the completed subagent action collapsed', async () => {
      await expect(subagentTurn.getByTestId('code-agent-transcript-process-summary')).toHaveAttribute('aria-expanded', 'true')
    ...
    await test.step('33 keep an intermediate failed tool out of the collapsed summary', async () => {
      ...
      await expect(failedTurn.getByTestId('code-agent-transcript-process-summary')).toHaveAttribute('aria-expanded', 'true')
    ...
    await test.step('37 show restrained live progress while the Agent works', async () => {
      ...
      await expect(liveGroup.getByTestId('code-agent-transcript-process-group-toggle')).toHaveAttribute('aria-expanded', 'false')
      await expect(liveTurn.getByTestId('code-agent-transcript-process-item')).toHaveCount(0)
      await sendPromise
    ...
    await test.step('41 keep client filesystem and terminal results in the ordered turn', async () => {
      ...
      await expect(clientTurn.getByTestId('code-agent-transcript-process-summary')).toHaveAttribute('aria-expanded', 'true')
    ...
    await test.step('43 classify a runtime failure without hiding the transcript', async () => {
      ...
      await expect(errorSummary).toHaveAttribute('aria-expanded', 'true')
    ...
    await test.step('46 expose a running client terminal as an ordered tool item', async () => {
      ...
      await expect(longTurn.getByTestId('code-agent-transcript-process-summary')).toHaveAttribute('aria-expanded', 'true')
