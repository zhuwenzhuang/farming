...
async function selectAssistantText(page: Page) {
  const assistant = page.locator('.code-terminal-grid .code-agent-transcript-assistant').last()
  await expect(assistant).toBeVisible()
  const text = await assistant.evaluate(element => {
  ...
  await expect(page.getByTestId('code-agent-transcript-selection-actions')).toBeVisible()
  return text
}
...
test.describe('ACP human-like browser matrix', () => {
  test('shows immediate sending feedback during slow admission without duplicating or losing drafts', { tag: '@iphone-human' }, async ({ page, workspaceRoot, isMobile }, testInfo) => {
    const agentId = await createAcpAgent(page, workspaceRoot)
    let submitted = 0
    ...
    for (const appearance of ['light', 'dark', 'paper']) {
      ...
      await page.getByTestId('code-acp-composer').screenshot({ path: testInfo.outputPath(`sending-${appearance}.png`), animations: 'disabled' })
    }
  ...
  for (const location of ['session', 'workspace', 'document', 'wide']) {
    test(`renders a Codex host visualization from ${location} directly inside the Chat result`, async ({ page, workspaceRoot }) => {
      ...
      await sendAcpMessage(page, location === 'workspace' ? 'workspace inline visualization' : location === 'document' ? 'inline visualization complete document' : location === 'wide' ? 'wide inline visualization' : 'inline visualization')
      ...
      await expect.poll(() => visualization.evaluate(el => el.getBoundingClientRect().width)).toBeLessThanOrEqual(location === 'wide' ? 1024 : 640)
      if (location === 'wide') await expect.poll(() => visualization.evaluate(el => el.getBoundingClientRect().width)).toBeGreaterThan(640)
      ...
      await expect(frame.getByText('Local JSON loaded / module loaded')).toBeVisible()
      await expect(frame.locator('.local-proof')).toHaveCSS('border-bottom-width', '3px')
      if (location === 'document') await expect(frame.locator('body')).toHaveCSS('font-family', 'Georgia, serif')
      else await expect(frame.locator('svg.lucide')).toBeVisible()
      await expect.poll(() => visualization.locator('iframe').evaluate(el => el.clientHeight)).toBeGreaterThan(480)
      const initialHeight = await visualization.locator('iframe').evaluate(el => el.clientHeight)
      await frame.getByRole('button', { name: 'Resize chart' }).click()
      await expect.poll(() => visualization.locator('iframe').evaluate(el => el.clientHeight)).toBeGreaterThan(initialHeight + 200)
      await frame.getByRole('button', { name: 'Resize chart' }).click()
      await expect.poll(() => visualization.locator('iframe').evaluate(el => el.clientHeight)).toBeLessThan(initialHeight + 10)
      await visualization.evaluate(el => { el.style.transform = 'translateY(5000px)' })
      await frame.locator('#chart-area').evaluate(el => { el.style.height = '700px' })
      await expect.poll(() => visualization.locator('iframe').evaluate(el => el.clientHeight)).toBeGreaterThan(initialHeight + 200)
      await frame.locator('#chart-area').evaluate(el => { el.style.height = '480px' })
      await expect.poll(() => visualization.locator('iframe').evaluate(el => el.clientHeight)).toBeLessThan(initialHeight + 10)
      await visualization.evaluate(el => { el.style.transform = '' })
      ...
      await visualization.getByRole('button', { name: 'Enlarge visualization' }).click()
      await expect(visualization.getByRole('button', { name: 'Close enlarged visualization' })).toBeVisible()
      await expect.poll(() => visualization.evaluate(el => el.getBoundingClientRect().width)).toBe(1440)
      await expect(frame.locator('body')).toHaveAttribute('data-view', 'cu')
      await visualization.getByRole('button', { name: 'Close enlarged visualization' }).click()
      await expect(frame.locator('body')).toHaveAttribute('data-view', 'cu')
      for (const appearance of ['light', 'dark', 'paper'] as const) {
        await page.locator('body').evaluate((body, value) => { body.dataset.appearance = value }, appearance)
        await expect.poll(() => frame.locator('body').evaluate(el => getComputedStyle(el).color)).toBe(location === 'document' ? 'rgb(52, 86, 120)' : await visualization.evaluate(el => getComputedStyle(el).color))
        await expect(frame.locator('body')).toHaveAttribute('data-view', 'cu')
      ...
      await expect.poll(() => frame.locator('body').evaluate(el => el.scrollWidth - el.clientWidth)).toBeLessThan(3)
      for (const appearance of ['light', 'dark', 'paper'] as const) {
        await page.locator('body').evaluate((body, value) => { body.dataset.appearance = value }, appearance)
        await visualization.scrollIntoViewIfNeeded()
        await visualization.screenshot({ path: test.info().outputPath(`inline-narrow-${location}-${appearance}.png`) })
      }
      await expect.poll(() => frame.locator('body').evaluate(el => el.scrollHeight - innerHeight)).toBeLessThan(3)
      // A browser resize scheduling notification must not poison a healthy
      // preview; genuine authored exceptions still produce visible diagnostics.
      await frame.locator('body').evaluate(() => window.dispatchEvent(new ErrorEvent('error', { message: 'ResizeObserver loop completed with undelivered notifications.', filename: 'about:srcdoc' })))
      await expect(visualization.getByRole('status')).toHaveCount(0)
      await frame.locator('body').evaluate(() => window.dispatchEvent(new ErrorEvent('error', { message: 'Chart calculation failed', error: new Error('Chart calculation failed') })))
      await expect(visualization.getByRole('status')).toHaveText('Chart calculation failed')
      await page.reload()
  ...
  test('renders standard Markdown images through authorized paths and preserves remote sources', { tag: '@iphone-human' }, async ({ page, workspaceRoot }) => {
    ...
    for (const appearance of ['light', 'dark', 'paper'] as const) {
      ...
      await trigger.click()
      const viewer = page.getByTestId('code-agent-transcript-image-overlay')
      await expect(viewer).toBeVisible()
      await expect.poll(() => viewer.locator('img').evaluate((img: HTMLImageElement) => img.complete && img.naturalWidth > 1)).toBe(true)
      await expect(page.locator('#root')).toHaveAttribute('inert', '')
      await expect(viewer.getByRole('button', { name: 'Close image preview' })).toBeFocused()
      await page.screenshot({ path: test.info().outputPath(`image-fullscreen-${appearance}.png`) })
  ...
  test('shows chronological Agent activity without duplicating the sidebar inventory', { tag: ['@iphone-human'] }, async ({ page, workspaceRoot, isMobile }, testInfo) => {
    ...
    await expect(timeline.getByTestId('code-agent-transcript-collaboration-group')).toHaveCount(0)
    await expect(timeline.getByTestId('code-collaboration-finished')).toHaveCount(0)
    const earlier = timeline.getByTestId('code-agent-transcript-collaboration-activities-earlier')
    for (let i = 0; i < 5 && await earlier.count(); i++) await earlier.click()
    const events = timeline.getByTestId('code-agent-transcript-collaboration-event')
    expect(await events.count()).toBeGreaterThan(3)
    await expect(timeline).toContainText('Review refresh')
    await expect(timeline).toContainText('Browser guards')
    await events.first().click()
    await expect(events.first()).toHaveAttribute('aria-expanded', 'true')
    await expect(timeline.getByTestId('code-agent-transcript-collaboration-evidence')).toBeVisible()
    const detailsButton = timeline.getByTestId('code-collaboration-open-details')
    await expect(detailsButton).toBeVisible()
    expect(await detailsButton.evaluate(element => element.scrollWidth <= element.clientWidth)).toBe(true)
    const detailsBounds = await detailsButton.boundingBox()
    const evidenceBounds = await timeline.locator('.code-agent-transcript-collaboration-evidence .code-agent-transcript-process-item').first().boundingBox()
    expect(detailsBounds).not.toBeNull()
    expect(evidenceBounds).not.toBeNull()
    expect(detailsBounds!.y + detailsBounds!.height).toBeLessThanOrEqual(evidenceBounds!.y)
    await detailsButton.scrollIntoViewIfNeeded()
    const originalEvents = await events.allTextContents()
    for (const appearance of ['light', 'dark', 'paper']) {
      await page.locator('body').evaluate((element, value) => { element.dataset.appearance = value }, appearance)
      await page.screenshot({ path: testInfo.outputPath(`chat-activity-${appearance}.png`), animations: 'disabled' })
    }
    await sendAcpMessage(page, 'collaboration follow up')
    const nextTurn = page.locator('.code-agent-transcript-turn').filter({ hasText: 'Cross-turn collaboration update complete.' })
    await expect(nextTurn).toBeVisible()
    await nextTurn.getByTestId('code-agent-transcript-process-summary').click()
    const nextFeed = nextTurn.getByTestId('code-agent-transcript-collaboration')
    await expect(nextFeed).toContainText('Cross turn child')
    await expect(nextFeed).not.toContainText('Browser guards')
    await expect(nextFeed.getByTestId('code-agent-transcript-collaboration-group')).toHaveCount(0)
    expect(await events.allTextContents()).toEqual(originalEvents)
    await expect(events.first()).toHaveAttribute('aria-expanded', 'true')
    await sendAcpMessage(page, 'image attachment ordinary follow up')
    const ordinary = page.locator('.code-agent-transcript-turn').filter({ hasText: 'image attachment ordinary follow up' })
    await expect(ordinary).toContainText('Received 0 image.')
    await expect(ordinary.getByTestId('code-agent-transcript-collaboration')).toHaveCount(0)
  })

  test('distinguishes same-prefix subagents in the activity feed', { tag: ['@iphone-human'] }, async ({ page, workspaceRoot, isMobile }, testInfo) => {
    ...
    const turn = page.locator('.code-agent-transcript-turn').filter({ hasText: 'Same-prefix collaboration example complete.' })
    await expect(turn).toBeVisible()
    await turn.getByTestId('code-agent-transcript-process-summary').click()
    const feed = turn.getByTestId('code-agent-transcript-collaboration')
    for (const [name, task] of [['Agent 1b995a', 'Audit mobile responsive behavior'], ['Agent aef497', 'Review release notes'], ['Agent 6c0c7d', 'Inspect CI failures']]) {
      const event = feed.getByTestId('code-agent-transcript-collaboration-event').filter({ hasText: name })
      await expect(event).toContainText(task)
      await expect(event).not.toContainText(/Preparing|准备中/)
    }
    for (const appearance of ['light', 'dark', 'paper']) {
      await page.locator('body').evaluate((element, value) => { element.dataset.appearance = value }, appearance)
      await page.screenshot({ path: testInfo.outputPath(`same-prefix-activity-${appearance}.png`), animations: 'disabled' })
    }
  })

  test('keeps explicitly opened event evidence stable while Agent activity streams', { tag: ['@iphone-human'] }, async ({ page, workspaceRoot, isMobile }) => {
    ...
    const feed = turn.getByTestId('code-agent-transcript-collaboration')
    await turn.getByTestId('code-agent-transcript-process-summary').click()
    const started = feed.getByTestId('code-agent-transcript-collaboration-event').filter({ hasText: 'Live reviewer' }).first()
    await expect(started).toBeVisible()
    await started.click()
    const firstId = await started.getAttribute('data-process-item-id')
    const stable = feed.locator(`button[data-process-item-id="${firstId}"]`)
    await expect(stable).toHaveAttribute('aria-expanded', 'true')
    await expect(page.getByText('Live collaboration demo complete.', { exact: true })).toBeVisible({ timeout: 15_000 })
    await expect(stable).toHaveAttribute('aria-expanded', 'true')
    await expect(feed).toContainText('Live review completed.')
    await expect(feed.getByTestId('code-collaboration-finished')).toHaveCount(0)
  })
  ...
  async function exerciseHumanCaseSection(page: Page, workspaceRoot: string, section: HumanCaseSection) {
    ...
    await test.step('32b open the child transcript without leaving the parent chat', async () => {
      ...
      await expect(panel).toContainText('The parser is consistent.')
      const closeDetails = panel.getByRole('button', { name: 'Collapse related session' })
    ...
    await test.step('33 keep an intermediate failed tool out of the collapsed summary', async () => {
      ...
      await expect(failedGroup.getByTestId('code-agent-transcript-process-group-toggle')).toContainText('Ran a command')
      await expect(failedGroup.getByTestId('code-agent-transcript-process-group-toggle')).not.toContainText('Action failed')
  ...
  for (const appearance of ['light', 'dark', 'paper'] as const) {
    test(`keeps streaming thought folded until its segment and detail are explicitly opened in ${appearance}`, async ({ page, workspaceRoot }) => {
      test.setTimeout(60_000)
      const workspace = path.join(workspaceRoot, `acp-streaming-thought-${appearance}`)
      fs.mkdirSync(workspace, { recursive: true })
      const agentId = await createAcpAgent(page, workspace)
      await openFarming(page)
      ...
      await page.screenshot({ path: test.info().outputPath(`thought-hover-${appearance}.png`) })
      await page.mouse.move(0, 0)
      ...
      await expect(thoughtChevron).toHaveCSS('opacity', '0.9')
      await page.screenshot({ path: test.info().outputPath(`thought-focus-${appearance}.png`) })
  ...
  test('opens and stops a live ACP subagent without leaving the parent chat', async ({ page, workspaceRoot }) => {
    ...
    await expect(preview.getByTestId('code-acp-subagent-fullscreen')).toBeFocused()
    await page.getByTestId('code-native-related-row').click()
    await expect(panel).toContainText('Checking the first candidate')
    await panel.getByTestId('code-acp-subagent-stop').click()
    await panel.getByRole('button', { name: 'Collapse related session' }).click()
  ...
  test('reviews ACP file changes without patch decision controls', async ({ page, workspaceRoot }) => {
    ...
    const reviewTrigger = turn.getByRole('button', { name: /^Review:/ })
    await reviewTrigger.click()
    const reviewDialog = page.getByRole('dialog', { name: 'Review' })
    const review = reviewDialog.getByTestId('code-agent-transcript-result-details')
    await expect(review).toBeVisible()
    const closeReview = reviewDialog.getByRole('button', { name: 'Close' })
...
test('subagent keeps independent drafts and reopens the same child', async ({ page, workspaceRoot }, testInfo) => {
  await page.setViewportSize({ width: 1600, height: 1000 })
  const workspace = path.join(workspaceRoot, 'subagent')
  fs.mkdirSync(workspace, { recursive: true })
  ...
  const selectedText = await selectAssistantText(page)
  await page.getByTestId('code-agent-transcript-selection-actions').getByRole('button', { name: 'Quote in chat' }).click()
  await expect.poll(() => parentInput.inputValue()).toContain(`> ${selectedText.split('\n')[0]}`)
  await parentInput.fill('')
  await parentInput.fill('/si')
  await expect(page.getByTestId('code-acp-command-side')).toBeVisible()
  await page.getByTestId('code-acp-command-side').click()
  await expect(parentInput).toHaveValue('/side ')
  ...
  expect(child.agentId).not.toBe(parentId)
  const pane = page.getByTestId('code-subagent-panel')
  await expect(pane).toBeVisible()
  const parentPane = page.locator('.code-terminal-grid')
  const [parentBox, subagentBox] = await Promise.all([parentPane.boundingBox(), pane.boundingBox()])
  expect(parentBox).not.toBeNull()
  expect(subagentBox).not.toBeNull()
  expect(Math.abs((parentBox?.width || 0) - (subagentBox?.width || 0))).toBeLessThanOrEqual(2)
  await expect(parentInput).toHaveValue('')
  await expect(page.locator('.code-agent-transcript-user', { hasText: '/side' })).toHaveCount(0)
  await parentInput.fill('retained parent draft')
  ...
  const quoteInSubagent = page.waitForResponse(response => response.url().endsWith(`/agents/${parentId}/subagent`))
  await page.getByTestId('code-agent-transcript-selection-actions').getByRole('button', { name: 'Ask in subagent' }).click()
  expect((await (await quoteInSubagent).json()).agentId).toBe(child.agentId)
  await expect.poll(() => childInput.inputValue()).toContain(`> ${selectedText.split('\n')[0]}`)
  await childInput.fill('retained child draft')
  await expect(page.locator('.code-composer-shell').getByTestId('code-acp-composer-input')).toHaveValue('retained parent draft')
  await pane.getByRole('button', { name: 'Collapse related session' }).click()
  await expect(pane).toHaveCount(0)
  const parentAgentRow = agentRow(page, parentId)
  await parentAgentRow.hover()
  const subagentRowButton = parentAgentRow.getByTestId('code-agent-row-subagent')
  await expect(subagentRowButton).toBeVisible()
  const reopened = page.waitForResponse(response => response.url().endsWith(`/agents/${parentId}/subagent`))
  await subagentRowButton.click()
  expect((await (await reopened).json()).agentId).toBe(child.agentId)
  await expect(childInput).toHaveValue('retained child draft')
  await childInput.fill('image attachment subagent independent reply')
  await pane.getByTestId('code-acp-composer-send').click()
  await expect(childInput).toHaveValue('')
  await expect(pane).toContainText('image attachment subagent independent reply')
  await expect(page.locator('.code-composer-shell').getByTestId('code-acp-composer-input')).toHaveValue('retained parent draft')
  await pane.getByRole('button', { name: 'Collapse related session' }).click()
  ...
  await expect(page.locator('.code-composer-shell').getByTestId('code-acp-composer-input')).toBeVisible()
  await page.getByRole('navigation', { name: 'Related sessions' }).getByRole('button', { name: 'Subagent', exact: true }).click()
  await expect(childInput).toBeVisible()
  await page.setViewportSize({ width: 1600, height: 1000 })
  await expect(pane).toContainText('Received 0 image.')
  await expect(agentRow(page, child.agentId)).toHaveClass(/related-child/)
  await expect(agentRow(page, child.agentId)).toHaveClass(/active/)
  await expect(agentRow(page, parentId)).not.toHaveClass(/active/)
  const resizer = page.getByRole('separator', { name: 'Resize related session' })
  await resizer.focus()
  const originalWidth = Number(await resizer.getAttribute('aria-valuenow'))
  await page.keyboard.press('ArrowRight')
  expect(Number(await resizer.getAttribute('aria-valuenow'))).toBeGreaterThan(originalWidth)
  for (const appearance of ['light', 'dark', 'paper']) {
    await page.locator('body').evaluate((body, value) => { body.dataset.appearance = value }, appearance)
    await page.screenshot({ path: testInfo.outputPath(`subagent-${appearance}.png`), animations: 'disabled' })
  }
  const stopped = await page.request.delete(`/farming/api/control/agents/${parentId}`)
  expect(stopped.ok()).toBeTruthy()
  await expect.poll(async () => {
    const response = await page.request.get(`/farming/api/agents/${child.agentId}/acp-session`)
    return response.ok() ? (await response.json()).session?.state : ''
  }).toBe('closed')
  const orphanSend = await page.request.post(`/farming/api/control/agents/${child.agentId}/messages`, {
    data: { message: 'Must not run after parent removal', requestId: 'subagent-parent-removed-test' },
  })
