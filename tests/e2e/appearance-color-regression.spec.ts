...
test('Light, Dark, and Paper Chat use neutral reading surfaces and semantic glyph colors', async ({ page, workspaceRoot }, testInfo) => {
  ...
  const codeBlock = answer.locator('.code-markdown-code-block').first()
  ...
  for (const appearance of ['light', 'dark', 'paper'] as const) {
    await setAppearance(page, appearance)
    expect(await background(codeBlock)).toBe(await resolvedColor(page, '--code-code-bg'))
    ...
    const codeBlockScreenshot = await stableScreenshot(codeBlock)
    await expectScreenshotRole(page, codeBlockScreenshot, '--code-code-bg', 0.55, `${appearance} code block`)
...
test('Light, Dark, and Paper Archive notices keep neutral action colors in every interaction state', async ({ page, workspaceRoot }, testInfo) => {
  ...
  for (const appearance of appearances) {
    ...
    expect(
      ...
      `${appearance} focused Project list must preserve the Sidebar surface`,
    ).toBeGreaterThan(0.7)
    await attachScreenshot(testInfo, `archive-sidebar-focus-${appearance}`, sidebarFocusScreenshot)
