...
test('reveals more Agent row information as the sidebar widens', async ({ page, workspaceRoot }) => {
  ...
  await expect(agentPreview.locator('strong')).toHaveText(longTitle)
  await expect(agentPreview.locator('strong')).toHaveCSS('white-space', 'normal')
  await expect(agentPreview.locator('strong')).toHaveCSS('font-size', '15px')
  await expect(agentPreview.locator('strong')).toHaveCSS('font-weight', '500')
  ...
  expect(previewBox!.y + previewBox!.height).toBeLessThanOrEqual(page.viewportSize()!.height - 8)
  await page.mouse.move(1000, 100)
  await expect(agentPreview).toHaveCount(0)
  ...
  await expect(projectPreview.getByTestId('code-project-hover-preview-workspace')).toHaveCSS('font-size', '13px')
  await page.mouse.move(1000, 100)
