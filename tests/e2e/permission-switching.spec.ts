...
test.describe('permission switching', () => {
  test('restarts a fresh Codex and never falls through to another agent', async ({ page, workspaceRoot }) => {
    ...
    expect(initialCodex?.providerSessionId).toMatch(/^tmp_uuid/)
    ...
    expect(replacement?.providerSessionId).toMatch(/^tmp_uuid/)
  ...
  test('keeps an observing browser on the same agent and view', async ({ page, context, workspaceRoot }) => {
    ...
    await observerPage.getByRole('button', { name: 'Terminal' }).click()
    ...
    await expect(observerPage.getByTestId('code-agent-chat-view')).toHaveClass(/hidden/)
