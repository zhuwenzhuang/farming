...
test('shows an Agent-owned Desktop only when present and switches Viewer control epochs', async ({
  ...
  await expect(computerPlugin.getByText('Desktop in Docker (Experimental)', { exact: true })).toBeVisible()
  ...
  const desktopMenu = page.getByRole('menu').filter({ has: page.getByRole('menuitem', { name: 'Delete Desktop' }) })
  ...
  await moreDesktopActions.click()
  page.once('dialog', dialog => dialog.accept())
  await page.getByRole('menuitem', { name: 'Delete Desktop' }).click()
