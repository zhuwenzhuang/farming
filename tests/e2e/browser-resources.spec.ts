...
test('mounts Agent-owned Browsers behind nested resource controls without layout noise', async ({
  ...
  await expect(browserRow.locator('.farming-browser-status')).toHaveCount(0)
  ...
  const runningStatus = browserRow.locator('.farming-browser-status')
  ...
  expect(runningRowBox.x + runningRowBox.width - runningStatusBox.x - runningStatusBox.width).toBeLessThanOrEqual(8)
