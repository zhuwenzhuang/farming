...
test('sends negotiated Codex ACP steer with mixed input and restores it once', async ({ page, workspaceRoot }) => {
  ...
  const steer = page.getByTestId('code-codex-transcript-steer')
  await expect(steer).toContainText('focus on the attached image')
  await expect(steer.getByTestId('code-codex-transcript-user-images').locator('img')).toHaveCount(1)
  await expect(page.getByText('Steer accepted: focus on the attached image', { exact: true })).toBeVisible()
  await expect(page.locator('.code-codex-transcript-turn').filter({ hasText: 'hold for steer' })).toHaveCount(1)

  await page.reload()
  await page.locator(`[data-testid="code-agent-row"][data-agent-id="${agentId}"]`).click()
  await expect(page.getByTestId('code-codex-transcript-steer')).toHaveCount(1)
  await expect(page.getByTestId('code-codex-transcript-steer')).toContainText('focus on the attached image')
})
