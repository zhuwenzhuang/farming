...
test.describe('Farming Code appearance skins', () => {
  ...
  test('applies Paper across the browser canvas, workbench, settings, and terminal', async ({ page, workspaceRoot }) => {
    ...
    await expect(appearanceSwatches.nth(3)).toHaveCSS('background-color', 'rgb(247, 244, 237)')
    const settingsCard = settingsPanel.locator('.code-settings-card').first()
    for (const side of ['top', 'right', 'bottom', 'left'] as const) {
      await expect(settingsCard).toHaveCSS(`border-${side}-color`, 'rgb(221, 214, 200)')
    }
    await settingsPanel.getByRole('button', { name: 'Close' }).click()

    const agentId = await createControlAgent(page, 'bash', projectDir)
    await page.locator(`[data-testid="code-agent-row"][data-agent-id="${agentId}"]`).click()
    await expect(page.locator('.code-project-row').first()).toHaveCSS('background-color', 'rgb(241, 238, 231)')
  ...
  test('applies and verifies the dark Codex skin across core surfaces', async ({ page, workspaceRoot }, testInfo) => {
    ...
    await expect(page.getByTestId('code-file-editor-statusbar')).toHaveCSS('background-color', 'rgb(33, 33, 33)')
    await expectReadableMutedDarkText(page.getByTestId('code-file-editor-statusbar'), 'file editor status bar')
