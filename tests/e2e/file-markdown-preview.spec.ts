...
test('preserves diagram inspection through file refresh, repaint and terminal render errors', async ({ page, workspaceRoot }) => {
  const workspace = path.join(workspaceRoot, 'file-diagram-inspection')
  fs.mkdirSync(workspace, { recursive: true })
  const file = path.join(workspace, 'diagram.md')
  const markdown = (label: string) => `# Diagram inspection\n\n\`\`\`mermaid\nflowchart LR\n A["Query"] --> B["${label}"]\n\`\`\`\n`
  fs.writeFileSync(file, markdown('Initial output'))
  await openFarming(page)
  await openNewAgentDialog(page)
  await startAgentFromOpenDialog(page, 'bash', workspace)
  await openProjectFile(page, 'file-diagram-inspection', 'diagram.md')
  const preview = page.getByTestId('code-file-markdown-preview')
  const figure = preview.locator('.code-markdown-mermaid')
  await expect(figure).toHaveAttribute('data-render-state', 'ready')
  const trigger = figure.getByRole('button', { name: 'Open fullscreen diagram' })
  await trigger.click()
  ...
  expect(await canvas.evaluate(element => (element as HTMLElement).style.transform)).toBe(before)
  fs.writeFileSync(file, '# Diagram inspection\n\n```mermaid\ninvalid diagram\n```\n')
  await expect(viewer.locator('.code-markdown-mermaid')).toHaveAttribute('data-render-state', 'error')
  await expect(viewer.getByRole('button', { name: 'Close fullscreen diagram' })).toBeEnabled()
  fs.writeFileSync(file, markdown('Recovered output'))
  await expect(viewer.locator('.code-markdown-mermaid-canvas')).toContainText('Recovered output')
