  ...
  test('covers 31 additional desktop user-facing UI scenarios', async ({ page, workspaceRoot }) => {
    ...
    await scenario('starting bash from a fast typed workspace uses the typed value', async () => {
      ...
      await expect.poll(async () => {
        ...
        const payload = await response.json() as { agents?: Array<{ id: string; cwd?: string }> }
        return payload.agents?.find(agent => agent.id === agentId)?.cwd ?? ''
      }).toBe(projectDir)
    })
    ...
    await scenario('sidebar search filters the active project and clears cleanly', async () => {
      ...
      await expect(page.getByTestId('code-search-result-project')).toHaveCount(1)
      // File search rows keep the Project name first and append a compact
      // workspace suffix so same-name Projects stay distinguishable.
      await expect(page.getByTestId('code-search-result-project').first()).toHaveText(new RegExp(`^${path.basename(projectDir)} · `))
    ...
    await scenario('image attachment uses a preview card and can be removed cleanly', async () => {
      ...
      await expect(attachment).not.toContainText('tiny-context.png')
      const attachmentBox = await attachment.boundingBox()
      expect(attachmentBox?.width ?? 0).toBeGreaterThanOrEqual(100)
      expect(attachmentBox?.height ?? 0).toBeLessThanOrEqual(90)
