...
test('stop-all matcher keeps supported Farming roots and their descendants', () => {
  const result = runMatcher([
    ...
    psLine(112, 1, '/home/farming/.farming/glibc228/lib/ld-2.28.so --library-path /home/farming/.farming/glibc228/lib /usr/bin/node /srv/farming/dist/acp/codex-acp-1.1.14.mjs --stdio'),
...
test('stop-all matcher ignores Farming paths used only as unrelated arguments', () => {
  const result = runMatcher([
    ...
    psLine(204, 1, '/bin/echo node /repo/dist/acp/codex-acp-1.1.14.mjs'),
