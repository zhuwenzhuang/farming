...
export interface CodeCopy {
  ...
  readOnlyShareExpires: (expiresAt: number) => string
...
const EN_COPY: CodeCopy = {
  ...
  createIsolatedDesktop: 'Create Desktop in Docker (Experimental)',
  ...
  readOnlyShareExpires: expiresAt => `View only · Valid until ${new Date(expiresAt).toLocaleString('en-US')}`,
  ...
  forkSameWorktree: 'Continue in new Agent',
  forkNewWorktree: 'Continue in new worktree',
  newWorktreeFork: 'Continued in a separate worktree',
  ...
  branchBlockedActiveAgents: 'Wait for active Agents to finish. Idle sessions can stay open; refresh if an Agent just became idle.',
...
const ZH_COPY: CodeCopy = {
  ...
  renameProject: '重命名项目',
  createBrowser: '创建浏览器',
  createIsolatedDesktop: '创建 Docker 中的桌面（实验性）',
  archiveAgent: '归档',
  ...
  readOnlyShareExpires: expiresAt => `只能查看 · 有效至 ${new Date(expiresAt).toLocaleString('zh-CN', { hour12: false })}`,
  ...
  forkSameWorktree: '在新 Agent 中继续',
  forkNewWorktree: '在新 worktree 中继续',
  newWorktreeFork: '使用独立 worktree 继续',
  scheduledTask: '周期任务',
  ...
  branchBlockedActiveAgents: '请等待正在执行的 Agent 任务结束。空闲会话可以保持打开；如果 Agent 刚变为空闲，请刷新。',
