...
interface TranscriptImage extends DataRecord {
  id: string;
  url: string;
  alt?: string;
  mimeType?: string;
...
interface TranscriptFile extends DataRecord {
  id?: string;
  name: string;
  content: string;
  error?: string;
...
interface ProcessItem extends DataRecord {
  id: string;
  type: string;
  title: string;
  detail: string;
...
interface TranscriptTurn extends DataRecord {
  id: string;
  userMessage: string;
...
function isTranscriptImage(value: unknown): value is TranscriptImage {
  return isDataRecord(value) && typeof value.id === 'string' && typeof value.url === 'string';
}

function isTranscriptFile(value: unknown): value is TranscriptFile {
...
interface ExtractedFileAttachments {
  text: string;
  files: TranscriptFile[];
}
...
const LOCAL_AUDIO_MIME_BY_EXT: Record<string, string> = {
  '.aac': 'audio/aac',
  '.flac': 'audio/flac',
  '.m4a': 'audio/mp4',
  '.mp3': 'audio/mpeg',
...
function extractMemoryCitationBlock(value: unknown): { text: string; detail: string } {
  const text = normalizeText(value);
  if (!text) return { text: '', detail: '' };
  const blocks: string[] = [];
  const cleaned = text.replace(/<oai-mem-citation>[\s\S]*?<\/oai-mem-citation>/g, (block: string) => {
    blocks.push(block);
    return '';
  });
  return {
...
function formatRawMemoryCitationBlock(block: unknown): string {
...
function detailForMemoryCitation(memoryCitation: unknown) {
  const citation = dataRecord(memoryCitation);
  if (!memoryCitation || !isDataRecord(memoryCitation)) return '';
  const rows = [];
  ...
  const threadIds = Array.isArray(citation.threadIds)
    ? citation.threadIds
    : (Array.isArray(citation.thread_ids) ? citation.thread_ids : []);
  if (threadIds.length) rows.push(`threads: ${threadIds.map((id: unknown) => compactInline(id)).filter(Boolean).join(', ')}`);
  return summarizeOutput(rows.join('\n'));
}

function appendMemoryCitation(turn: TranscriptTurn, id: unknown, memoryCitation: unknown, rawDetail: unknown = '') {
...
function finalMessageText(value: unknown, turn: TranscriptTurn, id: unknown, memoryCitation: unknown = undefined) {
...
function isInjectedContextMessage(value: unknown): boolean {
  return isCodexInjectedContextMessage(value);
}

function renderedAttachmentKindSet(options: TranscriptBuildOptions = {}): Set<string> {
  return new Set(Array.isArray(options.renderedAttachmentKinds) ? options.renderedAttachmentKinds : []);
}

function extractComposerFileAttachments(value: unknown): ExtractedFileAttachments {
  const text = normalizeText(value);
  if (!text) return { text: '', files: [] };
  const files: TranscriptFile[] = [];
  const pattern = /(^|\n{2,})\s*Attached file:\s*([^\n]*)(?:\n{2,}|\n)([\s\S]*?)(?=\n{2,}\s*Attached (?:file|image):|\n{2,}\s*<image\b|$)/gi;
  const stripped = text.replace(pattern, (_match: string, prefix: string, rawName: string, rawContent: string) => {
...
function findLastTranscriptTimestamp(text: string): { index: number; end: number } | null {
...
function extractReferencedPastedTranscriptContext(value: unknown): ExtractedFileAttachments {
  const text = normalizeText(value);
...
function extractCodexApprovalTranscriptContext(value: unknown): ExtractedFileAttachments {
  const text = normalizeText(value);
  if (
    !text ||
    text.length < 500 ||
...
function parseSubagentNotification(value: unknown): DataRecord | null {
  const text = normalizeText(value);
  ...
  const record = payload as DataRecord;
  const status = dataRecord(record.status);
  const completed = typeof status.completed === 'string' ? status.completed : '';
  const failed = typeof status.failed === 'string' ? status.failed : '';
  const detail = completed || failed || compactInline(status.message || record.message || '');
  return {
...
function stripRenderedAttachmentTagBlocks(value: unknown, renderedKinds: Set<string>) {
  const text = normalizeText(value);
  if (!text) return '';
  let next = text;
...
function stripRenderedComposerAttachmentBlocks(value: unknown, renderedKinds: Set<string>) {
  const text = normalizeText(value);
  if (!text) return '';
  let next = text;
  for (const kind of renderedKinds) {
...
function stripRenderedCodexHistoryAttachmentLinks(value: unknown, renderedKinds: Set<string>) {
  const text = normalizeText(value);
...
function visibleUserMessageText(value: unknown, options: TranscriptBuildOptions = {}): string {
...
function renderedAttachmentKindsForTurn(turn: TranscriptTurn) {
  const kinds = [];
...
function renderedAttachmentKindsForAttachments({
  images = [], audios = [], files = [],
}: { images?: unknown[]; audios?: unknown[]; files?: unknown[] } = {}) {
  const kinds = [];
...
function compactInline(value: unknown, fallback = ''): string {
  const text = normalizeText(value).replace(/\s+/g, ' ');
  return text || fallback;
}

function humanizeType(value: unknown, fallback = 'Event'): string {
...
function textFromContent(content: unknown): string {
  ...
  if (content && typeof content === 'object' && !Array.isArray(content)) {
    const record = content as DataRecord;
    ...
    if (Array.isArray(record.content_items)) return textFromContent(record.content_items);
    if (Array.isArray(record.contentItems)) return textFromContent(record.contentItems);
    return '';
  }
...
function imageAltFromContentPart(part: DataRecord, index: number): string {
...
function imageAltFromPath(filePath: unknown, index: number): string {
...
function imagesFromContent(content: unknown): TranscriptImage[] {
  if (!Array.isArray(content)) return [];
  const images: TranscriptImage[] = [];
  for (const partValue of content) {
    if (!isDataRecord(partValue)) continue;
...
function audioMimeFromUrl(url: unknown) {
  const dataMime = String(url || '').match(/^data:(audio\/[^;,]+)[;,]/i)?.[1];
  if (dataMime) return dataMime.toLowerCase();
  try {
    return LOCAL_AUDIO_MIME_BY_EXT[path.extname(new URL(String(url || '')).pathname).toLowerCase()] || '';
  } catch {
    return '';
  }
}

function audioNameFromPart(part: DataRecord, index: number) {
...
function audioFromLocalPath(filePath: unknown, index: number) {
...
function audiosFromContent(content: unknown) {
  if (!Array.isArray(content)) return [];
  const audios: DataRecord[] = [];
...
function inlineImageFromEntry(entryValue: unknown, index: number): TranscriptImage | null {
  if (typeof entryValue === 'string') {
    return isDisplayableImageUrl(entryValue)
      ? { id: `image-${index + 1}`, url: entryValue, alt: `Image ${index + 1}` }
      : null;
  }
  if (!isDataRecord(entryValue)) return null;
...
function imageFromValue(value: unknown, index: number, alt: unknown = ''): TranscriptImage | null {
...
function imagesFromImageItem(item: DataRecord) {
...
function imageFromLocalPath(filePath: unknown, index: number): TranscriptImage | null {
  const normalized = String(filePath || '').trim();
  if (!normalized || normalized.includes('\0')) return null;
...
function imagesFromUserMessagePayload(payload: DataRecord) {
  const images: TranscriptImage[] = [];
  const appendImage = (image: TranscriptImage | null) => {
  ...
  for (const entry of localImages) {
    const filePath = typeof entry === 'string'
      ? entry
      : (isDataRecord(entry) ? entry.path || entry.file || entry.url : '');
...
function summarizeStructuredOutput(output: unknown) {
  ...
  if (output && typeof output === 'object') {
    ...
    if (typeof record.content === 'string') return summarizeOutput(record.content);
    if (Array.isArray(record.content)) return summarizeStructuredOutput(record.content);
    if (Array.isArray(record.content_items)) return summarizeStructuredOutput(record.content_items);
    if (Array.isArray(record.contentItems)) return summarizeStructuredOutput(record.contentItems);
    return summarizeOutput(JSON.stringify(output, null, 2));
  }
  return '';
}

function imagesFromStructuredOutput(output: unknown) {
  if (Array.isArray(output)) return imagesFromContent(output);
  if (output && typeof output === 'object') {
    const record = dataRecord(output);
    const images: TranscriptImage[] = [];
    const append = (image: TranscriptImage | null) => {
      if (!image || images.length >= MAX_USER_IMAGES_PER_TURN) return;
      if (images.some(existing => existing.url === image.url)) return;
      images.push(image);
    };
    for (const field of ['content', 'content_items', 'contentItems']) {
      ...
      for (const image of imagesFromContent(record[field])) append(image);
    }
    return images;
  }
  return [];
...
function commandText(command: unknown) {
  if (Array.isArray(command)) return command.map((part: DataRecord) => String(part || '')).filter(Boolean).join(' ');
  return normalizeText(command);
}

function titleForParsedCommand(parsed: DataRecord) {
...
function titleForCommandExecution(payload: DataRecord) {
...
function summarizeApplyPatchText(input: unknown) {
  const text = normalizeText(input);
  if (!text.startsWith('*** Begin Patch')) return '';
  const rows: string[] = [];
  let active: PatchSummaryRow | null = null;
  const flush = () => {
    if (!active) return;
    const statLabel = [
...
function applyPatchTitleFromText(input: unknown) {
...
function processTitleForFunctionCall(payload: DataRecord) {
  const name = String(payload.name || '').trim();
  const args = parseJsonValue(payload.arguments);
  if (name === 'exec_command') {
    const cmd = normalizeText(args && typeof args === 'object' ? (args as DataRecord).cmd : '').replace(/\s+/g, ' ');
    return cmd ? `Ran ${cmd}` : 'Ran command';
  }
...
function processTypeForFunctionCall(payload: DataRecord) {
  const name = String(payload.name || '').trim();
...
function detailForFunctionCall(payload: DataRecord) {
...
function titleForLocalShell(payload: DataRecord) {
  const action = dataRecord(payload.action);
  const command = action.command || action.cmd || payload.command;
  const cmd = compactInline(command);
  return cmd ? `Ran ${cmd}` : 'Ran local shell';
}

function detailForLocalShell(payload: DataRecord) {
  const action = dataRecord(payload.action);
...
function summarizeCommandOutput(payload: DataRecord) {
  const chunks = [
    ...
    payload.stdout,
    payload.stderr,
  ].filter((value: unknown) => normalizeText(value));
...
function normalizePatchChanges(changes: unknown) {
  if (Array.isArray(changes)) {
    return changes.map((change: DataRecord, index: number) => {
...
function summarizePatchChanges(changes: unknown) {
  const normalized = normalizePatchChanges(changes);
  const rows = normalized.slice(0, 16).map((change: DataRecord) => {
...
function rawContentLineCount(content: unknown) {
  const text = normalizeText(content);
  if (!text) return 0;
  return text.split('\n').length;
}

function patchChangeStats(change: DataRecord) {
...
function titleForPatch(payload: DataRecord) {
...
function titleForReviewAction(actionValue: unknown) {
  const action = dataRecord(actionValue);
...
function detailForReview(payload: DataRecord) {
  const review = dataRecord(payload.review);
  const action = dataRecord(payload.action);
  return [
...
function detailForHook(run: DataRecord) {
  const entries = Array.isArray(run.entries)
    ? run.entries
        .map((entry: DataRecord) => {
...
function statusForHook(run: DataRecord, fallback: string = 'running') {
...
function titleForMcp(payload: DataRecord) {
  const invocation = dataRecord(payload.invocation);
...
function detailForMcp(payload: DataRecord) {
  const invocation = dataRecord(payload.invocation);
  const result = dataRecord(payload.result);
  const detail = [];
  ...
  if (payload.result && isDataRecord(payload.result)) {
    if (Array.isArray(result.content)) {
      const text = result.content
        .map((part: DataRecord) => (part && typeof part.text === 'string' ? part.text : ''))
        .filter(Boolean)
        .join('\n');
...
function textFromUserInput(content: unknown) {
  if (typeof content === 'string') return normalizeText(content);
  if (!Array.isArray(content)) return '';
  return content
    .map((part: DataRecord) => {
...
function textFromUserMessagePayload(payload: DataRecord) {
...
function turnItemType(item: DataRecord) {
  if (!item || typeof item !== 'object') return '';
  return String(item.type || '').trim().replace(/[_-]/g, '').toLowerCase();
}

function itemStatus(item: DataRecord, fallback: string = '') {
  return String(item.status || item.statusText || fallback);
}

function responseItemTurnId(payload: DataRecord) {
  if (!payload || typeof payload !== 'object') return '';
  const metadata = dataRecord(
    payload.internal_chat_message_metadata_passthrough || payload.internalChatMessageMetadataPassthrough,
  );
  return payload.turn_id || payload.turnId || (metadata && (metadata.turn_id || metadata.turnId)) || '';
}

function normalizeEventEnvelope(event: unknown) {
  if (!isDataRecord(event)) return { type: '', payload: null };
  if (typeof event.method === 'string') {
    return {
      type: event.method,
      payload: dataRecord(event.params),
    };
  }
  const payload = isDataRecord(event.payload) ? event.payload : event;
  return {
...
function detailForPlanSteps(steps: unknown) {
  ...
  return steps
    ...
    .filter((line: string) => line.trim() !== '[ ]' && line.trim() !== '[x]' && line.trim() !== '[>]')
    .join('\n');
}

function applyUserMessageToTurn(
  turn: TranscriptTurn,
  { id = '', message = '', images = [], audios = [] }: UserMessageInput = {},
) {
  if (!turn) return;
...
function appendTurnItem(turn: TranscriptTurn, item: DataRecord, status: string = '') {
  ...
  if (type === 'hookprompt') {
    ...
    const text = fragments.map((fragment: DataRecord) => fragment && fragment.text).filter(Boolean).join('\n\n');
    if (text) appendProcess(turn, { id: item.id, type: 'hook', title: 'Hook prompt', detail: text, status });
    return true;
  }
  if (type === 'agentmessage') {
    const text = Array.isArray(item.content)
      ? item.content.map((part: DataRecord) => part && part.text).filter(Boolean).join('\n\n')
  ...
  if (type === 'sleep') {
    ...
    appendProcess(turn, {
      id: item.id,
      type: 'sleep',
      title: typeof duration === 'number' ? `Slept for ${Math.round(duration / 1000)}s` : 'Slept',
      status: status || itemStatus(item),
    });
    return true;
  }
  if (type === 'imagegeneration') {
    const detail = [
      ...
      item.status,
    ].map((value: unknown) => compactInline(value)).filter(Boolean).join('\n');
    appendProcess(turn, {
      id: item.id,
...
function appendResponseItem(
  turn: TranscriptTurn,
  payload: DataRecord,
  functionCalls: Map<string, { title: string; type: string }>,
) {
  ...
  if (payload.type === 'web_search_call') {
    ...
    appendProcess(turn, {
      id: payload.id,
      type: 'web-search',
      title: compactInline(action.query, 'Searched web'),
...
function newTurn(id: unknown = ''): TranscriptTurn {
  return {
...
function hasRunningProcessItems(turn: TranscriptTurn) {
  return turn.processItems.some((item: DataRecord) => {
    const status = String(item.status || '').replace(/[_-]/g, '').toLowerCase();
    return status === 'running' || status === 'inprogress' || status === 'pending';
  });
}

function isStandaloneThreadSettingsTurn(turn: TranscriptTurn) {
  ...
  return items.length > 0 && items.every((item: DataRecord) => (
    item && item.type === 'event' && item.title === 'Thread Settings Applied'
  ));
}

function processItemIdentity(item: DataRecord) {
  return [
    String(item?.type || ''),
...
function dedupeAdjacentProcessItems(items: ProcessItem[]) {
  const deduped = [];
  let previousKey = '';
...
function sanitizeProcessItemForOutput(item: DataRecord) {
...
function normalizeTurnForOutput(turn: TranscriptTurn) {
...
function completeTurn(turn: TranscriptTurn, payload: DataRecord = {}) {
  if (!turn) return;
  ...
  const nestedTurn = dataRecord(payload.turn);
  if (typeof nestedTurn.completedAt === 'number') {
    turn.completedAt = nestedTurn.completedAt;
  }
  ...
  if (typeof nestedTurn.durationMs === 'number') {
    turn.durationMs = nestedTurn.durationMs;
  }
...
function appendTurnSnapshot(turns: TranscriptTurn[], turnValue: unknown) {
  if (!isDataRecord(turnValue)) return false;
  const turn = turnValue;
  const normalizedTurn = newTurn(turn.id);
  normalizedTurn.status = String(turn.status || normalizedTurn.status);
...
function appendProcess(turn: TranscriptTurn, item: DataRecord) {
  if (!turn || !item || !item.title) return;
  const id = String(item.id || `${turn.id}-process-${turn.processItems.length + 1}`);
  const detail = normalizeText(item.detail);
  const files = (Array.isArray(item.files) ? item.files : [])
    .filter(isTranscriptFile)
    .slice(0, MAX_USER_FILES_PER_TURN);
  const images = (Array.isArray(item.images) ? item.images : [])
    .filter(isTranscriptImage)
  ...
  if (existing) {
    ...
    if (next.images.length) {
      ...
      existing.images = [
        ...existing.images,
        ...next.images.filter(image => image.url && !urls.has(image.url)),
      ].slice(0, MAX_USER_IMAGES_PER_TURN);
    }
    if (next.files.length) {
      ...
      existing.files = [
        ...existing.files,
        ...next.files.filter(file => !keys.has(`${file.name}\0${file.content}\0${file.error}`)),
      ].slice(0, MAX_USER_FILES_PER_TURN);
    }
...
function appendProcessDelta(turn: TranscriptTurn, item: DataRecord) {
  if (!turn || !item || !item.title) return;
  const id = String(item.id || `${turn.id}-process-${turn.processItems.length + 1}`);
  const delta = normalizeDeltaText(item.delta);
  const existing = turn.processItems.find(entry => entry.id === id);
  if (existing) {
    existing.type = String(item.type || existing.type);
...
function buildTranscriptFromLines(lines: string[], options: TranscriptBuildOptions = {}) {
  const turns: TranscriptTurn[] = [];
  let active: TranscriptTurn | null = null;
  const functionCalls = new Map<string, { title: string; type: string }>();
  ...
  const resumeSnapshotTurn = (id: unknown) => {
    if (!id) return null;
  ...
  const finishActive = (payload: DataRecord = {}) => {
    if (!active) return;
  ...
  for (const line of lines) {
    ...
    const providerThread = dataRecord(payload.thread);
    if (eventType === 'thread/read' && Array.isArray(providerThread.turns)) {
      if (active && !isEmptyTurn(active)) turns.push(active);
      active = null;
      ...
      turns.length = 0;
      for (const turn of providerThread.turns) appendTurnSnapshot(turns, turn);
      continue;
    }
    ...
    if (eventType === 'turn.started' || eventType === 'turn_started' || eventType === 'turn/started') {
      const nestedTurn = dataRecord(payload.turn);
      const turn = ensureActive(payload.turn_id || payload.turnId || nestedTurn.id);
      turn.status = 'inProgress';
      if (typeof nestedTurn.startedAt === 'number') turn.startedAt = nestedTurn.startedAt;
      if (typeof nestedTurn.status === 'string') turn.status = nestedTurn.status;
    ...
    if (
      ...
      const status = eventType === 'item.started' || eventType === 'item/started'
        ...
        : eventType === 'item.updated' || eventType === 'item/updated'
          ? itemStatus(dataRecord(payload.item), 'running')
          : itemStatus(dataRecord(payload.item), 'completed');
      if (appendTurnItem(ensureActive(payload.turn_id || payload.turnId), dataRecord(payload.item), status)) continue;
    }

    if (eventType === 'turn.completed' || eventType === 'turn_complete' || eventType === 'turn/completed') {
      const completedTurn = dataRecord(payload.turn);
      if (Array.isArray(completedTurn.items)) {
        const turn = ensureActive(completedTurn.id);
        for (const item of completedTurn.items) {
          if (!isDataRecord(item)) continue;
          appendTurnItem(turn, item, itemStatus(item, String(completedTurn.status || 'completed')));
        }
      }
      finishActive({
        ...payload,
        type: 'turn_complete',
        turn_id: payload.turn_id || payload.turnId || completedTurn.id,
        duration_ms: payload.duration_ms || payload.durationMs || completedTurn.durationMs,
      });
      continue;
    }
    ...
    if (eventType === 'hook/started' || eventType === 'hook/completed') {
      const run = dataRecord(payload.run);
      const completed = eventType.endsWith('/completed');
    ...
    if (eventType === 'rawResponseItem/completed') {
      const turn = ensureActive(payload.turn_id || payload.turnId);
      if (appendResponseItem(turn, dataRecord(payload.item), functionCalls)) continue;
    }

    if (eventType === 'turn.failed' || eventType === 'error') {
      const turn = ensureActive(payload.turn_id || payload.turnId);
      const error = isDataRecord(payload.error) ? payload.error : payload;
      appendProcess(turn, {
        type: 'error',
    ...
    if (eventType === 'thread/realtime/itemAdded') {
      const turn = ensureActive(payload.turn_id || payload.turnId || `${payload.threadId || payload.thread_id || 'realtime'}-realtime`);
      const realtimeItem = dataRecord(payload.item);
      if (appendTurnItem(turn, realtimeItem, itemStatus(realtimeItem, 'completed'))) continue;
      if (appendResponseItem(turn, realtimeItem, functionCalls)) continue;
    }
    ...
    if (eventType === 'event_msg') {
      ...
      if (payload.type === 'user_message') {
        const previousTurn = active as DataRecord | null;
        if (previousTurn?.userMessage && (previousTurn.finalMessage || previousTurn.status !== 'inProgress')) {
          finishActive();
        }
        const turn = ensureActive(payload.turn_id);
      ...
      if (payload.type === 'item_completed' || payload.type === 'item_started' || payload.type === 'item_updated') {
        ...
        const status = payload.type === 'item_started'
          ...
          : payload.type === 'item_updated'
            ? itemStatus(dataRecord(payload.item), 'running')
            : itemStatus(dataRecord(payload.item), 'completed');
        if (appendTurnItem(turn, dataRecord(payload.item), status)) continue;
      }
      ...
      if (payload.type === 'mcp_tool_call_end') {
        appendProcess(ensureActive(payload.turn_id), {
          ...
          status: dataRecord(payload.result).is_error ? 'failed' : 'completed',
        });
        continue;
      }

      if (payload.type === 'web_search_end') {
        const query = payload.query || dataRecord(payload.action).query;
        appendProcess(ensureActive(payload.turn_id), {
          id: payload.call_id,
          type: 'web-search',
      ...
      if (!isIgnoredEventType(String(payload.type || ''))) {
        appendProcess(ensureActive(payload.turn_id), {
          id: payload.id || payload.call_id,
          type: 'event',
  ...
  const maxTurns = Number.isFinite(options.maxTurns)
    ? Math.max(1, Math.floor(options.maxTurns as number))
    : DEFAULT_MAX_TURNS;
  return turns
    .map(normalizeTurnForOutput)
    .filter((turn: TranscriptTurn) => turn && !isEmptyTurn(turn) && !isStandaloneThreadSettingsTurn(turn))
    .slice(-maxTurns);
}

function buildTranscriptFromEvents(events: unknown[], options: TranscriptBuildOptions = {}) {
  const lines = Array.isArray(events)
    ? events.filter((event: unknown) => event && typeof event === 'object').map((event: unknown) => JSON.stringify(event))
    : [];
