...
interface LocalSessionEngineOptions {
  configDir?: string;
...
function stripInheritedPromptEnv(env: NodeJS.ProcessEnv): void {
  for (const key of INHERITED_PROMPT_ENV_KEYS) {
    delete env[key];
  }
}

function extractLatestTerminalTitle(data: unknown): string | null {
  if (!data) return null;
...
let cachedPtyError: unknown = null;

function loadPtyModule(): PtyModule {
  if (cachedPty) return cachedPty;
  if (cachedPtyError) throw cachedPtyError;
  try {
    const packagedNodePtyModule: { nodePty: PtyModule } = require('./packaged-node-pty.cjs');
    cachedPty = packagedNodePtyModule.nodePty;
    return packagedNodePtyModule.nodePty;
  } catch (error) {
...
function createPtyProcess(
  ...
  try {
    ...
    const wrapped = new Error(`Farming requires node-pty to start interactive agents. Native PTY is unavailable: ${errorMessage(error)}`);
    wrapped.cause = error;
    throw wrapped;
  }
}

function normalizeShellSessionOptions(
...
class LocalSessionEngine extends SessionEngine {
  ...
  handleSessionExit(session: LocalSession, code: number): Promise<void> {
  ...
  handleSessionData(
    sessionId: string,
    rawData: string,
    expectedSession: LocalSession | null = null,
  ): void {
    ...
    if (!current.screenWorker) throw new TypeError('Terminal screen worker is unavailable');
  ...
  failSessionScreenState(session: LocalSession, error: unknown): void {
    const current = this.sessions.get(session.id);
  ...
  async resizeSession(
    ...
    try {
      resize.call(session.process, cols, rows);
    } catch (error) {
      if (errorMessage(error).includes('EBADF')) {
        session.status = 'exited';
