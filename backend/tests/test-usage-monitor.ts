...
async function run() {
  ...
  assert.strictEqual(summary.providers.length, 6);
  ...
  assert.strictEqual(summary.liveTimeline.points.length, 60);
  assert.strictEqual(summary.liveTimeline.windowMs, 60 * 60 * 1000);
  assert.strictEqual(summary.liveTimeline.bucketMs, 60 * 1000);
  assert.strictEqual(summary.liveTimeline.startAt % (60 * 1000), 0);
  assert.strictEqual(summary.liveTimeline.endAt % (60 * 1000), 0);
  ...
  assert.strictEqual(summary.providers.find(entry => entry.provider === 'pi').tokenUsage.available, false);
  ...
  const failedExportOpenCodeMonitor = new UsageMonitor({
    ...
    async openCodeCommandRunner(args) {
      ...
      const error = new Error('OpenCode export failed after listing the session') as NodeJS.ErrnoException;
      error.code = 'EIO';
      throw error;
    },
    windowMs,
