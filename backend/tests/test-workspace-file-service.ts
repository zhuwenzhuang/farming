...
function commandError(message: string): CommandError {
  return new Error(message) as CommandError;
}
...
async function run() {
  ...
  try {
    ...
    try {
      ...
      const watcherRecord = Array.from(watcherService.watchers.values())[0] as {
        subscribers: Set<unknown>;
      };
      assert.strictEqual(watcherRecord.subscribers.size, 2);
      await unsubscribeFirst();
      assert.strictEqual(watcherService.watchers.size, 1);
      assert.strictEqual(watcherRecord.subscribers.size, 1);
    ...
    fsp.lstat = async (targetPath, ...args) => {
      if (targetPath === vanishingAbsolutePath) {
        ...
        const error = commandError('vanished');
        error.code = 'ENOENT';
        throw error;
      }
    ...
    fs.promises.open = async (...args) => {
      ...
      if (String(args[0]).includes('.farming-')) {
        handle.datasync = async () => {
          const error = commandError('simulated datasync failure');
          error.code = 'EIO';
          throw error;
        };
      }
    ...
    if (hasCommand('rg')) {
      ...
      const timeoutService = new WorkspaceFileService({
        commandRunner: {
          run: async (_command, args) => {
            const error = commandError('ripgrep timed out');
      ...
      service.execFile = async (command, args, options) => {
        if (command === service.rgPath && args.includes('--json')) {
          const error = commandError('stdout maxBuffer length exceeded');
    ...
    if (hasCommand('git')) {
      ...
      service.execFile = async (command, args, options) => {
        if (command === service.gitPath && args.includes('blame')) {
          ...
          const error = commandError('git blame timed out');
          error.code = 'ETIMEDOUT';
          throw error;
        }
      ...
      const changeByPath = new Map<string, ChangeItem>(
      ...
      service.execFile = async (command, args, options) => {
        if (command === service.gitPath && args.includes('status') && args.includes('--untracked-files=all')) {
          ...
          const error = commandError('git status timed out');
          error.code = 'ETIMEDOUT';
          throw error;
        }
        return originalChangesExecFile(command, args, options);
