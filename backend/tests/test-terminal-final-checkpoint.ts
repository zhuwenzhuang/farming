...
type ScreenWorkerFixture = {
  append?: (
    data: string,
    stateRevision: number,
    outputSeq: number,
  ) => Promise<TerminalCheckpointFixture>;
  getState: () => Promise<TerminalCheckpointFixture | null>;
  dispose: () => Promise<void>;
};

type TerminalSessionFixture = {
  id: string;
  command: string;
  cwd: string;
  status: string;
  ...
  renderOutput: string;
  previewText: string;
  previewSnapshot: { rows: string[] } | null;
  previewCols: number;
  previewRows: number;
  title: string;
  ...
  screenWorker: ScreenWorkerFixture;
  finalCheckpoint?: Readonly<TerminalCheckpointFixture>;
};

function createSession(id: string): TerminalSessionFixture {
