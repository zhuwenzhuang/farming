...
function providerToolStatus(status: ToolCallStatus | 'cancelled'): ToolCallStatus {
  // Farming intentionally accepts the historical provider extension even
  // though the current ACP schema has no cancelled ToolCallStatus literal.
...
if (process.argv.includes('--fake-terminal-login')) {
  ...
  await new Promise<void>(resolve => process.stdin.once('data', value => {
...
function deterministicE2eSessionId(agentId: string | undefined): string {
...
class FakeAgent implements Agent {
  ...
  async extMethod(method, params) {
    if (method === '_session/steering' || method === '_codex/session/steer') {
      ...
      if (steerTurn.delayAcceptanceMs > 0) {
        await new Promise(resolve => setTimeout(resolve, steerTurn.delayAcceptanceMs));
      }
      if (steerTurn.echoUserMessage !== false) {
        for (const content of params.prompt || []) {
          await client.sessionUpdate({
            ...
            update: {
              ...
              content,
              _meta: { codex: { steer: true, turnId: steerTurn.turnId } },
            },
          });
        }
      }
      ...
      const turnId = steerTurn.turnId;
      if (steerTurn.received >= steerTurn.expected) {
  ...
  async prompt(params: PromptRequest): Promise<PromptResponse> {
    ...
    if (promptText.includes('subagent elicitation')) {
      ...
      await client.sessionUpdate({
        ...
        update: {
          sessionUpdate: 'agent_message_chunk',
          messageId: 'input-child-answer',
          content: {
            type: 'text',
            text: `Child confirmed: ${acceptedConfirmation(elicitation)}`,
          },
        },
      });
      await client.sessionUpdate({
    ...
    if (promptText.includes('Review parser edge cases (demo)')) {
      ...
      await client.sessionUpdate({ sessionId: params.sessionId, update: {
        ...
        _meta: { codex: { subagent: { threadId: childSessionId, path: '/root/parser_reviewer', activity: 'spawned' } } },
      } });
      const notifyNative = (update: Record<string, unknown>) => process.stdout.write(JSON.stringify({
        jsonrpc: '2.0', method: 'session/update', params: { sessionId: params.sessionId, update },
      }) + '\n');
    ...
    if (promptText.includes('long subagent')) {
      ...
      await new Promise<void>(resolve => cancelledSessions.set(childSessionId, resolve));
      ...
      await client.sessionUpdate({
        ...
        update: {
          sessionUpdate: 'tool_call_update',
          toolCallId: 'long-subagent-tool',
          status: providerToolStatus('cancelled'),
          _meta: { subagent_session_info: { session_id: childSessionId, message_start_index: 0, message_end_index: 1 } },
        },
      });
      await client.sessionUpdate({
    ...
    if (promptText.includes('client services')) {
      ...
      await client.sessionUpdate({
        ...
        update: {
          ...
          content: {
            type: 'text',
            text: `${file.content}; ${output.output}; exit=${exit.exitCode}; confirmed=${acceptedConfirmation(elicitation)}`,
          },
        },
      });
      return { stopReason: 'end_turn' };
    ...
    if (promptText.includes('long terminal')) {
      ...
      await client.sessionUpdate({
        ...
        update: {
          sessionUpdate: 'tool_call_update',
          toolCallId: 'long-terminal-tool',
          status: providerToolStatus(exit.signal ? 'cancelled' : 'completed'),
        },
      });
      await client.sessionUpdate({
        sessionId: params.sessionId,
