...
type TestWindow = typeof window & {
  ...
  __sentInputs: string[];
  __patchedSessionInputE2E?: boolean;
  pasteTerminalText(_text: string): void;
...
async function startDisposableSession(page, agentName) {
  ...
  await page.evaluate((expectedAgent) => {
    const item = Array.from(document.querySelectorAll('.agent-item'))
      .find((node) => node.textContent.includes(expectedAgent));
    if (!(item instanceof HTMLElement)) throw new Error(`${expectedAgent} list item not found`);
    item.click();
  }, agentName);
  ...
  await page.$eval('#workspace-input', (input, nextValue) => {
    (input as { value: string }).value = nextValue;
  ...
  await page.evaluate(() => {
    ...
    if (!(startButton instanceof HTMLElement)) throw new Error('Start button not found');
    startButton.click();
  });
  ...
  const terminalKind = await page.evaluate(async () => {
    const bundle = await (window as TestWindow).FarmingTerminalBridge.createInstance();
  ...
  await page.evaluate(() => {
    (window as TestWindow).__sentInputs = [];
    const originalSend = WebSocket.prototype.send;
    if (!(window as TestWindow).__patchedSessionInputE2E) {
      WebSocket.prototype.send = function patchedSend(payload) {
        try {
          if (typeof payload === 'string') {
            const data = JSON.parse(payload);
            if (data.type === 'input') {
              (window as TestWindow).__sentInputs.push(data.input);
            }
          }
        } catch {
      ...
      (window as TestWindow).__patchedSessionInputE2E = true;
    }
  });
...
async function openClaudeFromDialog(page, startAgent = false) {
  ...
  await page.evaluate(() => {
    ...
    if (!(item instanceof HTMLElement)) throw new Error('Claude list item not found');
    item.click();
  });
...
async function getSentInputs(page) {
  return page.evaluate(() => (window as TestWindow).__sentInputs || []);
}
...
async function runWithBrowser(browser) {
  ...
  await page.evaluate(() => (window as TestWindow).pasteTerminalText('printf "surface-bash\\n"\\r'));
  ...
  await clearSentInputs(page);
  await page.evaluate(() => {
    (window as TestWindow).showInputDialog();
  });
  await page.waitForSelector('#input-dialog.active', { timeout: 5000 });
  await page.evaluate(() => {
    const item = Array.from(document.querySelectorAll('.agent-item'))
      .find((node) => node.textContent.includes('bash'));
    if (!(item instanceof HTMLElement)) throw new Error('bash list item not found');
    item.click();
  });
  await page.waitForFunction(
    () => getComputedStyle(document.getElementById('workspace-input-container')).display !== 'none',
  ...
  await zshPage.evaluate(() => (window as TestWindow).pasteTerminalText('printf "surface-zsh\\n"\\r'));
  await new Promise((resolve) => setTimeout(resolve, 600));
  ...
  const dialogState = await dialogPage.evaluate(() => {
    ...
    return {
      ...
      workspaceValue: workspaceInput.value,
  ...
  const firstHistoryValue = await dialogPage.$eval(
    '#workspace-input',
  ...
  await dialogPage.keyboard.press('ArrowDown');
  const secondHistoryValue = await dialogPage.$eval(
    '#workspace-input',
    (node) => (node as { value: string }).value,
  );
  ...
  await retryPage.evaluate(() => {
    (window as TestWindow).__lastAlertMessage = null;
    window.alert = (message) => {
      (window as TestWindow).__lastAlertMessage = message;
    };
  });
  ...
  await retryPage.waitForFunction(
    () => typeof (window as TestWindow).__lastAlertMessage === 'string'
      && (window as TestWindow).__lastAlertMessage.includes('Workspace does not exist'),
    { timeout: 10000 }
  );
