...
async function run() {
  ...
  assert.strictEqual(resolveAcpLaunch('codex').version, '1.10.0');
  assert.strictEqual(resolveAcpLaunch('claude').version, '0.75.1');
  ...
  try {
    ...
    const sharedExtensionCalls = [];
    const sharedExtensionBinding = sharedRuntime.bindings.get(sharedAgents[0]);
    const sharedExtensionHandlers = sharedExtensionBinding.runtime.handlers.get(sharedAgents[0]);
    sharedExtensionBinding.runtime.handlers.set(sharedAgents[0], {
      ...sharedExtensionHandlers,
      extNotification(method, params) {
        sharedExtensionCalls.push({ method, params });
      },
    });
    sharedHandlerSets[0].extNotification('provider/notify/example', {
      sessionId: preparedShared[0].sessionId,
      value: 'routed-to-exact-owner',
    });
    assert.deepStrictEqual(sharedExtensionCalls, [{
      method: 'provider/notify/example',
      params: {
        sessionId: preparedShared[0].sessionId,
        value: 'routed-to-exact-owner',
      },
    }], 'shared ACP extension notifications must route through the exact Session owner');
  ...
  assert.deepStrictEqual(compatibleCodexLaunch.args.slice(0, 3), [
    ...
    assert.match(compatibleCodexLaunch.args[3], /(?:dist\/acp\/codex-acp-1\.10\.0\.mjs|codex-acp\/dist\/index\.js)$/);
  const compatibleClaudeLaunch = resolveAcpLaunch('claude');
  assert.match(
    compatibleClaudeLaunch.args.at(-1),
    /(?:dist\/acp\/claude-agent-acp-0\.75\.1\.mjs|claude-agent-acp\/dist\/index\.js)$/,
  );
  ...
  try {
    ...
    assert.deepStrictEqual(await runtime.requestPermission(closeRaceBinding, {
    ...
    let cancelledTurnAdmissions = 0;
    let cancelledProviderSubmissions = 0;
    const cancelledAdmission = runtime.prompt(
      'agent-acp-prompt-cancel',
      'cancel before provider submission',
      {
        onTurnAdmitted: admission => {
          cancelledTurnAdmissions += 1;
          assert.strictEqual(admission.previousState, 'idle');
        },
        onSubmitted: () => {
          cancelledProviderSubmissions += 1;
        },
      },
    );
    await new Promise(resolve => setImmediate(resolve));
    assert.strictEqual(cancelledTurnAdmissions, 1);
    assert.strictEqual(cancelledProviderSubmissions, 0);
    ...
    assert.deepStrictEqual(
      await runtime.requestPermission(closedBinding, {
    ...
    const waitingForInput = new Promise<{
    ...
    assert.deepStrictEqual(await runtime.requestPermission(permissionBinding, {
      ...request,
