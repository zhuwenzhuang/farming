  ...
  client.ensureConnected = async () => {
    ...
    client.socket = {
      ...
      write(payload, callback) {
        ...
        if (writes === 1) {
          if (mode === 'write-error') {
            const error = new Error('simulated broken native pty socket') as NodeJS.ErrnoException;
            error.code = 'EPIPE';
            callback(error);
            return;
          }
...
async function run() {
  ...
  rotatingClient.connectOnce = async () => {
    ...
    if (connectAttempts === 2) {
      const error = new Error('new native PTY host socket is not ready yet') as NodeJS.ErrnoException;
      error.code = 'ENOENT';
      throw error;
    }
  ...
  recoveredClient.request = async (method, _params, options: { startHost?: boolean } = {}) => {
  ...
  lostClient.request = async (method, _params, options: { startHost?: boolean } = {}) => {
    if (method !== 'recoverSessions') return null;
  ...
  timeoutRecoverClient.request = async (method) => {
    ...
    const error = new Error('native recover timed out') as NodeJS.ErrnoException;
