...
async function fetchJson(baseUrl, pathname): Promise<{
  body: ReviewJsonBody;
  response: Response;
}> {
  const response = await fetch(`${baseUrl}${pathname}`);
  const body = await response.json() as ReviewJsonBody;
...
async function run() {
  ...
  if (!address || typeof address === 'string') {
    throw new Error('Review diff test server did not expose a TCP address');
  }
