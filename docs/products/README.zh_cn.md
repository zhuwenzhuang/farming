...
它适合长对话、Project Files、轻量编辑、Git 证据和多轮 Review。桌面布局持续展示项目上下文；移动布局一次聚焦一个 Surface。
...
| Review | 独立 Revision / File / Diff 流程 | 打开共享 Review Route | Revision Bound Snapshot、Finding、Reviewed State |
...
| Mobile | 聚焦 Chat、Terminal、Files 与 Drawer | 紧凑分页控制台和 Session View | 同一个带鉴权服务和 Session |
...
Review 把工作看成同一个不断演进的 Change。对比 Revision、File State、Finding、Comment 和 Reviewed State 都绑定到相关 Snapshot。第二轮 Review 因此回答的是“真正有意义的变化是什么”，而不是展示另一个互不相关的 Agent Turn。
...
Farming Code Mobile 强调一个可读任务 Surface 和 Drawer。Farming CRT Mobile 会减少 Grid，同时保留键盘/Action 语义和可读 Agent Card。

<p align="center">
  <img src="code/assets/05-mobile-agent-chat.jpg" alt="Farming Code 移动 Chat" width="320">
  &nbsp;&nbsp;
  <img src="crt/assets/09-crt-mobile-dashboard.jpg" alt="Farming CRT 移动控制台" width="320">
</p>
