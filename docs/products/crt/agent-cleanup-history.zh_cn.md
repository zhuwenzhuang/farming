...
用户通过统一的 Archive Lifecycle 结束任何 Agent，包括 Main Agent。保守的 Inactivity Policy
与 Process Natural Exit 可以归档非 Main Agent。只有 Runtime 到达终态后，Farming 才从 Live
List 移除 Agent，并记录一份包含用户关心的 Identity、Workspace、Timing 与 End Reason 的
Archive Summary。CRT 继续显示紧凑的 `KILL` 文案，但该控件与 Farming Code 请求同一个
Archive 操作。

Main Agent 永远不能被自动清理，但用户可以显式归档。时间久不能证明 Runtime 已停止。对同一 Terminal Lifecycle
