...
- 配置后的 Project Name；
- Bottom Aligned、ANSI Aware 的 Terminal Tail，或展示最近可见 User Message、Agent Reply 与当前 Tool Step 的紧凑 Chat Trail；
- 稳定数字快捷标记。
...
Dashboard Preview 是监控摘要，不是交互式 Session Canvas。Terminal Card 最多每秒批处理一次 ANSI Aware Snapshot；结构化 Chat Card 读取后端清洗后的 Transcript，只投影最近可见 Turn 与当前 Activity。ACP Revision 经过节流且只更新受影响 Card，不会为每个流式 Chunk 重建整个 Grid。Session 打开时，该 Client 暂停后台 Dashboard Render 与 Preview Stream；关闭时只请求一次最新合并 State。
