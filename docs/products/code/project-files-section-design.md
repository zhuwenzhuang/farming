...
- Mounted Project headers can be dragged to reorder Projects. The backend-owned `projectWorkspaces` list persists the order, while pinned Projects remain a separate leading order group. Agent and Project drop indicators use the normal blue interaction accent rather than a destructive color.
