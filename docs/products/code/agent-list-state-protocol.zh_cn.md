...
Chat 当前轮次失败，或被确认的意外运行时丢失中断时，Agent 行显示红色空心
圆圈感叹号。后端拥有精确轮次身份和结果，与连接状态、未读状态分离。
已提交轮次进入 active，按顺序收到最终结果后进入 completed、cancelled、
failed 或 interrupted。针对该轮次接受取消时，在发送取消请求前记录 cancelling；
取消操作失败保留取消意图，通过既有操作错误反馈展示。工具或命令报错、用户取消、
主动生命周期移除和浏览器断线均不产生任务异常标记。Terminal 不使用此功能。

轮次结果在刷新、已读确认和运行时恢复后保留；新一轮实际提交后替换。
提交前校验失败和单纯重连不会清除标记。运行时归属与轮次身份阻止迟到结果覆盖
新轮次。Host 丢失绑定时，仅将此前观察到的活动轮次标记为中断，不根据缺失或
旧版证据推测失败，也不自动重发提示词。已有的有界取消和绑定恢复流程继续
保证瞬态状态能够收敛，本功能不增加重试或停止模式。

普通行、紧凑行、折叠栏和搜索结果共用 16 px 图形和主题错误色，错误原因可供
辅助技术和用户查看。悬停操作不能遮挡异常标记。验收覆盖取消与失败的顺序、
工具报错后成功、任务中途正常退出、恢复、持久化、迟到结果，以及 Light、Dark、Paper。
...
Project 标题与单行 Agent 行共享导航行高和垂直间距，包括紧凑布局的触控目标。
折叠 Project 时只显示标题及适用的操作；分支／Worktree 元数据和 Agent 列表可见性
按钮在展开后恢复。折叠状态由浏览器拥有，不改变 Agent 成员、活动状态或后端排序。
吸顶尺寸必须跟随可见内容，在展开与折叠时都能正确增大和缩小。

Project 与 Agent 拖动共享插入线样式。目标可见行的上半区／下半区分别表示插入前方／
后方；不可见的吸顶占位不参与命中判断和提示线定位。开始拖动时清除悬停预览，合法
释放只调用一次现有排序所有者，拖动结束或取消清除临时反馈，不改变折叠状态和当前
Agent。支持插入可见首行之前和末行之后；分页 Agent 的“显示更多”仍是完整列表的末尾
落点。离开目标、回到自身或进入其他排序分组时清除插入线；没有当前合法目标的释放不能
复用旧位置。浏览器拖动控制器同步记录目标，渲染仅投影反馈，同帧释放不能读取旧渲染状态。Project 只能在来源所属的置顶分组内排序。后端状态仍是排序权威；拖动反馈
不代表持久化成功，也不会重试失败或结果不确定的修改。

验收覆盖折叠的 Git Project 与展开的 Agent 并列、行间距一致、反复折叠展开和视口
变化、两类行前后插入反馈、取消、排序持久化，以及 Light、Dark、Paper 下的吸顶栈释放。

## 行移除后的选择

Farming 自建子 Agent 行沿用其他 Live Agent 行的待跟进标记和归档操作，目标为子 Agent 的身份。
子 Agent 保持在父 Agent 下方，不提供置顶按钮。
子 Agent 归档确认后停止其运行并关闭对应关联会话面板，保留父 Agent 的视图和运行状态。
归档被拒绝时子 Agent 保持可用；仅凭清单中缺失不能认定归档成功。

Code 的查看意图与后端成员关系分别管理。关闭或归档未选中的 Agent 保持当前内容；
移除当前 Agent 时，按其 Project 的侧栏顺序选择下一个可打开的 Agent，没有则选择
最近的前一个。整批关闭目标均须排除。活跃时间和全局已打开标签顺序不决定接替项，
置顶展示也不改变 Project 归属。Main、已归档、已结束、已停止和仅历史 Session
不能自动接替；此迁移不会启动或恢复运行时。

没有接替项时，保持无 Agent 选中，保留最近 Project 工作区供新建 Agent 和文件操作
使用，键盘焦点回到项目标题（项目本身已移除则回到列表）。空选择在列表更新和刷新后
仍然有效。全局最近活跃选择仅用于首次恢复，不是主动移除后的兜底。完整权威列表
移除当前行时，依据此前身份遵守同一项目边界；不完整快照不能触发移除导航。

关闭／归档受理时迁移选择，在 HTTP 与状态尚未对齐时也排除待移除项。明确拒绝恢复
行的可用性，但不覆盖后续查看选择；不确定结果沿用生命周期对账，不自动重发变更。
迟到响应不能覆盖新的用户选择。焦点恢复有界，并由后续指针或键盘输入取消。
内容与焦点分别处理：归档后台行可在其项目内恢复焦点，但不能激活邻居。

验收覆盖下一项／上一项／唯一 Agent、后台与连续移除、跨项目打开顺序、运行时替换、
权威列表移除、空选择更新／刷新，以及 Light、Dark、Paper 下的项目内焦点。
