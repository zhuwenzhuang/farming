...
- 左侧抽屉：放 Projects、Agents、Files、Search、History、Usage activity 和系统状态。
...
紧凑结构只由可用宽度决定，不再依赖触摸设备判断。桌面浏览器缩窄到与 iPhone 相同的宽度时，会显示同一套顶部栏、抽屉、主内容区和 Composer；触摸判断只用于软键盘和手势行为。
...
- 其他 viewer 改变共享 terminal 尺寸后，点击 terminal 会按当前视口重新测量；尺寸已经一致时不会再次发送 resize。
- Chat 与 Terminal 使用原生文本框，直接点击即可唤起 iPhone 键盘，并保留系统输入法、选区、粘贴和听写行为。
...
手机端的 Blame 列会自动压窄，优先保证代码正文仍然可读。大范围审查和逐行追溯请使用桌面端。
...
手机端的 Search / History 用于找回现场和快速切换；长时间整理请使用桌面端。
...
打开左侧抽屉并点击 **Usage activity**，会进入占满手机视口的用量详情。内容只使用一个纵向滚动区域，不产生页面级横向滚动。

52 周热力图用于查看整体分布；需要精确选择某一天时，使用原生的 **Selected day** 日期控件，再查看完整的 24 小时直方图。切换日期或等待数据加载时，详情框和直方图的大小保持不变；内容向下滚动后，顶部标题和关闭按钮仍然可触达。
...
用户从手机抽屉打开 Usage activity，选择某一天并查看完整的逐小时用量，然后关闭详情回到原来的抽屉。

验收点：

- Usage activity 入口的触摸高度不小于 44px。
- 详情占满手机视口，只出现纵向内容滚动，不出现页面级横向滚动。
- 切换日期前后，详情框和直方图尺寸不变。
- 内容滚动后，关闭按钮仍然可见并可触达。
