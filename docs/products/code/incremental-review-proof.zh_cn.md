...
1. 跨多轮和多 Agent 保持稳定的 Review identity。
2. 明确表达增量 patch、多 Agent 批量修改和替代实现的 revision lineage。
3. 具有稳定身份、重新定位、回复、验证、Reopen 和 Accepted Risk 状态的 Finding。
4. 把 Correctness Case 做成一等对象，并保存有作用域和 revision 新鲜度的 Evidence。
5. 以新风险、未决事项、Review delta 和失效证明为中心的默认注意力队列。
6. 不假设单 Agent、单轮修改，可以批量路由到合适 Agent 的反馈机制。
7. 保守判断 revision 后哪些 Finding 和 Evidence 可能失效的规则。
8. 明确设计推理、定向场景证明、集成与回归验证顺序的 readiness policy，而不是把测试成功当作 Review 全部。
9. 在 Chat 中汇报 Review 进展，不再让单轮 File Changes 成为主要交互。
10. 覆盖大 revision、整体重写、多 Agent 并行贡献、代码移动、Finding Reopen 和 Evidence 刷新失败的产品验证。
