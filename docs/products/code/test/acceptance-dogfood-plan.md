...
For Codex and Claude, verify the first cc-statistics scan creates the SQLite usage cache, an unchanged refresh reads no JSONL content, an appended session reads only bytes after the stored offset, and a server restart reuses the same cache. Exercise a large non-usage tool-output line to prove the token-only adapter remains memory-bounded. Totals and hourly/session attribution must remain identical before and after a warm-cache refresh. Verify the Python 3.10+ path on both macOS and Linux; a missing runtime must report unavailable usage without falling back to a second parser.
