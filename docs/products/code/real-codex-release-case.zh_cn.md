...
如果 Codex 在 Terminal 启动时报告 Hook 已变化，Gate 会跟随当前实际渲染的菜单选中态，选择**不信任并继续**后再执行后续步骤。它绝不会持久化 Hook 信任，也不会使用 `--dangerously-bypass-hook-trust`；无法识别的 Prompt 或菜单结构会直接让 Gate 失败。
