...
同一组控件也会更新正在运行的原生 Codex Terminal，而不再只修改下一次启动的 profile。Farming 会在下一条 Composer 消息之前依次完成 CLI 的交互式 `/model` 选择和兼容当前 CLI 版本的 `/fast` 切换，之后再提交这条消息。新 Terminal 选择 Standard 时会显式收到 `service_tier="default"`，因此用户 Codex 配置里的 Fast 值不会再造成 Farming 控件与真实 runtime 不一致；同时仍会持久化启动 profile，保证 Agent 重启后配置一致。这些 PTY 命令绝不会注入 ACP、旧 JSON、App Server、shell、Claude、OpenCode、Qoder 或其他非 Codex Terminal Session。
