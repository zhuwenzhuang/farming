...
当实时且空闲的 ACP binding 声明 `session/fork` 时，最新一条已完成 Chat 回答下方的 Fork 会先校验精确 transcript revision，再调用 ACP `session/fork`，并把返回的不同 provider Session 加载成同一 workspace 中的新 ACP Chat Agent。这个入口不会启动 Terminal，也不会按 provider 名称猜测能力。Agent 右键菜单中的 Fork 仍是显式的 provider CLI 路径，用于同 worktree Terminal 或新 worktree 工作流。
