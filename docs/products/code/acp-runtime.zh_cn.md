...
Conversation Fork 在 Terminal 与 ACP 下共用同一份 Provider adapter 契约。每个 runtime 的声明会明确：是否支持 Fork、由源 Session 还是目标进程物化子身份、允许哪些 worktree 模式，以及是否还必须由实时 runtime 协商对应 capability。后端准入、浏览器动作、History Fork 与 provider 特化执行都读取这份契约；旧的兼容布尔值只从它派生，不再维护第二份能力名单。因此统一 lifecycle 负责校验、身份、持久化与恢复，adapter 只特化 provider 操作。`target-process` 同时覆盖 Terminal CLI Fork，以及 Claude ACP 这类必须在持有子会话的 adapter 进程中创建 Fork 的实现；`source-session` 则先创建独立 provider Session，再由 child runtime 加载。普通 Shell 仍可以复制 Agent/worktree，但不会被描述成支持 Conversation Fork。

对 ACP，浏览器只会在 source 处于稳定 idle、可恢复 error 或 hibernated，且最近一次 runtime 协商声明了 `session/fork` 时展示 Conversation Fork。后端准入仍是权威：error 或 hibernated source 会先走正常 reconnect 路径，working 或 connecting source 会被直接拒绝且不会被中断；恢复后还会重新检查最新协商 capability，再决定是否调用 provider。因此资源压力回收不会绕过 Fork 安全，陈旧的浏览器投影也不能凭空制造支持。
...
`backend/tests/test-acp-runtime.ts` 会运行一个真实官方 SDK client 和确定性的假 ACP 子进程，验证新 Session、load 完整历史重放、prompt、权限选择、稳定 tool update、Session 列表和归一化 Session snapshot。`backend/tests/test-acp-session-config-recovery.ts` 验证未修改 Session 保留 Agent Home 默认，而显式 boolean 配置能跨 live reconnect、SIGKILL 形态冷恢复以及后续磁盘默认变化双向保持。`backend/tests/test-acp-runtime-hibernation.ts` 验证带 guard 的进程树关闭、保留 transcript、权威唤醒、休眠/唤醒竞态、revision fence 和唤醒后 Prompt 活性；`backend/tests/test-agent-manager-acp-hibernation.ts` 验证大量 working/idle Agent 不会因数量被回收、working Agent 会立即拒绝而不是排队、显式资源压力只回收一个准确 idle runtime、冷恢复没有隐藏的数量淘汰，并继续覆盖唤醒、zombie、capability token、主 Agent/固定 Agent 与进程身份 guard。`backend/tests/test-agent-capability-mcp.ts` 与 `backend/tests/test-agent-capability-tokens.ts` 验证共享 HTTP MCP 边界、canonical Workspace 身份、真实 Tool 身份路由，以及准确的 Agent/能力/Workspace 隔离。`backend/tests/test-acp-checkpoint-store.ts` 验证 reducer round-trip、identity 隔离、dirty fence 和持久化原子替换；`backend/tests/test-acp-checkpoint-recovery.ts` 验证冷/热场景 fail-closed load、revision reset、完整子 Session/patch fence 序列化、不确定失败 fencing、bare-resume fencing、工作区校验和 Agent Home 隔离。

`backend/tests/test-provider-adapters.ts` 验证每个 provider 在 Terminal/ACP 下的 Conversation Fork 策略、worktree 范围、实时 capability 要求和兼容投影；`backend/tests/test-code-agent-capabilities.ts` 验证浏览器动作读取同一份声明，包括 ACP runtime 协商，以及只允许同 worktree Fork 的 provider。
