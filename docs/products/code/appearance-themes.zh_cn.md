...
- 纸张是适合长时间阅读的显式浅色方案。工作台画布、框架、阅读面、Composer、
  ...
  墨色与中性色块，不增加装饰性描边；键盘焦点使用克制但清晰可见的墨色焦点环。绿色只
  保留给成功状态、数据可视化等确实承载语义的场景，其他状态色也继续保持自身语义。
...
- 常驻左侧导航在所有外观中都使用同一套中性 Surface 语言表达键盘焦点、鼠标 Hover 与
  ...
  文字、图标和操作入口保持可见，不得再叠加有色外框或焦点阴影。表单输入框和模态控件
  不属于该导航契约，继续使用各自有边界的焦点反馈。
...
- 工作台区域应保持可辨识，但不能变成割裂的色块。Paper 的面板和组合控件使用一档
  轻微的中性色块，而不使用装饰性描边或选中边界。键盘焦点环属于功能反馈，必须保持
  清晰可见；文字、光标、背景和其他交互状态提供其余反馈。语义状态色仍可在内容内部
