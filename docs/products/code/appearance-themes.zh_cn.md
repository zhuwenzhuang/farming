...
定量图表使用独立的 `--code-viz-series-1` 到 `--code-viz-series-6` 色板，不复用 UI 强调色或
成功/错误色。各主题保持蓝、赭黄、青绿、紫、玫红、灰蓝的系列身份。Dark 提升亮度并控制
饱和度；Paper 使用克制的墨色，不把数据系列变成黑白。主要图形相对画布至少保持 3:1 对比度，
用标签和正负号辅助识别，数值继续使用文字色。没有业务好坏含义的增加/减少默认使用蓝/赭黄。

可视化结构边线独立使用 `--code-viz-border`，因此 Paper 隐藏工作台装饰边框时，
不会同时隐藏图表坐标轴。
...
- 纸张使用纯色层次，而不是纹理滤镜。重复颗粒或全局透明效果会降低代码可读性，
  不属于该主题。
- Composer 在桌面和紧凑布局中均保留与正文画布区分的轻微输入区底色。聚焦与打开菜单
  不改变这一底色；移动端覆盖规则不能让输入区背景与正文融为一体。
