...
Project 与全局目录读取使用增量枚举，最多接纳 4,096 个可见条目，与 Decoration
协议的条目预算一致。超大目录在创建元数据任务前明确返回 `TOO_LARGE`，不能把部分
列表作为成功快照。元数据读取的并发量有界；取消和超时停止枚举与后续元数据调度、
关闭目录句柄，并隔离迟到结果。失败保留已有快照；用户可以打开子目录或搜索具体文件，
超大目录读取不能阻塞其他操作。
已展开子目录的失败显示在对应目录行上，避免读取被拒绝时看起来像空目录；根目录失败仍显示在 Files 状态区域。
超大目录失败提供限定到该已授权目录的搜索入口。搜索不会遍历文件系统根目录，
也不会把被拒绝的列表变成部分成功快照；退出搜索后恢复普通 Project 搜索。
