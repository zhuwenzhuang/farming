...
编辑器上下文菜单属于打开它的文件和源码视图。菜单立即打开；异步 Git 能力查询只能更新
可用操作，不能重新打开已关闭菜单或覆盖更新的菜单。切换文件或视图、卸载组件时，菜单和
尚未完成的操作都失效。剪贴板操作完成后，不能修改导航后的另一个编辑器模型。

剪切必须在确认写入剪贴板成功后才删除选中文本。写入失败保留文档，并使用共享行内错误提示；新的操作或文件／视图切换清除提示。

Git blame 的行号栏和作者/日期注释使用同一个菜单，加载中或失败后仍可隐藏 Blame。
检查可用性时保留禁用的检查中菜单项；检查失败显示明确的重试入口，不能把请求失败当作
文件不支持 Blame。只有后端明确返回不支持时才移除该入口。
注释行始终裁剪在编辑器视口内，并避开 Monaco 顶部固定代码行；裁剪范围跟随固定代码行的实际尺寸变化，即使它晚于滚动事件出现。不能覆盖面包屑、预览区域
或 blame 详情。详情遵循共享的外部指针和 Escape 关闭协议。

Blame 反映已保存的工作文件。存在未保存修改时，暂停注释并明确提示保存或撤销，避免把旧
行归属标到已变化的草稿上。保存、撤销和切换文件后刷新当前注释，并清除旧详情。已隐藏、
被更新请求取代或已卸载的请求不能再发布结果。加载必须有界地成功或失败；失败提示保持
可见，并允许重试或隐藏 Blame。验收覆盖上述状态转换、延迟能力/数据响应、未跟踪文件、
滚动、折叠、固定代码行、调整尺寸，以及 Light、Dark、Paper 三种外观。
...
直接忽略。恢复该界面不会关闭用户在加载过程中打开的紧凑侧栏；显式激活目标或关闭
侧栏仍会收起它。
...
单栏 Markdown Preview 默认使用居中的 860px 阅读面，并包含 36px 水平 Padding。宽屏布局把
该阅读面扩展到 Preview Panel 的全部可用宽度，同时保留相同 Padding。Browser 在当前
`FileEditorPane` 生命周期内按 `workspaceEditorModelKey` 记住此选择。Source、Split，以及两种
布局实际等宽的窄布局会隐藏该控件，但不清除选择。宽度重排需要保留当前阅读锚点；大型虚拟化
Preview 按布局分别保存测量值，使有界 DOM 与连续滚动保持不变。
...
历史仓库选择器使用共享 Sidebar 变体，与文件区行共享高度与文字规格。

工作区变更与 Git 历史位于 Files 内。单仓库不显示“主仓库”包装层；存在子模块时，变更下
按仓库分组，各仓库独立折叠，其下区分已跟踪和未跟踪列表。展示路径相对所属仓库，文件
身份仍相对 Project。仓库 Review 审阅已跟踪变更；展开未跟踪列表后提供明确的审阅入口，
不附加隐含的时间过滤。目录树仍是一棵 Project 树；子模块目录与其变更分组使用相同的次要类型标识。

历史在 Project 内选择一个仓库，按有界分页加载。展开提交后显示文件与父提交对比，不实现
第二套差异查看器。后端验证所选子仓库已初始化且位于授权 Project 内，子仓库消失时不能
改读父仓库。切换仓库取消旧请求、清除旧结果、保留历史展开状态；只有当前仓库可以提交
结果。失败结束加载并等待显式重试；刷新重新发现仓库。

后端拥有仓库清单、错误及各类别完整性，包括共享响应上限造成的遗漏。浏览器独立记忆
各仓库和列表的展开状态，刷新不能重新打开已收起的分组。数量表示已返回条目，仅受影响
的类别标注部分结果。仓库错误在对应仓库显示；发现或请求失败只在变更边界显示一次。
收起时使用紧凑状态标记，展开不完整列表后说明限制并提供所选范围的 Review；Review
仍保留明确的捕获容量限制。
