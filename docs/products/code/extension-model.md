...
Language Server is a built-in, viewing-oriented plugin over a user-managed VS Code Bridge. Farming auto-discovers a Bridge already running inside a VS Code extension host and proxies only hover, navigation, symbols, call/type hierarchy, and diagnostics into the existing Monaco editor. VS Code and its language extensions retain all provider configuration and process lifecycle ownership; Farming installs and starts neither the Bridge nor a language server, and it exposes no manual transport or per-language form. Connection state is authoritative and explicit (`Connected`, `Unavailable`, or `Error`), Project-root checks apply on both sides of the Bridge, and dirty Farming drafts do not receive stale saved-file semantics. See [Language Server](./language-server.md).
