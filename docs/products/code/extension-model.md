...
The authenticated Viewer proxies the runtime's session-scoped WebSocket stream. Its toolbar identifies the owning Agent as the other participant in shared control. Frames use high-quality JPEG to keep text readable without sacrificing interactive response. Viewport, pointer, wheel, keyboard, and text input return through the same Session. The Viewer paints at the frame's reported CSS dimensions and discards superseded frames when a client is slow. Agent commands and human input therefore operate the same Browser identity without Farming carrying a second raw-CDP action path.
...
Language Server is a built-in, viewing-oriented managed capability. Farming matches a saved file through an OpenCode-derived language registry, discovers the nearest Project root marker, prefers the standard server command on `PATH`, and lazily reuses one stdio process per server and root. Missing clangd or JDTLS may be prepared in the exact Config cache; other missing commands fail explicitly. Farming exposes no manual transport or per-language form, applies authoritative Project-root checks to inputs and results, and withholds semantic actions for dirty Farming drafts. See [Language Server](./language-server.md).
...
The MVP intentionally uses one operations implementation: the pinned `agent-browser` command and stream protocols, reached through a system-browser executable or the private loopback CDP endpoint of the Agent's Computer. User-supplied external-CDP settings remain read compatibility paths only. Its structured Agent surface covers lifecycle including explicit close, navigation and waits, bounded snapshots, parameterized screenshots, deterministic viewport and device emulation, DOM interaction, inspection and JavaScript, console/error/network diagnostics and interception with Project-scoped HAR export, cookies/storage, frames/dialogs, and Project-scoped upload/download. Computer Use remains the separate full-desktop capability for native browser chrome, dialogs, and arbitrary Linux applications; its Desktop Viewer and the Browser Viewer observe the same isolated Chromium when that source is selected.

Each Browser has a durable unique id, an Agent owner, and a Project root used for file isolation. The Agent menu offers **Create Browser** and, while no Desktop exists, **Create Isolated Desktop**; the Agent row shows its Resources control only after at least one Browser or Desktop exists. In the sidebar these Resources are hidden by default under **Agent → Resources → Browsers / Desktops**; empty Browser or Desktop sections are not rendered, and expanding or collapsing that hierarchy never changes runtime or Viewer state. A stopped, failed, starting, recovering, or stopping Browser row has no status dot; only a running row shows one green dot aligned at the far right. The Isolated Desktop remains present after Browser Stop or Delete. A Browser may be opened directly with the `browser` URL query parameter. Deleting a system-browser row stops its exact runtime before removing its isolated profile. Deleting an isolated row closes only its tab/Session; deleting the Agent owns exact Desktop removal. A legacy external-CDP row closes only Farming-created targets.
...
### Bounded Browser Runtime Recovery

A lost Browser stream is not the same fact as a dead Session, so Farming resolves the difference by exact identity instead of by the age of an event. The backend Browser manager owns the outcome; the runtime only reports transport facts. Every stream socket carries a monotonic stream generation, every Session carries its `sessionId` plus `sessionGeneration`, and every daemon is identified by its verified process identity, so a late close, frame, or exit from a superseded generation can never mutate the current one.

When a ready stream closes while Farming still owns the Session, each running Resource enters an explicit `recovering` state that the sidebar, the Viewer, and Agent reads all see; the lifecycle therefore becomes `running -> recovering -> running | failed`, and `recovering -> stopping -> stopped` when an explicit stop wins. Recovery is bounded and ordered:

1. every in-flight and queued mutation, including queued Viewer pointer, wheel, key, and text input, fails immediately as an uncertain outcome. Farming never replays a Browser action, because the page may already have applied it. New Agent actions are rejected with an actionable recovering error rather than queued;
2. if the exact daemon process is still alive, Farming re-attaches one new stream socket within a bounded step, re-selects the streamed tab, and reconciles tabs from authoritative daemon state. A tab that closed during the outage becomes a stopped Resource, a tab whose page moved refreshes its URL and title, and a tab whose admission cannot be proven is not adopted;
3. if the daemon is confirmed gone, or the re-attach fails or times out, a Session whose Chromium process Farming owns may restart at most once. The restart closes the superseded runtime, refuses to race a daemon that still holds the profile, and restores only the Resources that still belong to that Session, each from its own stored URL, under a new Session generation and new Resource generations so stale Viewer input and stale frames are rejected. A replacement that finishes after the recovery deadline is fenced from committing state. A CDP-backed or external Session keeps reconnect-only recovery, because Farming does not own that browser process;
4. otherwise every surviving Resource of that Session fails visibly with an actionable message, and the Session releases its runtime, isolated lease, bindings, and Viewer state exactly once.

Stopping or deleting the last Resource, dispose, and Agent-lifecycle teardown cancel Session recovery at admission time and always win: a cancelled recovery may not commit another transition. Stopping one tab in a shared Session claims only that Resource while recovery continues for the remaining tabs. A Farming restart treats a persisted `recovering` row as an interrupted runtime that must be cleaned up by exact Session identity.
