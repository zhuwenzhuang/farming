...
Both sources use the same exact, version-locked `agent-browser` runtime. Before the Server opens its port, startup preparation resolves a matching system installation or downloads and verifies the pinned platform artifact into Farming's immutable cache. For a local Resource, Farming gives an installed Chromium executable and isolated profile to `agent-browser`; there is no separate Farming Chromium launcher. For an external Resource, `agent-browser` connects to the configured loopback endpoint and creates one labeled tab. Farming does not access Docker, manage containers, or ship Chromium.
