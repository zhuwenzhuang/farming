...
Language Server is a built-in, viewing-oriented managed capability. Farming matches a saved file through an OpenCode-derived language registry, discovers the nearest Project root marker, prefers the standard server command on `PATH`, and lazily reuses one stdio process per server and root. Missing clangd or JDTLS may be prepared in the exact Config cache; other missing commands fail explicitly and may use the existing user-managed VS Code Bridge as a compatibility path. Farming exposes no manual transport or per-language form, applies authoritative Project-root checks to inputs and results, and withholds semantic actions for dirty Farming drafts. See [Language Server](./language-server.md).
