...
Resume 在指定 Home 中读取历史，并在精确身份的串行准入中复用这次新鲜读取。
Codex 精确查询直接读取匹配的 rollout，不受最近历史窗口限制；仅存在于索引中的
条目只用于发现，不足以证明会话仍然可用。

显式 Resume 只有在持久归档记录证明旧运行时已停止、且新读取的 Provider 历史
可用并未归档时，才能取消阻塞的 Archive。保留在内存中的运行时还必须已确认
退出、没有 ACP 绑定，且没有进行中的生命周期操作。准入等待恢复完成，Archive
与 Resume 使用同一精确身份的变更队列。读取、取消归档或持久化失败都会保留
阻塞。只读状态检查显示原始生命周期错误；结果不确定的请求不会自动重放。
