...
An existing history session uses `session/load` by default. This matters because ACP load replays the complete conversation through `session/update` notifications before the load request returns. Explicit `resume` reconnects the context without replaying old messages. Farming registers the session reducer before sending load so early replay notifications cannot be lost. Replay updates reduce into the ordered stream without broadcasting one browser invalidation per entry; clients receive one settled snapshot after restoration, so long rich transcripts do not repeatedly unmount while loading.
...
The same controls also update a live native Codex Terminal instead of changing only its next launch profile. Farming applies the change immediately through the CLI's interactive `/model` selection and version-compatible `/fast` toggle. It waits for the actual model and reasoning menus, selects the advertised entries, and confirms the resulting footer before the Composer can submit another message; it does not guess at remote-render timing. New Terminals receive an explicit `service_tier="default"` when Standard is selected, so a Fast value in the user's Codex config cannot make the runtime disagree with Farming's control. The launch profile is persisted after the live Terminal confirms the change, keeping a restarted Agent consistent without showing an unverified selection. These PTY commands are never injected into ACP, legacy JSON, App Server, shell, Claude, OpenCode, Qoder, or other non-Codex Terminal sessions.
