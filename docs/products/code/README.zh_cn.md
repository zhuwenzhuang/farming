# Farming Code

> English version: [README.md](./README.md)

Farming Code 是默认的浏览器工作区。用户使用说明统一在公开文档站维护：

- [概览](https://zhuwenzhuang.github.io/farming/cn/code/overview)
- [项目与 Agent](https://zhuwenzhuang.github.io/farming/cn/code/projects-and-agents)
- [Chat](https://zhuwenzhuang.github.io/farming/cn/code/chat)
- [Terminal](https://zhuwenzhuang.github.io/farming/cn/code/terminal)
- [Files](https://zhuwenzhuang.github.io/farming/cn/code/files)
- [搜索与 History](https://zhuwenzhuang.github.io/farming/cn/code/search-and-history)
- [手机与远程使用](https://zhuwenzhuang.github.io/farming/cn/code/mobile-and-remote)

面向维护者的长期契约继续保留在本目录：

- [ACP Runtime](acp-runtime.zh_cn.md)
- [Codex Runtime](codex-runtime.zh_cn.md)
- [Agent List 状态协议](agent-list-state-protocol.zh_cn.md)
- [Terminal 状态协议](terminal-state-protocol.zh_cn.md)
- [Extension 与 Resource 模型](extension-model.zh_cn.md)
- [Project Files 设计](project-files-section-design.zh_cn.md)
- [分享访问模型](share-access-model.zh_cn.md)
- [Review 基础](review-foundation.zh_cn.md)
- [Farming Desktop](desktop-app.zh_cn.md)
- [Language Server](language-server.zh_cn.md)
- [验收与 Dogfood 计划](test/acceptance-dogfood-plan.zh_cn.md)

这些文档描述状态所有权、转换、失败、恢复与验收标准，不再重复普通用户手册。
