...
每个受支持 Agent 也会在 Terminal 进程或 ACP Session 的创建与恢复边界收到同一段简短的 Farming 启动提示，但 Farming 不修改 Project 或 Provider 自己的 Instruction 文件。这段提示说明 Farming 把 `agent-browser` 封装成 Agent 的结构化操作入口和用户可见的共享 Viewer，让 Agent 更好地操作网页，也让用户在 Farming 中理解进展并随时接管。提示要求 Agent 通过 `"$FARMING_CLI_BIN_DIR/farming"` 这一实例精确入口查询 Capability，不依赖登录 Shell 可能重排的 `PATH`，也不能假设能力存在；Agent 先判断信息源：当前项目的代码、构建和本地状态用仓库 CLI 与文件工具，公开资料、竞品、行业事实和外部产品行为用原生结构化能力（例如 Web Search）或已授权的服务专用 Connector；仅仅出现仓库内的关键词不是本地搜索的理由。公开资料和竞品调研先确定比较维度与代表对象；默认选择 3 个语义不同的代表产品，效率硬规则是搜索、打开和页内查找总计至多 6 次，第 6 次外部读取后必须依据已有证据作答；只有用户明确要求、或直接冲突的额外证据会改变结论类别时，才扩大预算；原生搜索支持时批量发起独立查询，只为关键结论打开必要的第一方页面，能够说明是否支持、用户如何操作、对已有和新数据如何生效后就收敛；缺失或冲突的证据必须明确保留，不以低价值串行搜索填满答案。Browser Resource 用于网页交互、登录/表单、视觉或控制台/网络检查、用户复查或接管，不用于普通检索或静态阅读；确实需要该路径且 Browser 可用时，Agent 使用 Farming Browser，并将最终可复查状态留在用户可点击的 Browser Resource 中。Computer 只用于 CLI、结构化服务工具和 Farming Browser 都不能完成的桌面专用界面、浏览器工具栏、权限弹窗或非网页应用。Terminal Agent 用 Browser `list` 发现自己拥有的 Resource；Browser 顶层 Help 只披露起点，`help workflow` 给出标准流程，Topic Help 展开单个能力域，最后由 Command Help 披露精确参数。`farming-browser` 继续作为 npm Bin 别名，`farming browser mcp` 则是 Provider Adapter 和显式外部调用方共用的标准 stdio 入口。完整支持合同见 [Farming Browser Agent CLI](./browser-agent-cli.zh_cn.md)。
