...
Home 卡片复用 Project 和 Agent 的插入线样式：上半区／下半区分别表示插入前方／后方，
包括第一项之前和最后一项之后。离开目标、移到自身或其他 Provider、没有合法目标时释放、
以及取消拖动，都会清除反馈而不保存。合法释放通过现有配置所有者保存一次；保存期间禁用
调序，结果不确定时通过读取核对，不重放修改。方向键仍在组内移动一步。
