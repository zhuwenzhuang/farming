...
Extension 会发现兼容的系统 Chrome、Brave、Edge 或 Chromium 可执行文件，以 Headless 模式启动，并通过 WebSocket 直接连接原生 CDP。Farming 不携带 Chromium，Extension 也没有 Playwright 或 Puppeteer Runtime 依赖。`Page.startScreencast` 为受鉴权保护的工作区 Viewer 提供画面；Viewer 的 Pointer、Wheel、Keyboard 与 Resize 消息通过同一个 CDP Target 返回。Agent 操作也使用同一 Target，通过 Accessibility Snapshot、稳定的 Snapshot Ref、Screenshot、Navigation 和 CDP Input 工作。`farming browser` 是第一版 Agent Bridge，npm 安装同时保留 `farming-browser` 别名。
...
- 当前 `farming browser` Bridge 后续应如何通过 MCP、ACP/client-tool Extension 或其他 Farming Capability Transport 投影；
