...
| Verifying | 验证认证 HTTP、版本化 WebSocket、PTY Host、ACP Host，并创建和删除一个全新空 Chat。 | 任一步失败都进入回滚，不重放结果不确定的 mutation。 |
...
5. stop、smoke 清理、rollback 和 retention 都以确切 Config、Agent 和 image 身份为目标；
