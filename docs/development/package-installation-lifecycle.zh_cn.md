...
每个 Managed Image 拥有自己的固定版本 Node.js、npm 和兼容库。Node.js 与 npm 的版本声明
属于安装器元数据，不属于应用依赖，普通 npm 安装不会下载它们。目录安装器在 npm 安装完成后，
将经过校验的引导运行时移入暂存 Image。更新时，同一更新操作先将目标版本所需运行时安装到
私有暂存目录，再执行预检与发布。运行时准备失败不改变当前 Image；重试、所有权、取消和清理
沿用同一更新操作，不在 Image 间共享可变运行时。更新准备、新版启动以及
