# 公开文档站

> English version: [documentation-site.md](./documentation-site.md)

公开文档站是位于 `docs-site/` 下的独立 VitePress 项目。它的依赖、锁文件、构建命令
和 CI 工作流都与 Farming 应用分离；持久架构和维护者契约继续放在 `docs/` 中。

## 内容边界

公开站只描述已经适合解释和支持的用户行为。仍处于草稿阶段的 Surface 不进入站点。
已经实现但缺少代表性真实案例的能力，只能放入实验性功能分组，并在页面首屏显示明确的
实验性标识。

公开站同步发布中文与英文用户文档。中文页面位于 `docs-site/cn/`，英文页面位于
`docs-site/en/`，两种语言都保留语言切换入口；存在对应翻译时，切换语言应保持当前页面
路径。面向用户的内容变更应在同一个提交中同步更新中英文版本，除非该页面明确只适用于
一种语言。

## 展示契约

公开站首先是文档入口，不是产品营销页。首页保持简短并以任务为中心：先用简短的用户视角
介绍 Farming，再提供 Code、CRT、常用能力、插件和工作流入口。产品操作截图放在解释对应
界面的具体页面；首页 Hero 可以使用克制的、随主题变化的品牌视觉。Code 与 CRT 是两个主要
界面；Browser 和其他可选能力属于插件或实验性功能，不得作为同级顶层产品展示。

首屏可以轻量使用 Farming 的暖色视觉，正文阅读区应保持安静和高对比度。优先使用系统
字体、稳定布局、克制的卡片和直接的用户语言；避免自造口号、超大宣传区、客户背书式布局
或重复罗列功能。截图用于解释具体界面或工作流，是文档证据，不能代替正文。

安装命令面板的方式选择、复制按钮和代码共用一层主题背景。紧凑的顶部放置操作控件，
桌面命令使用 15px 等宽字体、26px 行高；小屏使用 14px 字体、24px 行高。代码背景透明，
在窄屏自动折行。环境要求放在面板外。

文档正文中的截图应可在放大查看器中打开。查看器必须支持鼠标和键盘激活、明确的
关闭控件、`Escape`、点击遮罩关闭、关闭后恢复焦点，并在手机上限制在可见区域内。
已经作为链接的图片继续保留原有跳转行为。
页面展示质量 95 的 WebP 衍生图，放大查看器打开原始 PNG。PNG 截图仍是权威素材；
文档站构建时会重新生成 WebP，包括首页插画。README 展示用的 WebP 提交到仓库，
其 PNG 原图变化后运行 `cd docs-site && npm run images:readme` 重新生成。

快速开始应是可执行的编号任务流，而不是功能清单。每个主要步骤都应给出可观察的完成标志，
同时明确保留环境要求、鉴权 URL 处理、Session 持续性和验证要求。正文页面应提供
复制当前页面原始 Markdown 的操作，以及预填公开页面和源文件位置的文档问题反馈入口。

需要分享指定主题时，使用 `?theme=dark` 或 `?theme=light` 查询参数。有效参数应在页面
渲染前生效；URL 已包含主题参数后，手动切换主题与站内导航都应让 URL 继续反映当前外观。
普通 URL 在用户主动切换主题前保持不变。

## 本地命令

```bash
cd docs-site
npm ci
npm run dev
npm run build
npm run preview
```

`npm run build` 是链接和静态构建的权威检查。公开站使用的产品截图放在
`docs-site/public/cn/assets/`，必须使用匿名工作区，不得包含私有 Host、Token 或草稿界面。

在仓库根目录重新生成产品截图：

```bash
npm run docs:product:screenshots
```

需要生成选定界面的深色版本、同时保留暖色原图时：

```bash
FARMING_SCREENSHOT_APPEARANCE=dark \
FARMING_SCREENSHOT_FILES=05-mobile-agent-chat.png,11-code-agent-process.png \
npm run docs:product:screenshots
```

该命令使用匿名、确定性的工作区和 Agent Fixture，对构建后的真实 Farming UI 进行
截图；它会更新产品文档的权威截图，并把公开站所需图片同步到
`docs-site/public/cn/assets/`。不要用手工拼接的效果图替代这些真实截图。
深色截图使用 `-dark` 文件名后缀；Usage activity、Farming CRT 等原生深色界面继续使用
权威文件名，不生成无意义的重复副本。

Farming Browser 的插件与 Viewer 截图由 Browser 专项 E2E 场景生成，仍然来自真实
界面，但暂不由上述主截图命令同步。修改 Browser 截图时，应运行对应的 Browser E2E
截图流程并同时检查公开站引用。

不要把文档站依赖加入根 `package.json`，不要使用 npm workspace，也不要让 Farming
应用构建依赖这个项目。

## 发布

首页 Hero 在快速开始按钮下方提供“npm 安装”与“指定目录安装”两种方式。默认选择 npm，适用于
已有受支持 Node.js 的主机；用户目录安装为缺少 Node.js 或受支持的旧 Linux 主机提供
私有运行环境。每种方式展示可复制的两行命令：先安装，再用 Farming CLI 显式启动。
切换方式会清除复制反馈；剪贴板失败时保留所选命令，允许手动复制。两种方式均不配置用户 Shell 环境。

文档构建将 `bin/install.sh` 原样复制到
公开的 `install.sh` 地址；脚本只有一份源文件，也包含在 npm 包中。首次发布此入口前，
需要先发布携带私有运行时依赖和 Linux 兼容库的 npm 版本。npm latest 宣告支持安装器前，
部署保持现有官网；Publish Release 工作流成功后，会再次触发文档构建和部署。

`.github/workflows/docs.yml` 从 `main` 构建文档，并把静态产物部署到 GitHub Pages。
仓库的 Pages Source 需要设置为 GitHub Actions。项目站默认 Base Path 为 `/farming/`；
未来使用独立域名时，可以通过 `FARMING_DOCS_BASE` 覆盖。
Canonical 与 Sitemap 的站点 Origin 可以通过 `FARMING_DOCS_ORIGIN` 覆盖。

发布页面应输出 Canonical URL、对应的 `zh-CN` 与 `en-US` 替代地址、
`x-default` 入口，以及本地化的 Open Graph 和 Twitter 元数据。社交分享元数据必须
使用公开的 Farming 图片，不得引用本地、私有或仅限草稿的资源。
