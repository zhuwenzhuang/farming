...
<ThemeImage light="/cn/assets/chat.png" dark="/cn/assets/chat-dark.png" paper="/cn/assets/chat-paper.png" alt="结构化 Chat" />
...
<ThemeImage light="/cn/assets/model-controls.png" dark="/cn/assets/model-controls-dark.png" paper="/cn/assets/model-controls-paper.png" alt="模型与运行控制" />
...
已完成回复的操作区提供复制和分享按钮。分享按钮复制一个临时只读链接，并把接收者定位到这一条具体回复；完整权限、失效时间和回退行为见[分享与只读访问](./sharing)。
