...
复制的链接采用 `/farming/s/<code>`，只带 22 位随机码，不在分享 URL 中展开凭证和定位参数。原有效期内可供多人重复打开，服务重启后也有效；关闭分享面板不会让已复制的链接失效。
...
点击侧边栏顶部的“分享当前页面”按钮会同时执行两件事：复制当前页面的只读短链接，并打开二维码面板。使用另一台设备的相机或系统二维码扫描器对准二维码即可打开 Farming；Farming 本身不需要调用当前设备的摄像头。
