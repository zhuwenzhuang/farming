...
Browser Resource 模块位于 `extensions/browser`，默认关闭；Agent Tool 和 MCP 挂载仍然按需进行。它只有一套原生 CDP 操作与 Viewer 实现，但有两个显式连接来源：启动兼容的 Chromium 系系统浏览器，或者通过 `FARMING_BROWSER_CDP_URL` 连接由用户或 Agent 管理的外部浏览器。外部 Endpoint 只允许回环地址；浏览器在另一台机器时由用户自行建立 Tunnel。Farming 不得访问 Docker Socket、选择或拉取镜像、管理容器、向客户端暴露 Endpoint，也不得关闭外部 Owner 的浏览器进程。每个外部 Resource 只创建和关闭自己的页面 Target；浏览器、容器、Profile 与 Endpoint 生命周期由外部 Owner 负责，重启恢复只把中断的 Row 标记为 Failed，不尝试终止外部浏览器。系统浏览器 Resource 继续使用独立 Profile 和精确进程组清理。Settings 根据实际来源显示“系统浏览器”或“外部浏览器”；两个来源都不可用时说明要求，不显示开关或 Project Browser 分组。不得加入 Playwright、Puppeteer、随包 Chromium、WebDriver、第二套操作实现或静默连接 Fallback。过期 Viewer Generation 必须被拒绝。Codex、Claude Code、OpenCode 和 Qoder 启动时从 `backend/farming-agent-bootstrap.zh_cn.md` 注入 Farming Bootstrap，并通过 `farming capabilities` 查询实时能力；不得自动挂载 Browser MCP。`farming browser` 是按需操作同一 Browser Identity 的 Agent Bridge，`farming browser mcp` 只允许显式启用，`farming-browser` 只是 npm Bin 别名。
