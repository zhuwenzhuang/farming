...
Browser Resource 模块位于 `extensions/browser`，默认关闭。Browser 在 ACP Session 创建边界已启用时，Farming 会通过 Provider Adapter 自动投影完整的 `browser_*` MCP Tool Catalog；Terminal 仍通过 CLI 按需访问。它唯一受支持的操作与 Viewer Runtime，是 Farming 启动依赖 Manifest 声明的精确版本 `agent-browser`。Farming 不得用原生 CDP 重写 Browser Automation，不得把 Playwright 或 Puppeteer 加入生产包，不得把 Chromium 随发行包交付，也不得加入 WebDriver 或保留静默的第二套实现。全新 Server 打开端口前，Farming Launcher 必须准备并使用已校验的不可变 `agent-browser` Cache 条目。Chromium 是独立的显式可选依赖：普通安装、更新和启动绝不能下载它。**插件 → 浏览器**只向普通用户展示“自动”“已发现的本机 Chromium”和“隔离浏览器”，不暴露原始 CDP 配置；自动模式优先使用兼容的本机 Executable，否则使用已经准备好的隔离运行时。准备隔离浏览器必须由用户显式触发，只能拉取经过审查的上游 `trycua/cuabot` 精确镜像 Digest，并且主 Registry 失败后只能尝试代码中审查过、指向相同字节的镜像 Registry；Farming 不维护自己的 Browser Image。隔离运行时按 Config 与 Browser Owner 管理带精确 Label 的容器，只把 CDP 发布到回环地址，再把这个内部 Endpoint 交给同一个锁定版本的 `agent-browser`。Browser Stop 必须先关闭 Tab 和 `agent-browser` Session，再停止精确容器；Browser Delete 才删除它。Server 重启时，Browser Recovery 之前必须先停止已经证明属于本实例的遗留隔离容器。容器复用、停止和删除前必须同时校验 Config、Browser Owner 与镜像 Digest。旧的受管 Chromium 和用户提供的外部 CDP 设置只保留读取兼容，不再是主要 UI 选项。切换可见来源时，必须先校验目标配置并停止全部运行中的 Browser，清理成功后才提交设置；失败时保留旧选择。同一 Agent 与 Browser Source 下的 Resource 是共享 `agent-browser` Session 中的带标签 Tab。过期 Viewer Generation 必须被拒绝；受鉴权保护的 Viewer 代理 Runtime 的 JPEG WebSocket Stream，并把 Pointer、Keyboard、Wheel 与 Viewport Input 映射回同一个 Session。Browser Action 与 Runtime Command 都必须串行。支持的 Agent Surface 覆盖导航/等待、DOM 交互、结构化检查/JavaScript、Console/Error/Network 证据、Cookie/Storage、Frame/Dialog 和 Project 级 Upload/Download。Codex、Claude Code、OpenCode 和 Qoder 通过 Farming Bootstrap 与 `farming capabilities` 发现实时能力；已启用的 Browser Capability 自动投影到新 ACP Session，`farming browser` 与 `farming browser mcp` 仍操作同一 Browser Identity。

隔离浏览器会并行做有界 Registry Probe，再按实测延迟尝试候选，但所有候选都必须指向同一个已审查 Digest。标准 Docker Daemon 镜像加速配置（包括账号级阿里云加速器）属于部署环境，不写成 Farming 自己的镜像地址。当前 Chromium 在容器内只把 CDP 绑定到回环地址，因此 Provider 使用内部 Relay，并且仍然只把 Relay 发布到宿主机回环地址。隔离 Browser Container 与完整 Computer Desktop Container 是两台不同容器，因为经过审查的完整桌面镜像不包含 Chromium；它们共享的是 Computer Extension 的 Docker Ownership Boundary。旧 Linux 上只有 `agentBrowser` 启动依赖切换到同一个精确 npm Package 中的静态 musl Artifact，Codex 与 Claude 仍使用正常平台 Artifact。Active Dependency Record 必须记录每个依赖实际选中的 Platform Key，防止清理误删正在使用的 musl Cache。
...
实验性的 Computer 模块位于 `extensions/computer`，默认关闭，并统一拥有 Farming 的 Docker 边界。完整桌面 Resource 使用 Farming 锁定的精确上游 `trycua/xfce-cua` Image Digest 与 Cua Driver 版本；同一 Extension 内的隔离 Browser Provider 只把锁定的上游 `trycua/cuabot` 镜像作为 Chromium/CDP 基础设施。Farming 不得维护自己的桌面或 Browser 镜像，也不得静默替换其他镜像或 Driver。镜像拉取只能由用户显式执行准备操作，绝不能成为安装、更新或启动依赖。旧 Docker seccomp 兼容模式只允许在明确的 Probe 失败后由用户显式启用。一个稳定 Agent Record 最多拥有一个 Computer Resource 和一台精确带 Label 的 Docker Container；不同 Agent 绝不能共享 Desktop、Driver Session 或 Viewer Credential。Chat/Terminal 切换保留 Computer；Agent 停止或归档会停止但保留它；删除 Agent 会删除经过精确校验的 Container 与 Resource；非活跃或已归档 Agent 不能重新启动 Computer。Server 不向 Container 暴露 Docker Socket，noVNC 只发布到回环地址，再通过带鉴权 Route 代理。Agent 在 ACP Session 创建边界只能使用锁定版本完整细粒度 `computer_*` Catalog 中声明的 Tool，Terminal 则使用渐进披露的 `farming computer` CLI；任意上游 Cua Tool 名称必须在进入 Docker 前失败。人工接管与 Agent 控制是互斥 Epoch；控制权切换先关闭新 Tool 准入，排空已接收的有界 Action，再推进 Epoch 并关闭过期 Viewer。交还 Agent 后必须获取新的 Desktop、Browser、Window 或 Accessibility Tree 状态观察才能继续修改，普通 Metadata 读取不能解除该栅栏。Action 串行执行；Stop、Delete 与 Reset 先关闭新的 Start/Tool 准入，并保持栅栏直到精确 Container 删除完成；过期 Generation 明确失败；超时代表结果不确定且绝不自动重放。
