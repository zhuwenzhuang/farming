...
Browser Resource 模块位于 `extensions/browser`，默认关闭；Agent Tool 和 MCP 挂载仍然按需进行。它唯一受支持的操作与 Viewer Runtime，是 Farming 启动依赖 Manifest 声明的精确版本 `agent-browser`。Farming 不得用原生 CDP 重写 Browser Automation，不得把 Playwright 或 Puppeteer 加入生产包，不得随包携带 Chromium，也不得加入 WebDriver 或保留静默的第二套实现。全新 Server 打开端口前，Farming Launcher 必须始终准备并使用已校验的不可变 `agent-browser` Cache 条目，不得复用或回退到系统安装。Local Resource 由 Farming 把已安装且兼容的 Chromium Executable 与隔离 Profile 交给 `agent-browser`；Farming 不再保留独立的 Chromium Launcher 或 Process Gate 实现。External Resource 则由 `agent-browser` 连接用户或 Agent 通过 `FARMING_BROWSER_CDP_URL` 管理的 Endpoint。外部 Endpoint 只允许回环地址；浏览器在另一台机器时由用户自行建立 Tunnel。Farming 不得访问 Docker Socket、选择或拉取镜像、管理容器、向客户端暴露 Endpoint，也不得关闭外部 Owner 的浏览器进程。每个 External Resource 只拥有自己的带标签 Tab 与 agent-browser Session；浏览器、容器、Profile 与 Endpoint 生命周期归外部 Owner。过期 Viewer Generation 必须被拒绝；受鉴权保护的 Viewer 代理 Runtime 的 JPEG WebSocket Stream，并把 Pointer、Keyboard、Wheel 与 Viewport Input 映射回同一个 Session。Codex、Claude Code、OpenCode 和 Qoder 启动时从 `backend/farming-agent-bootstrap.zh_cn.md` 注入 Farming Bootstrap，并通过 `farming capabilities` 查询实时能力；不得自动挂载 Browser MCP。`farming browser` 是按需操作同一 Browser Identity 的 Agent Bridge，`farming browser mcp` 只允许显式启用，`farming-browser` 只是 npm Bin 别名。
