...
发行包携带审阅过的精确 Codex 与 Claude ACP JavaScript Runtime，但不得携带 `@openai/codex-*` 或 `@anthropic-ai/claude-agent-sdk-*` 传递引入的平台 Agent CLI。Codex CLI、Claude Code CLI 与 `agent-browser` 都是启动依赖，而不是 Package 依赖。一份随版本检入的 Manifest 固定每个支持平台的 Artifact、来源 URL、Integrity、Executable Entry 与预期版本。全新 Launcher 只复用报告精确版本的系统 Executable；否则必须在 Server 打开端口前完成下载、校验、Staging 和不可变 Cache 条目的原子启用，并通过 `CODEX_PATH`、`CLAUDE_CODE_EXECUTABLE` 与 `FARMING_AGENT_BROWSER_BIN` 交给运行时。并发启动共用一把准备锁；损坏 Cache 不得成为 Active；准备失败必须让 Server 保持未启动并返回可操作错误。npm 与 Bundle 更新必须在旧 Server 仍运行时，执行新版本自己的启动依赖准备；只有成功后才可进入 `ready-to-restart`，失败时旧 Package 与旧 Server 都保持不变。Package Smoke 必须证明 Release 中不存在平台包时，两份 Vendored Adapter 仍能完成 ACP `initialize`。
