...
基于系统浏览器的 Browser Resource 模块位于 `extensions/browser`，默认关闭；Agent Tool 和 MCP 挂载仍然按需进行。系统没有兼容的 Chromium 系浏览器时，Settings 需要明确提示所支持的浏览器类型，但不展示开关，Project Browser 分组也保持隐藏。系统浏览器可用时，设置项名称使用“系统浏览器”，不得把浏览器本身称为“内置浏览器”或“共享浏览器”。关闭前必须先停止全部受管 Runtime；无法证明清理完成时保持启用并显式失败。每个 Project 可以拥有多个 Browser Identity；每个 Identity 只有一个独立 Profile 和一个串行生命周期 Owner。MVP 唯一支持的实现会以 Headless 模式启动兼容的 Chromium 系系统浏览器，并同时使用原生 CDP 完成 Agent 操作和受鉴权保护的 `Page.startScreencast` Viewer。不得加入 Playwright、Puppeteer、随包 Chromium、第二套浏览器实现或静默 Attach/Recovery Fallback。系统浏览器启动必须经过 Gate：只有精确 Process Group Identity 已持久化提交后才能真正启动 Chromium；启动恢复只有在证明该精确进程组已经退出后，才可以清空私有 Identity 并把先前 Live 的 Browser Row 标记为 Failed。过期 Viewer Generation 必须被拒绝，重启恢复不能认领未经证明的进程。Codex、Claude Code、OpenCode 和 Qoder 启动时从 `backend/farming-agent-bootstrap.zh_cn.md` 注入 Farming Bootstrap，并通过 `farming capabilities` 查询实时能力；不得自动挂载 Browser MCP。`farming browser` 是按需操作同一 Browser Identity 的 Agent Bridge，`farming browser mcp` 只允许显式启用，`farming-browser` 只是 npm Bin 别名。
...
- 更新行为必须识别安装方式。npm 安装读取 `farming-code` registry 元数据并支持一键更新：旧服务运行期间先安装目标版本，安装成功后才重启，进度持久化到 config 目录，新服务启动失败时尝试回退。准备好的 npm 包必须位于受管 package root 的同级目录，保证目录切换不跨文件系统；Helper 在停 Server 前持久化精确身份与 Durable Operation ID，并在第一次 Rename 前持久化精确 Switch Recovery Marker。稳定 Launcher 只有在 Live Root 缺失时才恢复该操作匹配的旧包；重启对账必须先清理 Marker 与 Backup，再发布成功。源码部署先在同级目录完成同步、依赖安装和构建，使用新版本的 Crash-only CLI 停止旧 Server，在新 Server 通过 CLI Readiness 前保留完整旧目录，并用 Durable Activation Phase 恢复中断切换。源码 checkout 通过 Git 更新，单文件 CLI 手动替换。标准 App bundle 的可信 HTTP(S) 目录或 manifest URL 保存为 `settings.updateUrl`，每个 bundle 必须匹配运行平台并提供 64 位 `sha256`。独立的 `linux-x64-legacy-glibc228` tar 是首次安装引导包：安装器只在需要时启用固定校验的 glibc 2.28 runtime，把包内应用安装到私有 `~/.farming/npm` prefix，并生成稳定兼容 launcher。后续应用版本直接走同一 prefix 的 npm 更新；只有兼容 runtime 本身变化时才需要新的引导包。
...
- **browserExtensionEnabled**：默认关闭；控制系统浏览器集成，不代表自动向 Agent 挂载 MCP
