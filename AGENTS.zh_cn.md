...
内置 Browser Resource 位于 `extensions/browser`。每个 Project 可以拥有多个 Browser Identity；每个 Identity 只有一个独立 Profile 和一个串行生命周期 Owner。MVP 唯一支持的实现会以 Headless 模式启动兼容的系统 Chrome、Brave、Edge 或 Chromium，并同时使用原生 CDP 完成 Agent 操作和受鉴权保护的 `Page.startScreencast` Viewer。不得加入 Playwright、Puppeteer、随包 Chromium、第二套浏览器实现或静默 Attach/Recovery Fallback。过期 Viewer Generation 必须被拒绝；重启恢复必须把先前 Live 的 Browser Row 标记为 Failed，不能认领未经证明的进程。`farming browser` 是第一版操作同一 Browser Identity 的 Agent Bridge；`farming-browser` 只是 npm Bin 别名。
