...
- **测试垃圾必须由创建者精确清理**：测试和临时复现创建的目录、Socket、进程与 Fixture，必须通过 `try/finally` 或测试框架拥有的 Teardown 在成功、失败、取消路径中全部清理。清理只能使用该测试创建并保存的精确身份；不得遗留测试垃圾，也不得用扫描并递归删除 `/tmp/farming-*` 等泛化路径的通用清理器补救
...
- **主动查看必须重新确认**：侧栏、计数和路由可以预取后台导航数据；但用户打开一个声称展示当前能力、清单、配置或健康状态的页面时，必须先作废上一次展示、进入有界加载态，并取得新的权威后端结果。缓存只能在权威 Owner 能够证明新鲜度时用于加速该次读取；前端不得把上一次访问的快照当成新请求的答案。请求失败必须明确显示，不得回退到旧快照
- **资源清理**：及时清理僵尸进程和过期数据
...
Computer 镜像可以使用 Docker Daemon 的标准镜像加速配置（包括账号级阿里云加速器）；这属于部署环境，而不是 Farming 自己的镜像地址。Chromium 下载源会进行有界探测并按实测延迟尝试。当前 Chromium 在 Computer 内只把 CDP 绑定到回环地址，因此 Provider 使用内部 Relay，并且仍然只把 Relay 发布到宿主机回环地址。启用隔离 Browser 时必须同时保持 Computer 启用，因为可见的 Computer Resource 是唯一的 Container 与 Desktop Owner。旧 Linux 上只有 `agentBrowser` 启动依赖切换到同一个精确 npm Package 中的静态 musl Artifact，Codex 与 Claude 仍使用正常平台 Artifact。Active Dependency Record 必须记录每个依赖实际选中的 Platform Key，防止清理误删正在使用的 musl Cache。

每个 Farming 页面只使用一条版本化主 WebSocket 承载 Chat、Terminal 以及轻量的 Browser / Computer Resource 元数据。协议协商完成后，Server 发送权威完整 Resource 快照；每次重连都重新发送，之后按 Resource 身份合并并推送带 Revision 的增量。Capability 探测和生命周期 Mutation 继续使用有界 HTTP 请求。Browser JPEG 与 Desktop noVNC 画面必须保持为独立的按需 WebSocket，绝不能进入主控制连接。不得再增加 Browser 或 Computer 专属 EventSource。前台当前状态读取优先于后台导航数据：打开 Terminal 时，必须在请求权威 `/session-view` Checkpoint 前同步取消正在进行的低优先级 Usage 与 Agent Session 预取。
...
实验性的 Computer 模块位于 `extensions/computer`，默认关闭，并统一拥有 Farming 的 Docker 边界。完整桌面 Resource 使用 Farming 锁定的精确上游 `trycua/xfce-cua` Image Digest 与 Cua Driver 版本；隔离 Browser 复用同一台可见的 Agent-owned Computer，并把显式准备的受管 Chromium 只读挂载进去，不再创建第二台隐藏 Browser Container。Farming 不得维护自己的桌面或 Browser 镜像，也不得静默替换其他镜像或 Driver。镜像拉取与 Chromium 下载只能由用户显式执行准备操作，绝不能成为安装、更新或启动依赖。旧 Docker seccomp 兼容模式只允许在明确的 Probe 失败后由用户显式启用。一个稳定 Agent Record 最多拥有一个 Computer Resource 和一台精确带 Label 的 Docker Container；不同 Agent 绝不能共享 Desktop、Driver Session、Viewer Credential、Browser Profile 或 CDP Endpoint。Chat/Terminal 切换保留 Computer；Agent 停止或归档会停止但保留它；删除 Agent 会删除经过精确校验的 Container 与 Resource；非活跃或已归档 Agent 不能重新启动 Computer。Server 不向 Container 暴露 Docker Socket，noVNC 与 Browser CDP Relay 都只发布到回环地址，用户侧访问再通过带鉴权 Route 代理。Agent 在 ACP Session 创建边界只能使用锁定版本完整细粒度 `computer_*` Catalog 中声明的 Tool，Terminal 则使用渐进披露的 `farming computer` CLI；任意上游 Cua Tool 名称必须在进入 Docker 前失败。人工接管与 Agent 控制是互斥 Epoch；控制权切换先关闭新 Tool 准入，排空已接收的有界 Action，再推进 Epoch 并关闭过期 Viewer。交还 Agent 后必须获取新的 Desktop、Browser、Window 或 Accessibility Tree 状态观察才能继续修改，普通 Metadata 读取不能解除该栅栏。Action 串行执行；Stop、Delete 与 Reset 先关闭新的 Start/Tool 准入，并保持栅栏直到精确 Container 删除完成；Browser Session 租用 Computer 时，用户直接 Stop/Delete 必须明确失败；过期 Generation 明确失败；超时代表结果不确定且绝不自动重放。
...
- 更新行为必须识别安装方式。npm 安装以其 Package Root 作为稳定 Bootstrap 身份，完整版本发布为不可变 Package Image，并通过安装级原子 Current Selection 选择新启动使用的版本。每个运行中的 Config 固定使用启动时的精确 Image。一键更新在旧 Server 存活时准备并验证目标版本，原子切换 Selection 后只重启发起更新的 Config；其他 Config 继续运行旧 Image，直到它们自行重启或更新。Selection 使用 Compare-and-swap，回退从发起 Config 的精确旧 Image 重启；清理必须保留 Current、Previous、近期和所有精确存活实例使用的 Image。用户之后显式用 npm 替换 Bootstrap Package 时，新安装版本成为后续启动的 Selection，但不会改写 live Image。Server 启动时必须用权威的实际运行版本收敛持久化的 `restarting` 状态：达到或超过目标版本即成功，未匹配的重启超过有界等待时间则失败并允许重试。源码 checkout 通过 Git 更新；单文件 CLI 与标准 App Bundle 手动替换，不提供可配置的 Server 端更新源。独立的 `linux-x64-legacy-glibc228` tar 是首次安装引导包：安装器只在需要时启用固定校验的 glibc 2.28 runtime，把包内应用安装到私有 `~/.farming/npm` prefix，并生成稳定兼容 launcher。后续应用版本直接走同一 prefix 的 npm 更新；只有兼容 runtime 本身变化时才需要新的引导包。
...
- 为发布同时新增或更新 `release-notes/vX.Y.Z.md` 与 `release-notes/vX.Y.Z.zh_cn.md`。Package 版本号、Git tag、文件名、H1、中英文互链、大写里程碑代号和升级目标必须严格一致。运行 `node scripts/verify-release-notes.mjs X.Y.Z`；Release workflow 必须从已校验元数据生成 `Farming X.Y.Z · CODENAME` 标题，并以英文 Release Note 作为正文。
