...
启动脚本直接使用目标机器的普通 Node.js 和 native runtime。Farming 不再携带、下载或选择私有系统 C 库。
...
- **Native PTY 无法启动**：检查目标系统的 Node.js 与打包的 `node-pty` runtime 是否兼容；Farming 不提供私有系统 runtime 兼容层。
